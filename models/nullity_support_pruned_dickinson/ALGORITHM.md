...
- `models/support_pruned_dickinson/`, copied as the complete starting implementation. Its recursive support generator comes from
...
Every certificate retains its complete interval signature $(S(u),N_A(u))$ for ordinary Dickinson lookup.
...
dynamic supports, and ordinary interval lookup are unchanged from `support_pruned_dickinson`.

## Complete Decision Flow

1. Require a nonempty square symmetric integer matrix.
...
ordinary lookup, and matrices with many uncovered supports can still approach the full power-set traversal. Large integer entries
