...
struct interval_certificate {
    support lower;
    support upper;
    size_t lower_cardinality = 0;
    size_t upper_cardinality = 0;
};

class cardinality_zdd {
public:
    cardinality_zdd(size_t dimension, size_t cardinality)
        ...
        , current_support_(dimension)
    {
        ...
        timeout_checkpoint();
        remaining_ = cardinality_family(cardinality_);
    }
    ...
    void add_interval(const support& lower, const support& upper, size_t lower_cardinality, size_t upper_cardinality)
    {
        if (cardinality_ < lower_cardinality || cardinality_ > upper_cardinality) return;
        remaining_ = subtract(remaining_, interval_family(lower, upper));
    }
...
public:
    ...
    bool check(const matrix_integer& matrix)
    {
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            cardinality_zdd supports(matrix.rows(), subset_dimension);
            for (const interval_certificate& certificate : certificates_)
                supports.add_interval(
                    certificate.lower, certificate.upper, certificate.lower_cardinality, certificate.upper_cardinality);

            while (supports.take_first(indices_)) {
                timeout_checkpoint();
                diagnostics_.visit_support();
                diagnostics_.secondary();
                COPOSIT_CARDINALITY_ZDD_DIAGNOSTICS("process", subset_dimension);
                if (!process_subset(matrix, supports)) {
                    diagnostics_.finish();
                    return false;
                }
            }
...
private:
    bool process_subset(const matrix_integer& matrix, cardinality_zdd& supports)
    {
        ...
        add_certificate(matrix, supports);
        diagnostics_.certificate();
        return true;
    }

    void add_certificate(const matrix_integer& matrix, cardinality_zdd& supports)
    {
        interval_certificate certificate{support(matrix.rows()), support(matrix.rows())};
        for (size_t local = 0; local < indices_.size(); ++local) {
            if (!solution_(local, 0).is_zero()) {
                certificate.lower.set(indices_[local]);
                ++certificate.lower_cardinality;
            }
        }
        ...
        for (size_t row = 0; row < matrix.rows(); ++row) {
            ...
            if (product_[row].sign() >= 0) {
                certificate.upper.set(row);
                ++certificate.upper_cardinality;
            }
        }
    ...
    std::vector<interval_certificate> certificates_;
...
#ifdef COPOSIT_CARDINALITY_ZDD_DICKINSON_TESTING
std::pair<size_t, size_t> cardinality_zdd_uncovered_count(
    size_t dimension, size_t cardinality, const std::vector<std::pair<uint64_t, uint64_t>>& intervals)
{
    cardinality_zdd diagram(dimension, cardinality);
    for (const auto& [lower_mask, upper_mask] : intervals) {
        ...
        const auto count_bits = [](uint64_t mask) {
            size_t result = 0;
            while (mask != 0) {
                mask &= mask - 1;
                ++result;
        ...
        diagram.add_interval(lower, upper, count_bits(lower_mask), count_bits(upper_mask));
    }
    ...
    while (diagram.take_first(indices)) {
        ...
        diagram.add_interval(exact, exact, cardinality, cardinality);
        ++count;
    }
