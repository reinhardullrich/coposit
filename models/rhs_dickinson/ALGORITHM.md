...
The underlying certificate theorem, ordinary algorithms, and permission to replace $\mathbf1$ by any strictly positive vector come
from:
...
is [`solver.cpp`](solver.cpp). The fixed subset traversal, ordinary coverage theorem, singular-vector rule, negative witness, and
...
larger $|N_A(u)|$, favoring coverage of more outside indices. An exact tie leaves the earlier candidate unchanged. The ordinary
...
solution and cannot change its coverage signature. Even when the ordinary vector already has a global upper set, higher-dimensional
...
The ordinary point $t=0$ was already scored before the direction search. These samples exhaust all possible support and sign
signatures on $t\ge0$.
...
If a direction has no positive root, its signature cannot improve the ordinary candidate, so it is skipped.
...
Dickinson's published construction certifies ordinary copositivity. Coposit instead decides only strict copositivity. For every
...
After all supports have been covered or processed, Dickinson's Lemma 5.2 and Corollary 5.3 imply that a completed ordinary
...
1. Validate that the input is nonempty, square, and exactly symmetric.
...
10. Calculate the ordinary vector's full product and initial coverage width.
11. If $|I|=1$, retain the ordinary vector unchanged.
...
- the ordinary all-ones solve and its negative witness;
...
- replace the ordinary vector only when a searched candidate improves the score.
