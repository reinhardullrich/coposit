...
and 2. The ordinary certificate is Theorem 4.6, and the strict conclusion uses Lemma 5.2 and Corollary 5.3. Dickinson explicitly
...
directly with FLINT integers. A nonpositive value rejects. A positive value proves nothing and invokes the maintained strict-only Dickinson
fallback. Thus the front end has no `double`, tolerance, reconstruction grid, iteration, random choice, or acceptance path.
...
published Dickinson traversal until strict copositivity is decided. A negative witness rejects immediately, and a generated
nonnegative zero also returns `false` immediately. If traversal completes without one, Corollary 5.3 proves strict copositivity.
...
The strict-only termination rule is a Coposit change, not part of the paper's ordinary Algorithm 1: a generated nonnegative zero
...
Timed native-module builds observe a shared signal flag during the one-step row scan and at principal-subset, certificate,
