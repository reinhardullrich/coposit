...
TEST(IntervalRecursiveDickinsonTest, RejectsNonStrictMode)
{
...
TEST(IntervalRecursiveDickinsonTest, DoesNotEmitSupportsAlreadyCoveredByIdentityCertificates)
{
    ...
    interval_recursive_dickinson_trace::clear();
    EXPECT_TRUE(model::solve(identity));

    ASSERT_EQ(interval_recursive_dickinson_trace::events.size(), 4U);
    for (const auto& event : interval_recursive_dickinson_trace::events)
        EXPECT_EQ(event, (interval_recursive_dickinson_trace::event{"process", 1}));
}
