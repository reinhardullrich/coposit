...
calculation. Dickinson Final nevertheless constructs every support and asks afterward whether a certificate covers it.
...
complete copy of `models/dickinson_final`; the exact linear algebra, sign decisions, witnesses, and certificate formula remain the
same.
...
`models/experiments/fracessa/solver.cpp`. General lower-and-upper blocking is the standard Boolean-cube interpretation used by
...
The outer loop retains Dickinson Final's increasing cardinalities $k=1,\ldots,n$. For one $k$, recursion visits bit positions from
...
6. Run Dickinson Final's exact principal solve or nullspace calculation.
...
the same preprocessing implementation and settings used with Dickinson Final.
