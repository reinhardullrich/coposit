# 75%-Remaining Wide-Certificate CBDD-Zed Dickinson
...
$d>3(n-k)/4$ prunes its complete interval $[L,U]$. A certificate with $d\leq3(n-k)/4$ remains mathematically valid and is recorded
...
The identifier is `wide_75_certificate_cbdd_zed_dickinson`. “75” names the sole experimental rule $d>3(n-k)/4$. “Zed” spells out
...
The 75%-of-remaining threshold is an experiment created in coposit; it is not proposed by Dickinson or Bryant and has no claimed
optimality.
...
9. Compute $d=|U|-|L|$. If $d>3(n-k)/4$, add and subtract $[L,U]$; otherwise add and subtract only $\{I\}$. The implementation
...
The 75%-of-remaining rule can deliberately discard nearly all pruning. If generated certificates repeatedly satisfy
$d\leq3(n-k)/4$, the
