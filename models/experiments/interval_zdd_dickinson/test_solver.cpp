...
TEST(IntervalZddDickinsonTest, PreservesStrictDickinsonDecisions)
{
...
TEST(IntervalZddDickinsonTest, RejectsNonStrictMode)
{
    EXPECT_THROW(model::solve(symmetric_matrix(1, {1}), model::copositivity_mode::copositive), std::invalid_argument);
}

TEST(IntervalZddDickinsonTest, SubtractsTheUnionOfBoundedIntervals)
{
...
TEST(IntervalZddDickinsonTest, MatchesBruteForceForEveryPairOfFourDimensionalIntervals)
{
...
TEST(IntervalZddDickinsonTest, DoesNotEmitSupportsAlreadyCoveredByIdentityCertificates)
{
    matrix_integer identity;
    identity.set_identity(4);
    interval_zdd_dickinson_trace::clear();
    EXPECT_TRUE(model::solve(identity));

    ASSERT_EQ(interval_zdd_dickinson_trace::events.size(), 4U);
    for (const auto& event : interval_zdd_dickinson_trace::events)
        EXPECT_EQ(event, (interval_zdd_dickinson_trace::event{"process", 1}));
}

TEST(IntervalZddDickinsonTest, KeepsPackedSupportsBeyondOneWord)
{
...
TEST(IntervalZddDickinsonTest, CooperativeTimeoutEscapesDiagramOperations)
{
    request_timeout();
