...
TEST(ClingoSatZedDickinsonTest, PreservesStrictDickinsonDecisions)
{
...
TEST(ClingoSatZedDickinsonTest, DistinguishesNonStrictFromStrictCopositivity)
{
...
TEST(ClingoSatZedDickinsonTest, ClassifiesBothPredicatesInOneTraversal)
{
...
TEST(ClingoSatZedDickinsonTest, KeepsSingletonCertificatesAcrossLaterCardinalities)
{
    matrix_integer identity;
    identity.set_identity(4);
    clingo_sat_zed_dickinson_trace::clear();
    EXPECT_TRUE(model::solve(identity));

    ASSERT_EQ(clingo_sat_zed_dickinson_trace::events.size(), 4U);
    for (const auto& event : clingo_sat_zed_dickinson_trace::events)
        EXPECT_EQ(event, (clingo_sat_zed_dickinson_trace::event{"process", 1}));
}

TEST(ClingoSatZedDickinsonTest, ExhaustsEverySupportWhenAllDickinsonIntervalsAreSingletons)
{
...
TEST(ClingoSatZedDickinsonTest, PublishesCertificateFreeIndexAndUpperSizeDistributionOnlyWithProgress)
{
    matrix_integer identity;
...
TEST(ClingoSatZedDickinsonTest, AcceptsABoundaryMatrixInNonStrictMode)
{
    EXPECT_TRUE(model::solve(symmetric_matrix(2, {1, -2, 4}), model::copositivity_mode::copositive));
}

TEST(ClingoSatZedDickinsonTest, HandlesSupportAtomsBeyondOneWord)
{
    ...
    ASSERT_EQ(setenv("COPOSIT_CLINGO_SAT_ZED_SCAN", "off", 1), 0);
    EXPECT_FALSE(model::solve(matrix));
    EXPECT_EQ(unsetenv("COPOSIT_CLINGO_SAT_ZED_SCAN"), 0);
}
