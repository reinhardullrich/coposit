    ...
    bool check(const matrix_integer& matrix)
    {
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            while (supports_.take_first(indices_)) {
                ...
                progress_.secondary();
                COPOSIT_ZDD_ZED_TRACE("process", subset_dimension);
...
private:
    bool process_z_block(const matrix_integer& matrix, const std::vector<size_t>& indices)
    {
        ...
        for (size_t start = 0; start < indices.size(); ++start) {
            ...
            COPOSIT_ZDD_ZED_TRACE("z-component", component.size());
            if (!factorization_.is_positive_definite()) {
                COPOSIT_ZDD_ZED_TRACE("z-reject", indices.size());
                return false;
            }
        }

        COPOSIT_ZDD_ZED_TRACE("z-pass", indices.size());
        return true;
    }
...
#ifdef COPOSIT_ZDD_ZED_DICKINSON_TESTING
