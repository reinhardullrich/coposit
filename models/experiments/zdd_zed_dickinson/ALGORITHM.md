...
$Z$-matrix blocks with the interval-ZDD Dickinson support representation.
...
The identifier is `zdd_zed_dickinson`. “Zed” spells out the $Z$ in $Z$-matrix so the extra certificate family remains visible. The
model began as a complete copy of `models/experiments/interval_zdd_dickinson`; its ZDD algebra, principal solve, nullspace vector,
...
If every tested block is PD, the prepass provides no later pruning and its complete cost is overhead before ordinary ZDD Dickinson.
