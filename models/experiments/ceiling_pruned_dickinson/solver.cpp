...
public:
    explicit support_generator(size_t dimension, progress::tracker* progress = nullptr)
    ...
    bool generate(Callback&& callback)
    {
        for (target_cardinality_ = 1; target_cardinality_ <= dimension_; ++target_cardinality_) {
            activate_pending();
            emitted_ = false;
            if (progress_ != nullptr) progress_->support_cardinality(target_cardinality_);
            ...
            if (!emitted_) {
                if (progress_ != nullptr)
                    for (size_t remaining = target_cardinality_ + 1; remaining <= dimension_; ++remaining)
                        progress_->skip_supports(saturated_binomial(dimension_, remaining));
                return true;
            }
        }
        return true;
...
private:
    ...
    bool generate_from(size_t bits_remaining, size_t needed, Callback& callback)
    {
        ...
        if (needed == 0) {
            emitted_ = true;
            if (progress_ != nullptr) progress_->visit_support();
        ...
        if (completes_forbidden(bit)) {
            if (progress_ != nullptr) progress_->skip_supports(saturated_binomial(bit, needed - 1));
        } else {
...
public:
    explicit maximal_z_blocks(const matrix_integer& matrix)
        : adjacency_(matrix.rows(), support(matrix.rows()))
    {
    ...
    bool visit(Visitor&& visitor) const
    {
        ...
        for (size_t index = 0; index < adjacency_.size(); ++index) candidates.set(index);
...
public:
    ...
    bool check(const matrix_integer& matrix)
    {
        const bool result = supports_.generate([&](const support& current, size_t cardinality) {
            ...
            COPOSIT_CEILING_DICKINSON_TRACE("process", cardinality);
...
private:
    bool process_z_block(const matrix_integer& matrix, const std::vector<size_t>& indices)
    {
        ...
        for (size_t start = 0; start < indices.size(); ++start) {
            ...
            COPOSIT_CEILING_DICKINSON_TRACE("z-component", component.size());
            ...
            if (classification_ != nullptr) {
                if (!positive_semidefinite) {
                    COPOSIT_CEILING_DICKINSON_TRACE("z-reject", indices.size());
                    return false;
                }
        ...
        COPOSIT_CEILING_DICKINSON_TRACE("z-pass", indices.size());
        return true;
    }
    ...
    void add_ceiling_certificate(const matrix_integer& matrix)
    {
        ...
        for (size_t row = 0; row < matrix.rows(); ++row) {
            ...
            if (product_.sign() < 0) {
                COPOSIT_CEILING_DICKINSON_TRACE("discard-certificate", indices_.size());
                return;
            }
        }
        ...
        progress_.certificate(matrix.rows() - lower_size);
        COPOSIT_CEILING_DICKINSON_TRACE("ceiling-certificate", lower_size);
    }
