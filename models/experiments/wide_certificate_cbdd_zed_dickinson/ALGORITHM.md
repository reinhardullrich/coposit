# Wide-Certificate CBDD-Zed Dickinson
...
Classification: coposit-created exact copositivity and strict-copositivity experiment copied in full from `cbdd_zed_dickinson`.
...
Only a certificate satisfying $d>n/2$ prunes its complete interval $[L,U]$. A certificate with $d\leq n/2$ remains mathematically
valid and is recorded in progress diagnostics, but it removes only the exact support $I$ that was just processed. Removing $I$ is
...
The identifier is `wide_certificate_cbdd_zed_dickinson`. “Wide certificate” names the sole experimental rule $d>n/2$. “Zed” spells
out the $Z$ in $Z$-matrix, and the `C` in CBDD means chain-reduced. The model is a complete copy of
`models/experiments/cbdd_zed_dickinson`; its Zed prepass, principal solve, nullspace vector, sign decisions, generated Dickinson
...
The half-order threshold is an experiment created in coposit; it is not proposed by Dickinson or Bryant and has no claimed
optimality.
...
9. Compute $d=|U|-|L|$. If $d>n/2$, add and subtract $[L,U]$; otherwise add and subtract only $\{I\}$.
...
The half-order rule can deliberately discard nearly all pruning. If generated certificates repeatedly satisfy $d\leq n/2$, the
