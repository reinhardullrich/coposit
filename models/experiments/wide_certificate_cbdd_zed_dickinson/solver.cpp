...
bool uses_full_certificate_interval(size_t dimension, size_t free_indices) noexcept
{
    return free_indices > dimension / 2;
}
...
private:
    ...
    size_t add_certificate(const matrix_integer& matrix)
    {
        ...
        if (uses_full_certificate_interval(matrix.rows(), free_indices)) {
...
{
    return uses_full_certificate_interval(dimension, free_indices);
}
