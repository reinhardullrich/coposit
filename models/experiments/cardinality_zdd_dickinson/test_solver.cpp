...
TEST(CardinalityZddDickinsonTest, RejectsNonStrictMode)
{
...
TEST(CardinalityZddDickinsonTest, DoesNotEmitSupportsAlreadyCoveredByIdentityCertificates)
{
    ...
    cardinality_zdd_dickinson_trace::clear();
    EXPECT_TRUE(model::solve(identity));

    ASSERT_EQ(cardinality_zdd_dickinson_trace::events.size(), 4U);
    for (const auto& event : cardinality_zdd_dickinson_trace::events)
        EXPECT_EQ(event, (cardinality_zdd_dickinson_trace::event{"process", 1}));
}
