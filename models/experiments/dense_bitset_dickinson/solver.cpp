    ...
    bool check(const matrix_integer& matrix)
    {
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            while (supports_.take_next(indices_)) {
                ...
                progress_.secondary();
                COPOSIT_DENSE_BITSET_TRACE("process", subset_dimension);
...
private:
    ...
    void add_certificate(const matrix_integer& matrix)
    {
        ...
        if (cleared > 0) progress_.skip_supports(cleared);
        progress_.certificate(upper_size - nonzero_locals_.size(), upper_size);
    }
