...
public:
    adaptive_sponsel_copomatrix_checker(size_t maximum_dimension, copositivity_mode mode)
        ...
        , mode_(mode)
        , diagnostics_(diagnostics::metric::adaptive, maximum_dimension)
    {
    }

    ~adaptive_sponsel_copomatrix_checker() { diagnostics_.finish(); }

    bool check(const matrix_integer& matrix, size_t sponsel_streak = 0, long double weight = 1.0L, size_t depth = 0)
    {
        timeout_checkpoint();
        diagnostics_.visit(matrix.rows(), depth);
        bool result;
        if (decide_small(matrix, result)) {
            if (result) diagnostics_.resolved(weight);
            return result;
        }
        ...
        diagnostics_.adaptive_routing(sponsel_streak);
        ...
        if (fmpz_cmp_ui(pivot.children.native_handle(), 2) <= 0 || sponsel_streak >= sponsel_streak_limit) {
            ...
            diagnostics_.adaptive_copomatrix(sponsel_streak, pivot.index, pivot_children, forced);
            return check_copomatrix(matrix, pivot, weight, depth);
        }
        diagnostics_.split();
        diagnostics_.adaptive_sponsel(sponsel_streak, pivot.index, pivot_children);
...
private:
    ...
    bool check_copomatrix(const matrix_integer& matrix, const copomatrix_pivot& pivot, long double weight, size_t depth)
    {
        ...
        const double child_count = diagnostics_.active() ? fmpz_get_d(pivot.children.native_handle()) : 0.0;
        ...
        diagnostics_.adaptive_stage(diagnostics::adaptive_engine::copomatrix,
                                 diagnostics::adaptive_phase::copomatrix_partition, 0, dimension);
        ...
        diagnostics_.adaptive_stage(diagnostics::adaptive_engine::copomatrix,
                                 diagnostics::adaptive_phase::principal_block, 0, child_dimension);
        ...
        diagnostics_.adaptive_stage(diagnostics::adaptive_engine::copomatrix,
                                 diagnostics::adaptive_phase::schur_block, 0, child_dimension);
    ...
    {
        diagnostics_.adaptive_copomatrix_child();
        const size_t dimension = matrix.rows();
        for (size_t i = 0; i < dimension; ++i) {
            timeout_checkpoint();
            if (diagonal_fails(matrix(i, i))) return false;
    ...
    {
        timeout_checkpoint();
        diagnostics_.adaptive_copomatrix_staircase();
        ...
        if (positive_begin == positive.size()) {
            ...
            diagnostics_.adaptive_stage(diagnostics::adaptive_engine::copomatrix,
                                     diagnostics::adaptive_phase::transform, 0, rays.size());
    ...
    {
        const size_t dimension = matrix.rows();
        diagnostics_.adaptive_stage(diagnostics::adaptive_engine::sponsel, diagnostics::adaptive_phase::edge_scan, 0, dimension);
        ...
        diagnostics_.adaptive_stage(diagnostics::adaptive_engine::sponsel, diagnostics::adaptive_phase::split_build);
        ...
        diagnostics_.adaptive_sponsel_split();
        ...
        return check(children.second, child_streak, diagnostics_.active() ? weight - first_weight : 0.0L, depth + 1);
    }

    bool passes_h_certificate(const matrix_integer& gram)
    {
        const size_t dimension = gram.rows();
        diagnostics_.adaptive_stage(diagnostics::adaptive_engine::sponsel, diagnostics::adaptive_phase::h_build, 0, dimension);
        ...
        diagnostics_.adaptive_stage(diagnostics::adaptive_engine::sponsel,
                                 diagnostics::adaptive_phase::h_factorization, 0, dimension);
        h_factorization_.factorize_inplace(stripped, false, &diagnostics_);
