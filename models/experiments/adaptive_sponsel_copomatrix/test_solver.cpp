...
TEST(AdaptiveSponselCopomatrixModelTest, ReportsSponselAndCopomatrixWorkSeparately)
{
    ...
    EXPECT_TRUE(model::solve(symmetric_matrix(4, {4, -1, 1, -1, 4, -1, 1, 4, -1, 4})));
    const diagnostics::snapshot sponsel = diagnostics::detail::load();
    EXPECT_EQ(sponsel.kind, diagnostics::metric::adaptive);
    ...
    identity.set_identity(4);
    EXPECT_TRUE(model::solve(identity));
    const diagnostics::snapshot copomatrix = diagnostics::detail::load();
    diagnostics::detail::state.enabled.store(false, std::memory_order_relaxed);
    diagnostics::detail::reset();

    EXPECT_EQ(copomatrix.kind, diagnostics::metric::adaptive);
