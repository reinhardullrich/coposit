...
Mode boundary: `solve(A, copositivity_mode::copositive)` decides non-strict copositivity and
...
Callers may apply the shared fused component/pre-check pipeline before entering this solver or invoke the model directly. The model
itself owns only the algorithm described below.
...
3. Otherwise apply Sponsel's non-strict or strict `H` certificate and exact Bundfuss edge split.
...
The maintained exact non-strict reconstruction and strict `H` adaptation are documented in
...
Its maintained exact non-strict implementation and strict adaptation are documented in
[`../baselines/copomatrix_2011/ALGORITHM.md`](../baselines/copomatrix_2011/ALGORITHM.md). The adaptive routing and cutoff follow the
structure of [`../experiments/adaptive_dutour_copomatrix/`](../experiments/adaptive_dutour_copomatrix/), with Sponsel replacing Dutour as the same-order
operation.
...
For a nonempty square symmetric integer matrix $A$, non-strict mode decides
...
Equality is accepted in non-strict mode and rejected in strict mode. The input parser supplies a nonempty square exactly symmetric
matrix; the model assumes that contract without revalidating it. A cooperative timeout remains unresolved and is never converted to
`false`.
...
The checker also retains the selected mode for the entire traversal. Children never change from non-strict to strict rules or
vice versa.
...
2. Reject a negative diagonal in non-strict mode or a nonpositive diagonal in strict mode.
...
check(G, streak, mode):
    ...
    if non-strict mode and any diagonal entry is < 0:
        return false
...
Order zero is internally true, although parsed input cannot be empty. Order one passes when its sole diagonal is nonnegative in
non-strict mode and positive in strict mode.
...
non-strict order two passes exactly when $a,c\geq0$ and either $b\geq0$ or $ac-b^2\geq0$. Strict order two replaces the three
...
criterion, with equality accepted only in non-strict mode. The implementation calls the shared integer-only
...
p&B
\end{pmatrix},
\qquad a\geq0\text{ in non-strict mode},
...
The principal child $B$ is always required. When $a=0$ in non-strict mode, no Schur matrix is formed. If any component of $p$ is
...
where $m$ is the selected mode, $\triangleright_m$ is $\geq$ in non-strict mode and $>$ in strict mode, and
...
For a non-strict-mode zero diagonal, the actual operation creates only the principal child and then either rejects on a negative
...
The selected edge rejects non-strict copositivity exactly when
...
Thus equality is retained as a valid zero in non-strict mode and rejected in strict mode.
...
In non-strict mode, the node is accepted if
...
- a diagonal entry is negative in non-strict mode or nonpositive in strict mode;
- a direct low-order criterion fails;
- a zero non-strict COPOMATRIX pivot has a negative off-diagonal component;
...
- the non-strict positive-semidefinite or strict positive-definite Sponsel `H` certificate passes;
...
the current call stack and pending siblings, but every pending Sponsel sibling is a full dense exact matrix. Its base storage therefore
grows as $O(dn^2)$ for same-order branch depth $d$. Memory can become limiting around order 1,000 and above, or earlier on an unusually
...
- non-strict nonnegative diagonals and strict positive diagonals;
- minimum negative-entry edge selection and first-tie rule;
- non-strict $\gamma^2>\alpha\beta$ and strict $\gamma^2\geq\alpha\beta$ edge rejection;
- the stripped `H` matrix;
- non-strict positive-semidefinite and strict positive-definite `H` certification;
...
- the non-strict zero-pivot rule;
...
The cutoff guarantees dimensional diagnostics but may replace a deep binary refinement with a very broad projection.
...
In non-strict mode a boundary zero is valid and must eventually be covered rather than treated as rejection. Before the cutoff, the
...
|---|---|
| `solve` | Retain the selected mode, allocate reusable LDLT storage, and start at streak zero. |
...
| `diagonal_fails` | Apply nonnegative non-strict or positive strict diagonal semantics consistently. |
