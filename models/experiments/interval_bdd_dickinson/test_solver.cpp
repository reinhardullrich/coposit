...
TEST(IntervalBddDickinsonTest, PreservesStrictDickinsonDecisions)
{
...
TEST(IntervalBddDickinsonTest, RejectsNonStrictMode)
{
    EXPECT_THROW(model::solve(symmetric_matrix(1, {1}), model::copositivity_mode::copositive), std::invalid_argument);
}

TEST(IntervalBddDickinsonTest, SubtractsTheUnionOfBoundedIntervals)
{
...
TEST(IntervalBddDickinsonTest, MatchesBruteForceForEveryPairOfFourDimensionalIntervals)
{
    ...
    for (const auto& first : valid_intervals) {
        for (const auto& second : valid_intervals) {
            ...
            for (size_t cardinality = 1; cardinality <= dimension; ++cardinality)
                EXPECT_EQ(model::interval_bdd_uncovered_count(dimension, cardinality, intervals).first,
                          brute_uncovered_count(dimension, cardinality, intervals));
        }
    }
}

TEST(IntervalBddDickinsonTest, DoesNotEmitSupportsAlreadyCoveredByIdentityCertificates)
{
    matrix_integer identity;
    identity.set_identity(4);
    interval_bdd_dickinson_trace::clear();
    EXPECT_TRUE(model::solve(identity));

    ASSERT_EQ(interval_bdd_dickinson_trace::events.size(), 4U);
    for (const auto& event : interval_bdd_dickinson_trace::events)
        EXPECT_EQ(event, (interval_bdd_dickinson_trace::event{"process", 1}));
}

TEST(IntervalBddDickinsonTest, KeepsPackedSupportsBeyondOneWord)
{
...
TEST(IntervalBddDickinsonTest, CooperativeTimeoutEscapesDiagramOperations)
{
    request_timeout();
    EXPECT_THROW(model::interval_bdd_uncovered_count(15, 7, {}), timeout_requested);
    reset_timeout();
}
