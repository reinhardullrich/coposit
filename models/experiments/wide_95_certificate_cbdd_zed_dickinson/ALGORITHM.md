# 95%-Remaining Wide-Certificate CBDD-Zed Dickinson
...
$d>19(n-k)/20$ prunes its complete interval $[L,U]$. A certificate with $d\leq19(n-k)/20$ remains mathematically valid and is
recorded in progress diagnostics, but it removes only the exact support $I$ that was just processed. Removing $I$ is necessary
bookkeeping: extracting the first support from the CBDD does not mutate it, so leaving $I$ in the remaining family would return the
same support forever.
...
The identifier is `wide_95_certificate_cbdd_zed_dickinson`. “95” names the sole experimental rule $d>19(n-k)/20$. “Zed” spells out
...
The 95%-of-remaining threshold is an experiment created in coposit; it is not proposed by Dickinson or Bryant and has no claimed
optimality.
...
9. Compute $d=|U|-|L|$. If $d>19(n-k)/20$, add and subtract $[L,U]$; otherwise add and subtract only $\{I\}$. The implementation
...
The 95%-of-remaining rule can deliberately discard nearly all pruning. If generated certificates repeatedly satisfy
$d\leq19(n-k)/20$, the
