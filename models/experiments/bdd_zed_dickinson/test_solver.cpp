...
std::pair<size_t, size_t> bdd_zed_uncovered_count(
...
TEST(BddZedDickinsonTest, PreservesStrictDickinsonDecisions)
{
...
TEST(BddZedDickinsonTest, RejectsNonStrictMode)
{
    EXPECT_THROW(model::solve(symmetric_matrix(1, {1}), model::copositivity_mode::copositive), std::invalid_argument);
}

TEST(BddZedDickinsonTest, SubtractsTheUnionOfBoundedIntervals)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011}, {0b100, 0b110}};
    EXPECT_EQ(model::bdd_zed_uncovered_count(3, 1, intervals).first, 1U);
    EXPECT_EQ(model::bdd_zed_uncovered_count(3, 2, intervals).first, 1U);
}

TEST(BddZedDickinsonTest, RepresentsAWholeZedDownsetWithAnEmptyLowerEndpoint)
{
    const std::vector<std::pair<uint64_t, uint64_t>> downset{{0, 0b101}};
    EXPECT_EQ(model::bdd_zed_uncovered_count(3, 1, downset).first, 1U);
    EXPECT_EQ(model::bdd_zed_uncovered_count(3, 2, downset).first, 2U);
    EXPECT_EQ(model::bdd_zed_uncovered_count(3, 3, downset).first, 1U);
}

TEST(BddZedDickinsonTest, MatchesBruteForceForEveryPairOfFourDimensionalIntervals)
{
    ...
    for (const auto& first : valid_intervals) {
        for (const auto& second : valid_intervals) {
            ...
            for (size_t cardinality = 1; cardinality <= dimension; ++cardinality)
                EXPECT_EQ(model::bdd_zed_uncovered_count(dimension, cardinality, intervals).first,
                          brute_uncovered_count(dimension, cardinality, intervals));
        }
    }
}

TEST(BddZedDickinsonTest, KeepsPackedSupportsBeyondOneWord)
{
...
TEST(BddZedDickinsonTest, CooperativeTimeoutEscapesDiagramOperations)
{
    request_timeout();
    EXPECT_THROW(model::bdd_zed_uncovered_count(15, 7, {}), timeout_requested);
    reset_timeout();
}
