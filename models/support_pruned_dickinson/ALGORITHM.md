...
ordinary Dickinson lookup. The new pruning is therefore a change in how already-proved redundant supports are generated, not a new
copositivity test.
...
- FracESSA revision `95e0ec019cf11a60c6423508e8768536a0b88860`, especially
  ...
  `cpp/src/fracessa.cpp`. Coposit's maintained local reconstruction is `models/fracessa/solver.cpp`.
...
adding that negative-product index can leave the interval. Such a signature is stored for ordinary coverage but is never made a
generator rule.
...
2. it preserves the current cardinality's ordinary Dickinson order and behavior.
...
strictly copositive. A passing check does not create a certificate. The model still performs ordinary Dickinson coverage and, when
...
5. stores $(S(w),N_A(w))$ for ordinary Dickinson interval lookup.

The full vector and product values are then discarded.

## Strict-Only Decision

Dickinson's published Algorithms 1 and 2 decide ordinary copositivity. Coposit asks only for strict copositivity.
...
nonnegative zero proves that no such zero exists and returns `true`.
...
1. Require a nonempty square symmetric matrix.
...
5. Recursively construct the next support in numeric-mask order, cutting off a branch as soon as it contains an active forbidden
   support.
6. Apply the shared direct test to emitted supports of size at most three; return `false` on failure.
...
9. Return `false` on a nonpositive nonsingular solution or a nonnegative singular null vector.
