...
namespace coposit::model::adaptive_zischg_sponsel_copomatrix_testing {
...
TEST(AdaptiveZischgSponselCopomatrixModelTest, UsesDirectStopsThroughOrderThree)
{
...
TEST(AdaptiveZischgSponselCopomatrixModelTest, UsesSponselHCertificateWhenEveryPivotIsWide)
{
    EXPECT_TRUE(model::solve(symmetric_matrix(4, {4, -1, 1, -1, 4, -1, 1, 4, -1, 4})));
}

TEST(AdaptiveZischgSponselCopomatrixModelTest, RetainsTheExactSponselSplit)
{
    EXPECT_TRUE(model::solve(symmetric_matrix(4, {5, -3, 2, -3, 5, -3, 2, 5, -3, 5})));
}

TEST(AdaptiveZischgSponselCopomatrixModelTest, DoesNotDecomposeAnOrdinarySponselNode)
{
    using namespace model::adaptive_zischg_sponsel_copomatrix_testing;
    const matrix_integer disconnected_wide = symmetric_matrix(
        6, {4, -1, -1, 1, 1, 1, 4, -1, 1, 1, 1, 4, 1, 1, 1, 4, -1, -1, 4, -1, 4});

    EXPECT_TRUE(model::solve(disconnected_wide));
    EXPECT_EQ(decomposition_count(), 0U);
}

TEST(AdaptiveZischgSponselCopomatrixModelTest, ForcesCopomatrixAtTheTenThousandSplitCutoff)
{
    using namespace model::adaptive_zischg_sponsel_copomatrix_testing;
    const matrix_integer wide = symmetric_matrix(4, {4, -1, 1, -1, 4, -1, 1, 4, -1, 4});

    EXPECT_EQ(streak_limit(), 10000U);
    EXPECT_TRUE(solve_with_streak(wide, streak_limit()));
}

TEST(AdaptiveZischgSponselCopomatrixModelTest, DecomposesOnlyAfterCopomatrixProjection)
{
    using namespace model::adaptive_zischg_sponsel_copomatrix_testing;

    EXPECT_TRUE(model::solve(symmetric_matrix(5, {1, 0, 0, 0, 0, 2, -1, 0, 0, 2, 0, 0, 2, -1, 2})));
    EXPECT_GE(decomposition_count(), 1U);

    EXPECT_FALSE(model::solve(symmetric_matrix(5, {1, 0, 0, 0, 0, 1, -1, 0, 0, 1, 0, 0, 2, -1, 2})));
    EXPECT_GE(decomposition_count(), 1U);
}

TEST(AdaptiveZischgSponselCopomatrixModelTest, ClassifiesRetainedBoundaryRegression)
{
    EXPECT_FALSE(model::solve(symmetric_matrix(5, {1, -1, 1, 2, -3, 2, -3, -3, 4, 5, 6, -4, 5, -8, 16})));
}

TEST(AdaptiveZischgSponselCopomatrixModelTest, PreservesArbitraryPrecisionScaling)
{
    integer scale;
...
TEST(AdaptiveZischgSponselCopomatrixModelTest, HasNoFormerDimensionLimit)
{
    matrix_integer identity;
...
TEST(AdaptiveZischgSponselCopomatrixModelTest, RejectsInvalidMatrixShapes)
{
