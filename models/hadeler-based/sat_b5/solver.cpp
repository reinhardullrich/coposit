...
public:
    ...
    bool check(const matrix_integer& matrix)
    {
        ...
        while (low <= matrix.rows()) {
            if (!process_low_support(matrix, low)) return finish(false);
...
private:
    bool process_low_support(const matrix_integer& matrix, size_t& low)
    {
        while (low <= matrix.rows()) {
            ...
            COPOSIT_SAT_B5_DIAGNOSTICS("stage_low", low);
    ...
    bool process_selected_support(const matrix_integer& matrix)
    {
        ...
        COPOSIT_SAT_B5_DIAGNOSTICS("process_low", indices_.size());
    ...
    void install_pair_curvature_exclusions(const matrix_integer& matrix)
    {
        ...
        for (size_t first = 0; first < matrix.rows(); ++first) {
            ...
            for (size_t second = first + 1; second < matrix.rows(); ++second) {
                ...
                COPOSIT_SAT_B5_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_SAT_B5_TESTING
    ...
    bool build_schur_reduction(const matrix_integer& matrix, matrix_integer& reduced)
    {
        ...
        for (const size_t outside : outside_indices_) {
            ...
            if (column_sum.sign() > 0) {
                COPOSIT_SAT_B5_DIAGNOSTICS("schur_prefilter_reject", indices_.size());
                return false;
            }
        }
    ...
    bool try_schur_reduction(const matrix_integer& matrix)
    {
        ...
        COPOSIT_SAT_B5_DIAGNOSTICS("schur_reduction", indices_.size());
...
#endif
            COPOSIT_SAT_B5_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    bool add_certificate()
    {
        ...
        COPOSIT_SAT_B5_DIAGNOSTICS("dickinson", indices_.size());
        support_context_.release(std::move(lower));
        support_context_.release(std::move(upper));
        return true;
    }
    ...
    void add_upward_closure()
    {
        ...
        COPOSIT_SAT_B5_DIAGNOSTICS("support_upward", indices_.size());
    }
...
#ifdef COPOSIT_SAT_B5_TESTING
bool sat_b5_prefers_negative_singular_orientation_for_testing(size_t positive_products,
...
size_t sat_b5_combined_ray_sweep_count_for_testing() noexcept
{
    return last_combined_ray_sweep_count;
}

size_t sat_b5_combined_ray_improvement_count_for_testing() noexcept
{
    return last_combined_ray_improvement_count;
}

size_t sat_b5_pair_curvature_exclusion_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t sat_b5_support_curvature_exclusion_count_for_testing() noexcept
{
...
bool sat_b5_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer& matrix, const std::vector<size_t>& indices)
{
...
size_t sat_b5_shortlist_limit_for_testing(size_t matrix_dimension, size_t support_dimension)
{
    return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool sat_b5_prefers_ray_candidate_for_testing(size_t candidate_upper, size_t candidate_width, size_t candidate_gains,
...
size_t sat_b5_fixed_support_upper_size_for_testing() noexcept
{
...
size_t sat_b5_pair_exclusion_uncovered_count(
...
size_t sat_b5_interval_clause_size(size_t dimension, uint64_t lower_mask, uint64_t upper_mask)
{
    support_context support_context_(dimension);
    interval_sat diagram(support_context_);
    support lower = support_context_.make();
