...
Classification: coposit-created exact CP/SCP experiment. SAT-B3 alternates between low and high cardinalities. The low frontier uses
...
SAT-B3 uses that observation as a search certificate.
...
The identifier is `sat_b3`.

- **SAT** names the incremental Boolean representation of the unresolved supports.
- **B3** means the third experiment in the curvature-based SAT line.

The model is an independent experiment copied from [`sat_b2`](../sat_b2/ALGORITHM.md). It changes only the high-frontier downward
certificate: the floating filter also admits likely singular positive-semidefinite supports, and the exact stage accepts such a
support when $Bx=\mathbf1$ is consistent. It retains SAT-B2's traversal, low-frontier Halfspace-Rays machinery, SAT clauses, and
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, SAT-B3 solves
...
When low-frontier curvature does not already remove the upward closure, SAT-B3 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. SAT-B3 performs the inherited exact
...
SAT-B3 adds one singular case. Suppose
...
SAT-B3 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
