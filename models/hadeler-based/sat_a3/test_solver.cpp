...
size_t sat_a3_monotone_lp_attempt_count_for_testing() noexcept;
size_t sat_a3_monotone_lp_feasible_count_for_testing() noexcept;
size_t sat_a3_monotone_extension_count_for_testing() noexcept;
size_t sat_a3_monotone_exact_rejection_count_for_testing() noexcept;
size_t sat_a3_monotone_incumbent_upper_size_for_testing() noexcept;
size_t sat_a3_monotone_final_upper_size_for_testing() noexcept;
...
TEST(SatA3Test, PreservesExactCombinedClassifications)
{
...
TEST(SatA3Test, SelectsAnExactImprovingHalfspaceDirection)
{
    EXPECT_TRUE(model::solve(symmetric_matrix(4, {6, -4, 5, -5, 6, -7, 5, 10, -10, 14})));
    EXPECT_GT(model::sat_a3_optimized_certificate_count_for_testing(), 0U);
}

TEST(SatA3Test, ExactMonotoneExtensionAddsAnIndexWithoutLosingTheRaysUpperSet)
{
    ...
    EXPECT_TRUE(model::sat_a3_certificate_for_testing(matrix, {0, 1, 2}, lower, upper));
    EXPECT_GT(model::sat_a3_monotone_lp_attempt_count_for_testing(), 0U);
    EXPECT_GT(model::sat_a3_monotone_lp_feasible_count_for_testing(), 0U);
    EXPECT_GT(model::sat_a3_monotone_extension_count_for_testing(), 0U);
    EXPECT_EQ(model::sat_a3_monotone_incumbent_upper_size_for_testing(), 5U);
    EXPECT_GT(model::sat_a3_monotone_final_upper_size_for_testing(), 5U);
    for (size_t index = 0; index < 9; ++index)
        if (model::sat_a3_monotone_incumbent_contains_for_testing(index)) EXPECT_TRUE(context.contains(upper, index));
}

TEST(SatA3Test, ChoosesTheSingularOrientationWithTheLargerUpperSet)
{
    EXPECT_TRUE(model::sat_a3_prefers_negative_singular_orientation_for_testing(1, 2));
    EXPECT_FALSE(model::sat_a3_prefers_negative_singular_orientation_for_testing(2, 1));
    EXPECT_FALSE(model::sat_a3_prefers_negative_singular_orientation_for_testing(2, 2));
}

TEST(SatA3Test, UsesWidthAsTheSecondaryObjective)
{
    const matrix_integer matrix = symmetric_matrix(2, {2, 1, 2});
    EXPECT_TRUE(model::sat_a3_check_support_for_testing(matrix, {0, 1}));
    EXPECT_EQ(model::sat_a3_fixed_support_upper_size_for_testing(), 2U);
    EXPECT_GT(model::sat_a3_optimized_certificate_count_for_testing(), 0U);
}

TEST(SatA3Test, ExposesTheExactOptimizedFixedSupportCertificate)
{
    ...
    EXPECT_TRUE(model::sat_a3_certificate_for_testing(identity, {0}, lower, upper));
...
TEST(SatA3Test, AdaptiveShortlistRemainsBounded)
{
    EXPECT_EQ(model::sat_a3_shortlist_limit_for_testing(5, 5), 5U);
    EXPECT_EQ(model::sat_a3_shortlist_limit_for_testing(80, 80), 27U);
    EXPECT_EQ(model::sat_a3_shortlist_limit_for_testing(3000, 3000), 64U);
    EXPECT_EQ(model::sat_a3_shortlist_limit_for_testing(3000, 10), 10U);
}

TEST(SatA3Test, RayRetentionNeverTradesWidthForIncidentalGains)
{
    EXPECT_FALSE(model::sat_a3_prefers_ray_candidate_for_testing(20, 4, 8, 0, 20, 5, 1, 0));
    EXPECT_TRUE(model::sat_a3_prefers_ray_candidate_for_testing(20, 5, 8, 0, 20, 5, 1, 0));
}

TEST(SatA3Test, CombinedRayEscapesACoordinateWiseUpperMaximum)
{
    ...
    EXPECT_TRUE(model::sat_a3_check_support_for_testing(matrix, {0, 1, 2}));
    EXPECT_EQ(model::sat_a3_fixed_support_upper_size_for_testing(), 6U);
    EXPECT_EQ(model::sat_a3_combined_ray_sweep_count_for_testing(), 1U);
    EXPECT_EQ(model::sat_a3_combined_ray_improvement_count_for_testing(), 1U);
}

TEST(SatA3Test, SearchesAtMostTheBestTwoDistinctCombinedRays)
{
    ...
    EXPECT_TRUE(model::sat_a3_check_support_for_testing(matrix, {0, 1, 2, 3}));
    EXPECT_EQ(model::sat_a3_combined_ray_sweep_count_for_testing(), 2U);
}

TEST(SatA3Test, StoresTheUnionOfDickinsonIntervalsInSat)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011}, {0b100, 0b110}};
    EXPECT_EQ(model::sat_a3_uncovered_count(3, 1, intervals), 1U);
    EXPECT_EQ(model::sat_a3_uncovered_count(3, 2, intervals), 1U);
}

TEST(SatA3Test, AddsTheExistingCardinalityOutputOnlyToExpiringIntervals)
{
    EXPECT_EQ(model::sat_a3_interval_clause_size(8, 0b00000001, 0b00000111), 7U);
    EXPECT_EQ(model::sat_a3_interval_clause_size(8, 0b00000001, 0b11111111), 1U);
}

TEST(SatA3Test, PublishesTheChosenCertificateDistribution)
{
    matrix_integer identity;
...
TEST(SatA3Test, RecordsFactoredSingularSupportsOnlyWhenDiagnosticsAreEnabled)
{
...
TEST(SatA3Test, CooperativeTimeoutInterruptsSat)
{
    request_timeout();
    EXPECT_THROW(model::sat_a3_uncovered_count(15, 7, {}), timeout_requested);
    reset_timeout();
}
