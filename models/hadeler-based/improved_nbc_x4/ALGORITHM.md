...
Classification: coposit-created exact CP/SCP experiment. Improved NBC-X4 alternates between low and high cardinalities. The low frontier uses
...
optimized certificate may be offered to a targeted continuous LP that tries to add one missing upper index without losing an existing
one. X4 first shrinks the ordinary certificate and skips the LP when that already gives the maximum possible interval. Otherwise, a
numerical LP point is only a proposal: X4 reconstructs and checks it with exact integers, shrinks it, and keeps it only when its final
exact interval strictly contains the already-shrunk ordinary interval. The high
...
Improved NBC-X4 uses that observation as a search certificate.
...
- If a low-frontier face is strictly convex, the retained factorization is reused to build and optimize a Dickinson interval. This
  replaces the former exact-support block. X4 first deletes dispensable used coordinates from the ordinary vector. If this reaches
  the maximum interval, it stops there. Otherwise, a small LP asks whether a different positive right-hand side could retain the
  original upper endpoint and add one chosen missing index. A proposed vector is reconstructed and shrunk exactly. It is used only
  when its final interval strictly contains the already-shrunk ordinary interval.
...
The identifier is `improved_nbc_x4`. It is an experiment derived from `improved_nbc_x3`.

- **Improved NBC** names the separately owned resumable derivative of NBC MiniSat All.
- **X4** names X3 with the ordinary lower-endpoint shrink moved before the optional targeted LP.

The model is an independent copy of [`improved_nbc_x3`](../improved_nbc_x3/ALGORITHM.md). It preserves X3's traversal, tests,
Halfspace-Rays construction, targeted LP, exact reconstruction, shrinking, clauses, and compaction. Its only change is the order:
shrink the ordinary result first, skip an unnecessary LP when that result is already maximal, and otherwise compare the shrunk LP
candidate directly with the retained shrunk baseline. `improved_nbc_x3` remains unchanged as the direct comparison model.
The dense simplex code is a private reduced copy of the implementation in
[`sat_halfspace_milp_dickinson`](../sat_halfspace_milp_dickinson/ALGORITHM.md); X4 uses no MILP variables or branch-and-bound.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Improved NBC-X4 solves
...
When low-frontier curvature does not already remove the upward closure, Improved NBC-X4 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Improved NBC-X4 performs the inherited exact
...
For one outside target $j\notin U_R$, X4 numerically solves
...
The implementation parameterizes $b_i=\varepsilon+(1-|I|\varepsilon)y_i$ with $y\ge0$ and $\mathbf1^Ty=1$, so the LP itself has
only nonnegative variables. Targets are tried in increasing index order, and the first exactly verified improvement stops the scan.
The LP does not maximize $|U|$ and does not claim optimality over all right-hand sides. Other missing indices may nevertheless become
nonnegative at the same point.

The LP is deliberately non-authoritative. If its floating solution has a nonnegative numerical margin, X4 rounds the relative
...
this optional enlargement; the exact ordinary branch is unchanged. If the dense tableau would exceed 8,388,608 cells, X4 also
skips this optional stage and uses the ordinary result.
...
X4 first applies that shrink to the ordinary Rays vector. If its interval has distance $n-1$, no larger interval exists and the LP is
skipped. Otherwise X4 restores the unshrunk Rays state for the unchanged LP proposal, then shrinks an exactly verified LP vector.
Let their final intervals be
...
carry a worse lower endpoint after shrinking. X4 inserts only one interval, never two nearly overlapping alternatives.
...
Improved NBC-X4 retains SAT-B3's singular case. Suppose
...
3. Ask the low frontier for one open support. Copy and exactly factor its principal matrix. Install an upward closure immediately
   ...
   a nonsingular certificate with a missing outside upper index, first shrink the ordinary Rays interval. If it is maximal, insert it
   immediately. Otherwise restore the original Rays state, run the targeted LP, and exactly reconstruct and shrink its first accepted
   proposal. Insert that proposal only if exact containment proves that it strictly contains the retained ordinary interval.
...
the exact ordinary branch or final classification.
...
Improved NBC-X4 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
...
that case X4 pays the LP cost and deliberately inserts the already-shrunk ordinary certificate.
