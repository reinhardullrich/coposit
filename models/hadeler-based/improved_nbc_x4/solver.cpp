...
#ifdef COPOSIT_IMPROVED_NBC_X4_TESTING
...
std::uint64_t last_x4_interval_ns = 0;
#endif
...
public:
  ...
  bool optimize_support_for_testing(const matrix_integer &matrix,
    ...
    last_x4_interval_ns = static_cast<std::uint64_t>(
...
private:
  ...
  void record_interval_certificate(const support &lower, const support &upper) {
    ...
    event << "model=improved_nbc_x4 n=" << product_.size()
  ...
  bool process_low_support(const matrix_integer &matrix, size_t &low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X4_DIAGNOSTICS("stage_low", low);
  ...
  bool process_high_support(const matrix_integer &matrix, size_t low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X4_DIAGNOSTICS("stage_high", high);
  ...
  bool process_selected_support(const matrix_integer &matrix,
    ...
    COPOSIT_IMPROVED_NBC_X4_DIAGNOSTICS(
  ...
  void install_pair_curvature_exclusions(const matrix_integer &matrix) {
    ...
    for (size_t first = 0; first < matrix.rows(); ++first) {
      ...
      for (size_t second = first + 1; second < matrix.rows(); ++second) {
        ...
        COPOSIT_IMPROVED_NBC_X4_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_IMPROVED_NBC_X4_TESTING
  ...
  bool process_subset(const matrix_integer &matrix,
    ...
    if (!from_low_frontier) {
      ...
      if (!floating_filter_.looks_positive_semidefinite(indices_)) {
        ...
        COPOSIT_IMPROVED_NBC_X4_DIAGNOSTICS("high_float_reject",
                                            indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X4_TESTING
  ...
  bool process_nonsingular_subset(const matrix_integer &matrix) {
    ...
    const coverage_score rays_score = current_score_;
    shrink_lower_preserving_upper(matrix);
    current_score_ = score(solution_, 0, product_);
    if (current_score_.width + 1 == matrix.rows())
      return add_certificate();
    baseline_solution_ = solution_;
    baseline_product_ = product_;
    const coverage_score baseline_score = current_score_;

    solution_ = rays_solution_;
    product_ = rays_product_;
    current_score_ = rays_score;
    ...
    if (lp_result == exact_lp_candidate_result::unchanged) {
      solution_ = baseline_solution_;
      product_ = baseline_product_;
      current_score_ = baseline_score;
      return add_certificate();
    }

    shrink_lower_preserving_upper(matrix);
    if (interval_strictly_dominates(baseline_solution_, baseline_product_,
                                    solution_, product_)) {
      current_score_ = score(solution_, 0, product_);
#ifdef COPOSIT_IMPROVED_NBC_X4_TESTING
  ...
  exact_lp_candidate_result try_targeted_enlargement() {
    ...
    lp_scaled_rows_.assign(outside_dimension,
                           std::vector<double>(support_dimension, 0.0));
    ...
    for (size_t target = 0; target < outside_dimension; ++target) {
      ...
      detail::tiny_simplex lp(lp_required_rows_.size() + 2,
                              support_dimension + 2);
      size_t constraint = 0;
      for (const size_t required : lp_required_rows_) {
        const auto &coefficients = lp_scaled_rows_[required];
        double sum = 0.0;
        for (size_t column = 0; column < support_dimension; ++column) {
          sum += coefficients[column];
          lp.set_coefficient(constraint, column,
                             -simplex_scale * coefficients[column]);
        }
        lp.set_coefficient(constraint, support_dimension, 1.0);
        lp.set_coefficient(constraint, support_dimension + 1, -1.0);
        lp.set_rhs(constraint, positive_floor * sum);
        ++constraint;
      }
      for (size_t column = 0; column < support_dimension; ++column) {
        lp.set_coefficient(constraint, column, 1.0);
        lp.set_coefficient(constraint + 1, column, -1.0);
      }
      lp.set_rhs(constraint, 1.0);
      lp.set_rhs(constraint + 1, -1.0);
      lp.set_objective(support_dimension, 1.0);
      lp.set_objective(support_dimension + 1, -1.0);
      ...
      if (status != detail::tiny_simplex::status::optimal ||
          proposal[support_dimension] - proposal[support_dimension + 1] < -1e-9)
        continue;
#ifdef COPOSIT_IMPROVED_NBC_X4_TESTING
      ++targeted_lp_feasible_count_;
#endif
      proposal.resize(support_dimension);
      for (size_t column = 0; column < support_dimension; ++column)
  ...
  static bool lp_problem_fits(size_t support_dimension,
    ...
    return support_dimension == 0 ||
           outside_dimension + 4 <=
               maximum_tableau_cells / (support_dimension + 4);
  }
...
#endif
      COPOSIT_IMPROVED_NBC_X4_DIAGNOSTICS("combined_ray", ray + 1);
  ...
  bool add_certificate() {
    ...
    support_context_.release(std::move(lower));
    support_context_.release(std::move(upper));
    COPOSIT_IMPROVED_NBC_X4_DIAGNOSTICS("dickinson", indices_.size());
    return true;
  }
  ...
  void add_upward_closure() {
    ...
    COPOSIT_IMPROVED_NBC_X4_DIAGNOSTICS("support_upward", indices_.size());
  }
  ...
  void add_downward_closure(std::string_view kind) {
    ...
    record_closure_certificate(kind, "downward", certificate_frontier_,
                               indices_);
    COPOSIT_IMPROVED_NBC_X4_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X4_TESTING
    ++downward_count_;
#endif
  }
  ...
  matrix_integer baseline_solution_;
  ...
  std::vector<integer> baseline_product_;
...
#ifdef COPOSIT_IMPROVED_NBC_X4_TESTING
bool improved_nbc_x4_prefers_negative_singular_orientation_for_testing(
...
size_t improved_nbc_x4_combined_ray_sweep_count_for_testing() noexcept {
  return last_combined_ray_sweep_count;
}

size_t improved_nbc_x4_combined_ray_improvement_count_for_testing() noexcept {
  return last_combined_ray_improvement_count;
}

size_t improved_nbc_x4_pair_curvature_exclusion_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t
improved_nbc_x4_support_curvature_exclusion_count_for_testing() noexcept {
...
size_t
improved_nbc_x4_targeted_lp_exact_candidate_count_for_testing() noexcept {
...
bool improved_nbc_x4_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
size_t improved_nbc_x4_shortlist_limit_for_testing(size_t matrix_dimension,
                                                   size_t support_dimension) {
  return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool improved_nbc_x4_prefers_ray_candidate_for_testing(
...
size_t improved_nbc_x4_fixed_support_upper_size_for_testing() noexcept {
  return last_fixed_support_upper_size;
}

bool improved_nbc_x4_traditional_interval_available_for_testing() noexcept {
  return last_traditional_interval_available;
}

size_t improved_nbc_x4_traditional_lower_size_for_testing() noexcept {
  return last_traditional_lower_size;
}

size_t improved_nbc_x4_traditional_upper_size_for_testing() noexcept {
  return last_traditional_upper_size;
}

std::uint64_t improved_nbc_x4_traditional_interval_ns_for_testing() noexcept {
  return last_traditional_interval_ns;
}

std::uint64_t improved_nbc_x4_interval_ns_for_testing() noexcept {
  return last_x4_interval_ns;
}
...
size_t improved_nbc_x4_pair_exclusion_uncovered_count(size_t dimension,
                                                      size_t cardinality,
                                                      size_t first,
                                                      size_t second) {
  support_context context(dimension);
...
size_t improved_nbc_x4_interval_clause_size(size_t dimension,
                                            uint64_t lower_mask,
                                            uint64_t upper_mask) {
  size_t lower_size = 0;
  size_t upper_size = 0;
