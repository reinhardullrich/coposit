...
private:
    ...
    void worker_loop(subset_evaluator& evaluator)
    {
        ...
        for (;;) {
            ...
            while (!cancel_.load(std::memory_order_relaxed)) {
                ...
                try {
                    evaluator.evaluate(*matrix_, (*batch_)[index], results_[index]);
        ...
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            for (;;) {
                ...
                for (size_t index = 0; index < batch.size(); ++index) {
                    ...
                    COPOSIT_MULTITHREADED_CBDD_DIAGNOSTICS("process", subset_dimension);
                    ...
                    diagnostics_.decision_diagram_certificate();
                }
