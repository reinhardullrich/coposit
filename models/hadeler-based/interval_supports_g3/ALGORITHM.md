...
Classification: coposit-created exact CP/SCP experiment. Interval-Supports-G3 alternates between low and high cardinalities. The low frontier uses
...
Interval-Supports-G3 uses that observation as a search certificate.
...
The remaining supports are generated directly from the stored Boolean intervals. Two independent cardinality streams start at $1$
and $n$. The model alternates one open low support and one open high support, exactly as Improved NBC-B7 does. Every new certificate
enters both live streams immediately, so a downward certificate found at the high frontier can remove low candidates before the low
...
No cardinality is materialized as a list. Each frontier stores only a depth-first stack of disjoint unexplored prefixes. Every prefix
fixes an initial sequence of membership bits together with its selected count. The included child is visited first and the excluded
child remains on the stack. Returning one complete support therefore leaves an exact continuation without a blocking rule or a
repeated support.
...
The identifier is `interval_supports_g3`.

- **Interval Supports** names the direct generator whose only retained rules are Boolean intervals.
- **G3** marks the third experiment in the support-generator G line.

The model is an independent copy of [`improved_nbc_b7`](../improved_nbc_b7/ALGORITHM.md). It preserves that model's two-frontier
schedule, floating high-frontier screen, exact nonsingular and singular downward tests, and low-frontier Halfspace-Rays mathematics.
Its only intended change is replacing Improved NBC and its interval compaction with the direct `interval_supports` generator. The
source model remains unchanged for direct comparison. Dickinson intervals come from Peter J. C.
...
Using these curvature facts as Boolean-lattice pruning rules, alternating individual low and high supports, and using the resulting
search as a complete exact CP/SCP classifier are coposit experiments rather than algorithms from those papers. The generator is
project-local code in `cpp/interval_supports.cpp` and has no SAT dependency.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Interval-Supports-G3 solves
...
When low-frontier curvature does not already remove the upward closure, Interval-Supports-G3 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Interval-Supports-G3 performs the inherited exact
...
The interval could equivalently be written as the Boolean clause
...
The implementation stores $L(u)$ and $U(u)$ directly rather than encoding this clause.

## Stored Interval Rules

### Upward closure

If the reduced Hessian on $I$ is not positive definite, the generator stores
...
If $B=A_I$ is positive definite, every principal submatrix indexed by a nonempty subset of $I$ is positive definite. The generator
therefore stores
...
This removes $I$ and every nonempty subset of $I$. For $I=[n]$ the stored interval is the complete Boolean lattice, so no support
remains and the proof is complete.

Interval-Supports-G3 retains SAT-B3's singular case. Suppose
...
positive-semidefinite matrix. The high frontier's prefix cursor advances past that support without storing a rejection interval. The
independent exact low frontier can still reach the same support later.

## Direct Interval Traversal

For $[L,U]$, the constrained coordinates are

$$
C=L\cup([n]\setminus U).
$$

If $C$ is nonempty, let $t=\max C$. Before coordinate $t$ is assigned, the interval cannot cover a complete search prefix because a
later constrained coordinate remains undecided. Immediately after $t$ is assigned, either the prefix contains $L$ and is contained
in $U$, in which case every completion of that prefix lies in $[L,U]$, or this interval can never cover a descendant of that prefix.
The generator therefore buckets each interval by $t$ and tests it only at that depth. Along one root-to-leaf path, a retained static
interval is tested at most once.

Each live prefix also remembers how many intervals existed when it was created. If a new interval is added after that prefix has
already passed the interval's trigger, the prefix checks only those newly added rules when it resumes. If the trigger is still ahead,
the ordinary trigger bucket handles it later. This gives immediate cross-frontier pruning without rebuilding either stream.

Intervals are retained with stable append-only identifiers. The generator deliberately performs no eager containment compaction:
doing a global containment scan and rebuilding the rule array after every certificate would make insertion quadratic. A single
$[\varnothing,[n]]$ interval is recognized immediately as complete coverage.

## Complete Decision Flow

1. Create the two direct interval streams and install every failed pair-curvature certificate.
...
4. If the frontiers have not met, ask the high frontier for one open support. Run the floating $LDL^T$ filter first. A floating
   ...
   $Bx=\mathbf1$.
5. Alternate steps 3 and 4. When either frontier exhausts a cardinality, advance that frontier without rebuilding the generator. A
   floatingly rejected high support remains available to the exact low frontier.
...
Runtime diagnostics report the current low or high cardinality, selected and processed supports, retained interval exclusions, and the
...
Interval-Supports-G3 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
...
accumulated. When certificates are weak, direct depth-first traversal must still visit many genuinely open prefixes. Redundant or
contained intervals remain stored, so a large redundant certificate family can increase trigger-bucket work and memory use. This is
the intended engine difference from Improved NBC-B7 and the main object of the comparison.
