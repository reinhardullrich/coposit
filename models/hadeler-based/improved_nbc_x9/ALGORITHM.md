...
Classification: coposit-created exact CP/SCP experiment. Improved NBC-X9 is X6 plus one optional spectral-guided certificate search.
After X6 has chosen its exact low-frontier interval, X9 uses the negative eigenvectors of the full matrix to propose positive
right-hand sides near that exact vector. It reconstructs every retained proposal exactly, keeps the best proposal, applies the exact
Halfspace-Rays cleanup to it, and replaces the X6 interval only when the final exact interval has larger width $D=|U|-|L|$, breaking
a width tie by larger $|U|$. The high frontier and every other part of X6 are unchanged.
...
Improved NBC-X9 uses that observation as a search certificate.
...
- If a low-frontier face is strictly convex, the retained factorization is reused to build and optimize a Dickinson interval. This
  replaces the former exact-support block. X6 next asks a small LP whether a different positive right-hand side could retain the
  current upper endpoint and add one chosen missing index. A proposed vector is reconstructed exactly. Every success becomes the
  starting point of another LP round. Only after no further exact enlargement is found does X6 shrink the lower endpoint. The final
  chain interval and the separately shrunken ordinary X3 interval are compared by larger $D$, then larger $|U|$.
- X9 then treats that winning X6 vector as the centre of a floating-point search along the full matrix's negative eigenvectors,
  pairwise sums and differences of those directions, and a fixed deterministic sample of their span. At most twelve promising
  positive right-hand sides survive. Exact arithmetic constructs and scores every survivor. The best survivor receives one complete
  exact Halfspace-Rays cleanup. If that result does not strictly beat X6, X9 restores the unchanged X6 certificate.
...
The identifier is `improved_nbc_x9`. It is an experimental spectral-guided extension of `improved_nbc_x6`.

- **Improved NBC** names the separately owned resumable derivative of NBC MiniSat All.
- **X9** is the next isolated X-series experiment and names the spectral-guided Halfspace cleanup added after X6.

The model is an independent copy of [`improved_nbc_x6`](../improved_nbc_x6/ALGORITHM.md). It preserves X6's two-frontier traversal,
floating high-frontier screen, exact nonsingular and singular downward tests, low-frontier Halfspace-Rays machinery, repeated targeted
LP, exact lower shrinking, Boolean clauses, and interval compaction. Its only mathematical change is the optional spectral-guided
right-hand-side search and exact Halfspace cleanup described here. `improved_nbc_x6` remains unchanged as X9's direct comparison
baseline.
The dense simplex code is a private reduced copy of the implementation in
[`sat_halfspace_milp_dickinson`](../sat_halfspace_milp_dickinson/ALGORITHM.md); X6 uses no MILP variables or branch-and-bound.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Improved NBC-X9 solves
...
When low-frontier curvature does not already remove the upward closure, Improved NBC-X9 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Improved NBC-X9 performs the inherited exact
breakpoint sweeps along those directions, preferring larger width $D=|U|-|L|$ and then larger $|U|$. It retains a bounded shortlist of
coordinate rays under the same ordering, using gains and losses only to break an exact interval-score tie, and tests at most two
complementary combined rays after a coordinate-wise stall. Every accepted candidate is represented with exact integers; no
floating-point comparison enters this inherited Rays stage or its certificate.
...
For one outside target $j\notin U_R$, X6 numerically solves
...
with $t\ge0$ are useful, $t$ is one nonnegative variable rather than the difference of two nonnegative variables. X6 also eliminates
...
region that X6 can accept, this is equivalent to the unreduced formulation, but with two fewer variables and one fewer constraint.
Targets are tried in increasing index order. The first exactly verified improvement stops that scan, becomes the current vector,
and starts a new scan that preserves its enlarged upper endpoint. The chain stops when the upper endpoint is full or a complete scan
finds no exact improvement. Every successful round adds at least one upper index, so at most $n-|U_R|$ rounds can succeed. The LP
does not maximize $|U|$ and does not claim optimality over all right-hand sides. Other missing indices may nevertheless become
nonnegative at the same point. After the ray phase has finalized its exact direction basis, the outside-row coefficient table is
fixed. X6 therefore constructs its binary64 representation once and reuses it across all continuation rounds for that support.

The LP is deliberately non-authoritative. If its floating solution has a nonnegative numerical margin, X6 normalizes it by its
largest coordinate, multiplies every coordinate by $10^9$, and rounds to a positive integer. It then recomputes $u=B^{-1}b$ and
$Au$ with exact integers. A reconstruction is retained only if every old upper index and the selected target are exactly
nonnegative. Numerical infeasibility or failed reconstruction merely forgoes that optional round; the complete exact X3 branch is
unchanged. If the dense tableau would exceed 8,388,608 cells, X6 also skips this optional stage and uses the ordinary X3 certificate.
...
X6 first completes the entire LP enlargement chain and then applies that same shrink independently to the original Rays vector and
the final exactly verified LP vector. Let their final intervals be
$[L_X,U_X]$ and $[L_P,U_P]$, with widths $D_X=|U_X|-|L_X|$ and $D_P=|U_P|-|L_P|$. X6 keeps the LP branch exactly when

$$
D_P>D_X,
\qquad\text{or}\qquad
D_P=D_X\ \text{ and }\ |U_P|>|U_X|.
$$

The same ordering applies to every competing interval candidate, regardless of whether it came from an all-ones solve, a coordinate
ray, a combined ray, or the targeted LP. X6 inserts only the winning interval; an exact tie retains the earlier candidate.
...
Improved NBC-X9 retains SAT-B3's singular case. Suppose
...
3. Ask the low frontier for one open support. Copy and exactly factor its principal matrix. Install an upward closure immediately
   ...
   a nonsingular certificate with a missing outside upper index, run the targeted LP and exactly reconstruct its first accepted
   proposal. Continue from every exact enlargement until the upper endpoint is full or one complete scan finds none. Then apply
   lower shrinking once to the final LP vector and independently to the original Rays vector. Insert whichever interval has larger
   $D=|U|-|L|$, breaking a width tie by larger $|U|$.
   Before insertion, X9 also proposes at most twelve positive right-hand sides from the full matrix's floating negative-eigenspace,
   reconstructs and scores each proposal exactly with the retained factorization, runs one exact Halfspace-Rays cleanup from the best
   proposal, and inserts the hybrid interval only if it strictly beats the X6 interval under the same score.
...
All proof-producing matrix entries, factorization state, right-hand-side directions, breakpoint comparisons, reconstructed LP and
spectral coefficients, products, interval-score comparisons, inertia signs, kernel vectors, and witnesses use arbitrary-precision
integers. Binary64 is used only for the high-frontier filter, the targeted LP proposal, and the full-matrix eigendecomposition that
proposes spectral right-hand sides. A floating proposal or rejection is not stored and cannot affect the exact X6 branch or final
classification.
...
Improved NBC-X9 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
...
Each targeted LP round tries one missing upper index at a time and accepts the first exact improvement. The resulting greedy chain
can miss a larger interval that requires losing an old upper index temporarily or choosing a different target order. Repeated dense
LP work can outweigh its coverage gain when Halfspace-Rays already found a strong certificate. Near-boundary numerical proposals
often fail exact reconstruction; in that case X6 stops the chain and compares the final verified LP interval with the ordinary X3
certificate by larger $D$, then larger $|U|$.
...
The spectral search is deliberately bounded, so it can miss a useful direction. Conversely, matrices with many negative eigenvalues
produce many floating direction samples and may pay substantial proposal and exact-verification cost without improving the X6
certificate. This experiment exists to measure whether the extra interval width repays that cost in complete traversals.
