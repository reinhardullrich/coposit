...
namespace coposit::improved_nbc_x9_diagnostics {
...
inline void record_interval(std::string_view stage, size_t detail,
                            std::vector<size_t> lower,
                            std::vector<size_t> upper) {
  interval_events.push_back(
      {stage, detail, std::move(lower), std::move(upper)});
}

} // namespace coposit::improved_nbc_x9_diagnostics

#define COPOSIT_IMPROVED_NBC_X9_DIAGNOSTICS(...)                               \
  ::coposit::improved_nbc_x9_diagnostics::record(__VA_ARGS__)

#else

#define COPOSIT_IMPROVED_NBC_X9_DIAGNOSTICS(...) ((void)0)
