...
std::uint64_t last_x6_interval_ns = 0;
#endif
...
bool better_shortlist_candidate(const ray_candidate &candidate,
  ...
  if (better_score(candidate.score, current.score))
    return true;
  if (better_score(current.score, candidate.score))
    return false;
...
public:
  ...
  bool optimize_support_for_testing(const matrix_integer &matrix,
    ...
    last_x6_interval_ns = 0;
    captured_vector_available_ = false;
    ...
    last_x6_interval_ns = static_cast<std::uint64_t>(
...
private:
  ...
  void record_test_interval(std::string_view stage, size_t detail = 0) const {
    ...
    lower.reserve(indices_.size());
    upper.reserve(product_.size());
    for (size_t local = 0; local < indices_.size(); ++local) {
      if (!solution_(local, 0).is_zero())
        lower.push_back(indices_[local] + 1);
    }
  ...
  bool process_nonsingular_subset(const matrix_integer &matrix) {
    ...
    const bool selected_lp = better_score(targeted_score, rays_score);
    if (selected_lp) {
  ...
  bool add_certificate() {
    ...
    if (captured_lower_ != nullptr) {
      ...
      captured_vector_available_ = true;
      support_context_.release(std::move(lower));
      support_context_.release(std::move(upper));
      return true;
    }
  ...
  bool captured_vector_available_ = false;
#endif
};
...
bool improved_nbc_x9_certificate_and_vector_for_testing(
    ...
    std::vector<integer> &product) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
      .optimize_support_vector_for_testing(matrix, indices, lower, upper,
                                           solution, product);
}
