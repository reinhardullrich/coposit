# Breadth-First Singular-Lift Dickinson

Classification: coposit-created exact CP/SCP experiment. It copies `layered_singular_lift_dickinson` and changes its singular-support
traversal from recursive depth first to FIFO breadth first. All mathematical tests and retained certificates remain the same.
...
changes the exact nullspace constraints. A FIFO queue processes every reachable singular child of one lifted cardinality before any
child of the next cardinality. At nullity one there is a unique projective direction, so the model checks both signs of that direction.
If either sign reaches the ceiling, it stores the support of that final vector.
...
The identifier is `breadth_first_singular_lift_dickinson`.

- “Breadth first” means that, for one outer root, every lifted support of cardinality $t$ is processed before cardinality $t+1$.
...
The complete starting implementation is the local
`models/hadeler-based/layered_singular_lift_dickinson/solver.cpp`. Its cardinality-ordered forbidden-support generator comes from
...
This model is not described by Dickinson's paper and is not a literature baseline. Singular-superset lifting, the first-nullity-one
stopping rule, the outer-layer barrier, the call-wide lifted-support cache, pending-antichain minimization, and the breadth-first queue
are coposit changes. Relative to `layered_singular_lift_dickinson`, only the traversal order and its queue storage are new.
...
greater than one, the breadth-first singular lift begins from $I$.

## Breadth-First Singular Lifting
...
Initialize a FIFO queue with the root $I$. Remove the oldest queued support $T$, form its children in increasing $j$ order, and factor
each child exactly.

- If $A_{T'}$ is nonsingular, that branch ends.
- If $A_{T'}$ is singular and has nullity greater than one, append $T'$ to the back of the queue.
- If $A_{T'}$ has nullity one, recover its unique exact kernel ray, test it, and end that branch.

Every edge raises cardinality by exactly one. Consequently, FIFO order processes all queued supports with lifted $k=t$ before any
support with lifted $k=t+1$. This guarantee is per outer root: the complete lift below one root finishes before ordinary Dickinson
moves to the next root of the same root cardinality.
...
the route used to reach it. A duplicate already waiting in the queue is safe to skip because that queued copy will later expand the
same child family. A duplicate from an earlier root is safe to skip because that earlier root's queue was completely drained before
returning. Outer roots remain eligible because the cache suppresses only repeated lifting work, not ordinary outer
Dickinson processing.

The breadth-first search deliberately does not restrict a state's children to indices larger than the last added index. Such a canonical path rule
...
best possible enlargement of the outer root; the queue visits every singular enlargement reachable before a nullity-one stop.
...
- `lift_frontier` and `lift_maximum_frontier`: the current and largest FIFO queue sizes;
...
The lifted support graph can be enormous. If many principal supersets remain singular with nullity greater than one and few
nullity-one directions reach the ceiling, the model pays for a large additional family of exact factorizations without gaining
pruning. The call-wide visited set prevents duplicate work but can itself approach the size of a large part of the Boolean lattice.
...
a high-dimensional nullspace rather than adding principal constraints. The queue does not optimize arbitrary basis combinations, so
it may explore many lifts before a unique direction appears.

Breadth-first order delays deep exploration, but it must retain the entire unresolved frontier. A broad singular layer can therefore
use much more memory than the recursive depth-first variant. Breadth first can also be slower when one early depth-first path contains
a useful certificate: it insists on finishing every shallower lifted support before reaching that path's next level.
...
number of distinct lifted supports and the breadth-first frontier can still dominate runtime and memory.
