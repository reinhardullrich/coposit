...
Classification: coposit-created exact CP/SCP experiment. Improved NBC-X6 alternates between low and high cardinalities. The low frontier uses
...
optimized certificate is repeatedly offered to a targeted continuous LP that tries to add one missing upper index without losing an
existing one. Every numerical LP point is only a proposal. X6 reconstructs and checks each proposal with exact integers, continues
from every accepted upper-endpoint enlargement, and shrinks the lower endpoint only after that chain stops. It keeps the LP branch
when its final exact interval has larger width $D=|U|-|L|$, breaking a width tie by larger $|U|$. The high
...
Improved NBC-X6 uses that observation as a search certificate.
...
The identifier is `improved_nbc_x6`. It is the current project incumbent, derived from `improved_nbc_x3`.

- **Improved NBC** names the separately owned resumable derivative of NBC MiniSat All.
- **X6** names X3 with its targeted LP stage repeated before lower-endpoint shrinking.

The model is an independent copy of [`improved_nbc_x3`](../improved_nbc_x3/ALGORITHM.md). It preserves X3's two-frontier traversal,
floating high-frontier screen, exact nonsingular and singular downward tests, low-frontier Halfspace-Rays machinery, targeted LP,
exact lower shrinking, Boolean clauses, and interval compaction. Its only change is to repeat every successful targeted LP
enlargement before shrinking. `improved_nbc_x3` remains unchanged as X6's direct comparison baseline.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Improved NBC-X6 solves
...
When low-frontier curvature does not already remove the upward closure, Improved NBC-X6 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Improved NBC-X6 performs the inherited exact
...
Improved NBC-X6 retains SAT-B3's singular case. Suppose
...
coefficients, products, interval-score comparisons, inertia signs, kernel vectors, and witnesses use arbitrary-precision integers.
Binary64 is used only for the high-frontier filter and the targeted LP proposal. A floating rejection is not stored and cannot affect
the exact X3 branch or final classification.
...
Improved NBC-X6 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
