...
#ifdef COPOSIT_IMPROVED_NBC_X3_TESTING
...
std::uint64_t last_x3_interval_ns = 0;
#endif
...
public:
  ...
  bool optimize_support_for_testing(const matrix_integer &matrix,
    ...
    last_x3_interval_ns = static_cast<std::uint64_t>(
...
private:
  ...
  void record_interval_certificate(const support &lower, const support &upper) {
    ...
    event << "model=improved_nbc_x3 n=" << product_.size()
  ...
  bool process_low_support(const matrix_integer &matrix, size_t &low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X3_DIAGNOSTICS("stage_low", low);
  ...
  bool process_high_support(const matrix_integer &matrix, size_t low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X3_DIAGNOSTICS("stage_high", high);
  ...
  bool process_selected_support(const matrix_integer &matrix,
    ...
    COPOSIT_IMPROVED_NBC_X3_DIAGNOSTICS(
  ...
  void install_pair_curvature_exclusions(const matrix_integer &matrix) {
    ...
    for (size_t first = 0; first < matrix.rows(); ++first) {
      ...
      for (size_t second = first + 1; second < matrix.rows(); ++second) {
        ...
        COPOSIT_IMPROVED_NBC_X3_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_IMPROVED_NBC_X3_TESTING
  ...
  bool process_subset(const matrix_integer &matrix,
    ...
    if (!from_low_frontier) {
      ...
      if (!floating_filter_.looks_positive_semidefinite(indices_)) {
        ...
        COPOSIT_IMPROVED_NBC_X3_DIAGNOSTICS("high_float_reject",
                                            indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X3_TESTING
...
#endif
      COPOSIT_IMPROVED_NBC_X3_DIAGNOSTICS("combined_ray", ray + 1);
  ...
  bool add_certificate() {
    ...
    support_context_.release(std::move(lower));
    support_context_.release(std::move(upper));
    COPOSIT_IMPROVED_NBC_X3_DIAGNOSTICS("dickinson", indices_.size());
    return true;
  }
  ...
  void add_upward_closure() {
    ...
    COPOSIT_IMPROVED_NBC_X3_DIAGNOSTICS("support_upward", indices_.size());
  }
  ...
  void add_downward_closure(std::string_view kind) {
    ...
    record_closure_certificate(kind, "downward", certificate_frontier_,
                               indices_);
    COPOSIT_IMPROVED_NBC_X3_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X3_TESTING
    ++downward_count_;
#endif
  }
...
#ifdef COPOSIT_IMPROVED_NBC_X3_TESTING
bool improved_nbc_x3_prefers_negative_singular_orientation_for_testing(
    size_t positive_products, size_t negative_products) noexcept {
  return negative_orientation_has_larger_upper(positive_products,
                                               negative_products);
}

size_t improved_nbc_x3_optimized_certificate_count_for_testing() noexcept {
  return last_optimized_certificate_count;
}

size_t improved_nbc_x3_combined_ray_sweep_count_for_testing() noexcept {
  return last_combined_ray_sweep_count;
}

size_t improved_nbc_x3_combined_ray_improvement_count_for_testing() noexcept {
  return last_combined_ray_improvement_count;
}

size_t improved_nbc_x3_pair_curvature_exclusion_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t
improved_nbc_x3_support_curvature_exclusion_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t improved_nbc_x3_pair_upward_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t improved_nbc_x3_support_upward_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t improved_nbc_x3_downward_count_for_testing() noexcept {
  return last_downward_count;
}

size_t improved_nbc_x3_high_float_rejection_count_for_testing() noexcept {
  return last_high_float_rejection_count;
}

size_t improved_nbc_x3_targeted_lp_attempt_count_for_testing() noexcept {
  return last_targeted_lp_attempt_count;
}

size_t improved_nbc_x3_targeted_lp_feasible_count_for_testing() noexcept {
  return last_targeted_lp_feasible_count;
}

size_t
improved_nbc_x3_targeted_lp_exact_candidate_count_for_testing() noexcept {
  return last_targeted_lp_exact_candidate_count;
}

size_t improved_nbc_x3_targeted_lp_accepted_count_for_testing() noexcept {
  return last_targeted_lp_accepted_count;
}

bool improved_nbc_x3_floating_psd_candidate_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool improved_nbc_x3_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
size_t improved_nbc_x3_shortlist_limit_for_testing(size_t matrix_dimension,
                                                   size_t support_dimension) {
  return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool improved_nbc_x3_prefers_ray_candidate_for_testing(
...
bool improved_nbc_x3_check_support_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool improved_nbc_x3_certificate_for_testing(const matrix_integer &matrix,
                                             const std::vector<size_t> &indices,
                                             support &lower, support &upper) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
      .optimize_support_for_testing(matrix, indices, lower, upper);
...
size_t improved_nbc_x3_fixed_support_upper_size_for_testing() noexcept {
  return last_fixed_support_upper_size;
}

bool improved_nbc_x3_traditional_interval_available_for_testing() noexcept {
  return last_traditional_interval_available;
}

size_t improved_nbc_x3_traditional_lower_size_for_testing() noexcept {
  return last_traditional_lower_size;
}

size_t improved_nbc_x3_traditional_upper_size_for_testing() noexcept {
  return last_traditional_upper_size;
}

std::uint64_t improved_nbc_x3_traditional_interval_ns_for_testing() noexcept {
  return last_traditional_interval_ns;
}

std::uint64_t improved_nbc_x3_interval_ns_for_testing() noexcept {
  return last_x3_interval_ns;
}
...
size_t improved_nbc_x3_uncovered_count(
...
size_t improved_nbc_x3_pair_exclusion_uncovered_count(size_t dimension,
                                                      size_t cardinality,
                                                      size_t first,
                                                      size_t second) {
  support_context context(dimension);
...
size_t improved_nbc_x3_interval_clause_size(size_t dimension,
                                            uint64_t lower_mask,
                                            uint64_t upper_mask) {
  size_t lower_size = 0;
  size_t upper_size = 0;
