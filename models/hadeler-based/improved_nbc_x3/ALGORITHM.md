...
Classification: coposit-created exact CP/SCP experiment. Improved NBC-X3 alternates between low and high cardinalities. The low frontier uses
...
one. A numerical LP point is only a proposal. X3 reconstructs and checks it with exact integers, applies X2 lower-endpoint shrinking
...
Improved NBC-X3 uses that observation as a search certificate.
...
- If a low-frontier face is strictly convex, the retained factorization is reused to build and optimize a Dickinson interval. This
  replaces the former exact-support block. X3 next asks a small LP whether a different positive right-hand side could retain the
...
The identifier is `improved_nbc_x3`. It is the direct predecessor of the current `improved_nbc_x6` incumbent.

- **Improved NBC** names the separately owned resumable derivative of NBC MiniSat All.
- **X3** names X2 plus a third certificate stage: a targeted LP proposal between Halfspace-Rays and lower-endpoint shrinking.

The model is an independent copy of [`improved_nbc_x2`](../improved_nbc_x2/ALGORITHM.md). It preserves X2's two-frontier traversal,
floating high-frontier screen, exact nonsingular and singular downward tests, low-frontier Halfspace-Rays machinery, exact lower
shrinking, Boolean clauses, and interval compaction. Its only change is the targeted LP proposal and the exact comparison of the two
final intervals. `improved_nbc_x2` remains unchanged as the direct comparison model from which X3 was derived.
The dense simplex code is a private reduced copy of the implementation in
[`sat_halfspace_milp_dickinson`](../sat_halfspace_milp_dickinson/ALGORITHM.md); X3 uses no MILP variables or branch-and-bound.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Improved NBC-X3 solves
...
When low-frontier curvature does not already remove the upward closure, Improved NBC-X3 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Improved NBC-X3 performs the inherited exact
...
For one outside target $j\notin U_R$, X3 numerically solves
...
with $t\ge0$ are useful, $t$ is one nonnegative variable rather than the difference of two nonnegative variables. X3 also eliminates
...
region that X3 can accept, this is equivalent to the unreduced formulation, but with two fewer variables and one fewer constraint.
...
The LP is deliberately non-authoritative. If its floating solution has a nonnegative numerical margin, X3 rounds the relative
...
this optional enlargement; the complete exact X2 branch is unchanged. If the dense tableau would exceed 8,388,608 cells, X3 also
...
X3 applies that same shrink independently to the Rays vector and to an exactly verified LP vector. Let their final intervals be
...
carry a worse lower endpoint after shrinking. X3 inserts only one interval, never two nearly overlapping alternatives.
...
Improved NBC-X3 retains SAT-B3's singular case. Suppose
...
the exact X2 branch or final classification.
...
Improved NBC-X3 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
...
that case X3 pays the LP cost and deliberately inserts the unchanged X2 certificate.
