    ...
    size_t dimension, size_t cardinality, const std::vector<std::pair<uint64_t, uint64_t>>& intervals)
{
    ...
    for (uint64_t candidate = 1; candidate < (uint64_t{1} << dimension); ++candidate) {
        ...
        for (const auto& [lower, upper] : intervals)
            covered |= (lower & ~candidate) == 0 && (candidate & ~upper) == 0;
        result += !covered;
    }
    return result;
}

TEST(BddB3Test, AlternatesIndividualLowAndHighSupportsWithDirectionalPruning)
{
    bdd_b3_diagnostics::clear();
    ...
    EXPECT_EQ(model::bdd_b3_pair_upward_count_for_testing(), 0U);
    EXPECT_EQ(model::bdd_b3_support_upward_count_for_testing(), 0U);
    EXPECT_EQ(model::bdd_b3_downward_count_for_testing(), 1U);
    EXPECT_EQ(model::bdd_b3_exact_count_for_testing(), 0U);
    EXPECT_NE(std::find(bdd_b3_diagnostics::events.begin(), bdd_b3_diagnostics::events.end(),
                       bdd_b3_diagnostics::event{"dickinson", 1}),
              bdd_b3_diagnostics::events.end());
}

TEST(BddB3Test, UsesTheFloatingFilterBeforeExactHighFrontierWork)
{
    bdd_b3_diagnostics::clear();
    (void)model::classify(symmetric_matrix(3, {1, -4, -2, 1, 0, 1}));

    const auto high = std::find(bdd_b3_diagnostics::events.begin(), bdd_b3_diagnostics::events.end(),
                                bdd_b3_diagnostics::event{"process_high", 3});
    ASSERT_NE(high, bdd_b3_diagnostics::events.end());
    ASSERT_NE(std::next(high), bdd_b3_diagnostics::events.end());
    EXPECT_EQ(*std::next(high), (bdd_b3_diagnostics::event{"high_float_reject", 3}));
    EXPECT_GT(model::bdd_b3_high_float_rejection_count_for_testing(), 0U);
}

TEST(BddB3Test, FloatingFilterOnlyProposesExactPositiveSemidefinitenessChecks)
{
    EXPECT_TRUE(model::bdd_b3_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {2, 0, 0, 3, 0, 4}), {0, 1, 2}));
    EXPECT_TRUE(model::bdd_b3_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {5, -1, 2, 5, 2, 2}), {0, 1, 2}));
    EXPECT_FALSE(model::bdd_b3_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {1, 0, 0, -1, 0, 1}), {0, 1, 2}));
    EXPECT_FALSE(model::bdd_b3_floating_psd_candidate_for_testing(
        symmetric_matrix(2, {0, 1, 0}), {0, 1}));
}

TEST(BddB3Test, FloatingFilterUsesTheSelectedSubmatrixScale)
{
    const matrix_integer matrix = symmetric_matrix(3, {1'000'000'000'000'000'000L, 0, 0, 1, 0, 1});
    EXPECT_TRUE(model::bdd_b3_floating_psd_candidate_for_testing(matrix, {1, 2}));
}

TEST(BddB3Test, UsesConsistentAllOnesSystemToPruneSingularPsdSupportDownward)
{
    bdd_b3_diagnostics::clear();
    ...
    EXPECT_EQ(model::bdd_b3_downward_count_for_testing(), 1U);
    EXPECT_NE(std::find(bdd_b3_diagnostics::events.begin(), bdd_b3_diagnostics::events.end(),
                       bdd_b3_diagnostics::event{"downward", 4}),
              bdd_b3_diagnostics::events.end());
}

TEST(BddB3Test, FloatingRejectionsRemainVisibleToTheExactLowFrontier)
{
    const std::vector<std::vector<size_t>> rejected{{0, 1}};
    EXPECT_EQ(model::bdd_b3_high_filter_uncovered_count(3, 2, rejected, true), 2U);
    EXPECT_EQ(model::bdd_b3_high_filter_uncovered_count(3, 2, rejected, false), 3U);
}

TEST(BddB3Test, BuildsAnExactDickinsonIntervalOnTheLowPath)
{
    ...
    EXPECT_TRUE(model::bdd_b3_certificate_for_testing(identity, {0}, lower, upper));
...
TEST(BddB3Test, RetainsTheExactHalfspaceRayOptimization)
{
    ...
    for (uint64_t mask = 1; mask < 16 && !improved; ++mask) {
        ...
        (void)model::bdd_b3_check_support_for_testing(matrix, indices);
        improved = model::bdd_b3_optimized_certificate_count_for_testing() > 0;
    }
    EXPECT_TRUE(improved);
}

TEST(BddB3Test, StoresDickinsonIntervalsInTheSharedBddState)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011}, {0b100, 0b110}};
    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 1, intervals), 1U);
    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 2, intervals), 1U);
}

TEST(BddB3Test, ExpiresBoundedIntervalsWithoutDamagingOverlappingLiveCoverage)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b0001, 0b0011}, {0b0001, 0b1111}};
    EXPECT_EQ(model::bdd_b3_uncovered_count(4, 3, intervals), brute_uncovered_count(4, 3, intervals));
}

TEST(BddB3Test, MatchesBruteForceForEveryPairOfFourDimensionalIntervals)
{
    ...
    for (const auto& first : valid_intervals) {
        for (const auto& second : valid_intervals) {
            ...
            for (size_t cardinality = 1; cardinality <= dimension; ++cardinality)
                EXPECT_EQ(model::bdd_b3_uncovered_count(dimension, cardinality, intervals),
                          brute_uncovered_count(dimension, cardinality, intervals));
        }
    }
}

TEST(BddB3Test, EncodesUpwardDownwardAndExactSupportBlocks)
{
    const std::vector<std::vector<size_t>> none;
    const std::vector<std::vector<size_t>> support_01{{0, 1}};

    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 1, support_01, none, none), 3U);
    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 2, support_01, none, none), 2U);
    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 3, support_01, none, none), 0U);

    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 1, none, support_01, none), 1U);
    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 2, none, support_01, none), 2U);
    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 3, none, support_01, none), 1U);

    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 1, none, none, support_01), 3U);
    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 2, none, none, support_01), 2U);
    EXPECT_EQ(model::bdd_b3_uncovered_count(3, 3, none, none, support_01), 1U);
}

TEST(BddB3Test, PreservesExactCombinedClassifications)
{
...
TEST(BddB3Test, ExhaustivelyClassifiesSmallTwoByTwoIntegerMatrices)
{
...
TEST(BddB3Test, MatchesTheIndependentExactThreeByThreeCriterion)
{
...
TEST(BddB3Test, AppliesPairCurvaturePrepass)
{
    ...
    EXPECT_EQ(model::bdd_b3_pair_upward_count_for_testing(), 1U);
}
