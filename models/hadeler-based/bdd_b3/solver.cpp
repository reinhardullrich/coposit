...
class interval_bdd {
public:
    explicit interval_bdd(support_context& context)
        : context_(context)
        , dimension_(context.dimension())
        , expiring_(dimension_ + 1, empty)
        , cardinality_roots_(dimension_ + 1, missing)
        , current_support_(context.make())
    {
        nodes_.push_back({dimension_, empty, empty});
        nodes_.push_back({dimension_, unit, unit});
    }
    ...
    bool take_first(std::vector<size_t>& indices, bool high_frontier = false)
    {
        ...
        size_t root = empty;
        if (high_frontier) {
            if (high_cardinality_ != cardinality_) {
                high_cardinality_ = cardinality_;
                high_remaining_ = subtract(cardinality_family(cardinality_), covered_);
            }
            root = high_remaining_;
        } else {
            if (low_cardinality_ != cardinality_) {
                expire_before(cardinality_);
                low_cardinality_ = cardinality_;
                low_remaining_ = subtract(cardinality_family(cardinality_), covered_);
            }
            root = low_remaining_;
        }
        if (root == empty) return false;

        context_.clear(current_support_);
        while (root != unit) {
            assert(root != empty);
            const node value = nodes_[root];
            if (value.low != empty) root = value.low;
            else {
                context_.set(current_support_, actual_index(value.variable));
                root = value.high;
            }
        }
        context_.extract_set_indices(current_support_, indices);
    ...
    {
        size_t upper_size = 0;
        for (size_t index = 0; index < dimension_; ++index) upper_size += context_.contains(upper, index);
        assert(upper_size >= expiration_cursor_);
        const size_t certificate = interval_family(lower, upper);
        covered_ = unite(covered_, certificate);
        if (upper_size < dimension_) expiring_[upper_size] = unite(expiring_[upper_size], certificate);
        if (low_cardinality_ != missing) low_remaining_ = subtract(low_remaining_, certificate);
        if (high_cardinality_ != missing) high_remaining_ = subtract(high_remaining_, certificate);
        ++interval_count_;
    }
    ...
    void add_pair_upward_closure(size_t first, size_t second)
    {
        ...
        context_.set_all(upper);
        add_interval(lower, upper);
    ...
    void add_exact_support(const std::vector<size_t>& indices)
    {
        ...
        add_interval(exact, exact);
        context_.release(std::move(exact));
    }

    void reject_from_high_frontier(const std::vector<size_t>& indices)
    {
        if (high_cardinality_ != indices.size()) {
            high_cardinality_ = indices.size();
            high_remaining_ = subtract(cardinality_family(indices.size()), covered_);
        }
        support exact = context_.make();
        for (const size_t index : indices) context_.set(exact, index);
        const size_t rejected = interval_family(exact, exact);
        context_.release(std::move(exact));
        high_remaining_ = subtract(high_remaining_, rejected);
    }
...
private:
    ...
    struct node { size_t variable; size_t low; size_t high; };
    struct node_key {
        size_t variable;
        size_t low;
        size_t high;
    ...
    struct pair_key {
        size_t left;
        size_t right;
        bool operator==(const pair_key& other) const noexcept { return left == other.left && right == other.right; }
    };
    ...
    size_t actual_index(size_t variable) const noexcept { return dimension_ - 1 - variable; }
    size_t variable(size_t root) const noexcept { return root < 2 ? dimension_ : nodes_[root].variable; }
    ...
    size_t interval_family(const support& lower, const support& upper)
    {
        ...
        for (size_t variable_value = dimension_; variable_value-- > 0;) {
            ...
            if (context_.contains(lower, bit)) root = make_node(variable_value, empty, root);
            else if (!context_.contains(upper, bit)) root = make_node(variable_value, root, empty);
        }
        return root;
    }
    ...
    {
        if (cardinality_roots_[cardinality] != missing) return cardinality_roots_[cardinality];
        ...
        cardinality_roots_[cardinality] = build(0, cardinality);
        return cardinality_roots_[cardinality];
    }
    ...
    std::vector<size_t> cardinality_roots_;
    size_t covered_ = empty;
    size_t low_remaining_ = empty;
    size_t high_remaining_ = empty;
    size_t low_cardinality_ = missing;
    size_t high_cardinality_ = missing;
...
private:
    ...
    bool process_low_support(const matrix_integer& matrix, size_t& low, size_t high)
    {
        while (low <= high) {
            ...
            COPOSIT_BDD_B3_DIAGNOSTICS("stage_low", low);
    ...
    bool process_high_support(const matrix_integer& matrix, size_t low, size_t& high)
    {
        while (low <= high) {
            ...
            COPOSIT_BDD_B3_DIAGNOSTICS("stage_high", high);
    ...
    bool process_selected_support(const matrix_integer& matrix, pruning_direction direction, bool from_low_frontier)
    {
        ...
        COPOSIT_BDD_B3_DIAGNOSTICS(from_low_frontier ? "process_low" : "process_high", indices_.size());
    ...
    void install_pair_curvature_exclusions(const matrix_integer& matrix)
    {
        ...
        for (size_t first = 0; first < matrix.rows(); ++first) {
            ...
            for (size_t second = first + 1; second < matrix.rows(); ++second) {
                ...
                COPOSIT_BDD_B3_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_BDD_B3_TESTING
        ...
        const matrix_integer& matrix, pruning_direction direction = pruning_direction::upward, bool from_low_frontier = true)
    {
        if (!from_low_frontier) {
            ...
            if (!floating_filter_.looks_positive_semidefinite(indices_)) {
                ...
                COPOSIT_BDD_B3_DIAGNOSTICS("high_float_reject", indices_.size());
#ifdef COPOSIT_BDD_B3_TESTING
...
#endif
            COPOSIT_BDD_B3_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    bool add_certificate()
    {
        ...
        COPOSIT_BDD_B3_DIAGNOSTICS("dickinson", indices_.size());
        return true;
    }
    ...
    void add_upward_closure()
    {
        ...
        COPOSIT_BDD_B3_DIAGNOSTICS("support_upward", indices_.size());
    }
    ...
    void add_downward_closure()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_BDD_B3_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_BDD_B3_TESTING
        ++downward_count_;
#endif
    }
    ...
    void add_exact_support()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_BDD_B3_DIAGNOSTICS("exact", indices_.size());
#ifdef COPOSIT_BDD_B3_TESTING
        ++exact_count_;
#endif
    }
    ...
    std::optional<interval_bdd> supports_;
#ifdef COPOSIT_BDD_B3_TESTING
...
#ifdef COPOSIT_BDD_B3_TESTING
bool bdd_b3_prefers_negative_singular_orientation_for_testing(size_t positive_products,
                                                               size_t negative_products) noexcept
{
    return negative_orientation_has_larger_upper(positive_products, negative_products);
}

size_t bdd_b3_optimized_certificate_count_for_testing() noexcept
{
    return last_optimized_certificate_count;
}

size_t bdd_b3_combined_ray_sweep_count_for_testing() noexcept
{
    return last_combined_ray_sweep_count;
}

size_t bdd_b3_combined_ray_improvement_count_for_testing() noexcept
{
    return last_combined_ray_improvement_count;
}

size_t bdd_b3_pair_curvature_exclusion_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t bdd_b3_support_curvature_exclusion_count_for_testing() noexcept
{
    return last_support_curvature_exclusion_count;
}

size_t bdd_b3_pair_upward_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t bdd_b3_support_upward_count_for_testing() noexcept
{
    return last_support_curvature_exclusion_count;
}

size_t bdd_b3_downward_count_for_testing() noexcept
{
    return last_downward_count;
}

size_t bdd_b3_exact_count_for_testing() noexcept
{
    return last_exact_count;
}

size_t bdd_b3_high_float_rejection_count_for_testing() noexcept
{
    return last_high_float_rejection_count;
}

bool bdd_b3_floating_psd_candidate_for_testing(const matrix_integer& matrix, const std::vector<size_t>& indices)
{
...
bool bdd_b3_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer& matrix, const std::vector<size_t>& indices)
{
...
size_t bdd_b3_shortlist_limit_for_testing(size_t matrix_dimension, size_t support_dimension)
{
    return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool bdd_b3_prefers_ray_candidate_for_testing(size_t candidate_upper, size_t candidate_width, size_t candidate_gains,
...
bool bdd_b3_check_support_for_testing(const matrix_integer& matrix, const std::vector<size_t>& indices)
{
    return dickinson_checker(matrix.rows(), copositivity_mode::strictly_copositive).check_support_for_testing(matrix, indices);
}

bool bdd_b3_certificate_for_testing(
    const matrix_integer& matrix, const std::vector<size_t>& indices, support& lower, support& upper)
{
    return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
...
size_t bdd_b3_fixed_support_upper_size_for_testing() noexcept
{
    return last_fixed_support_upper_size;
}

size_t bdd_b3_uncovered_count(
    size_t dimension, size_t cardinality, const std::vector<std::pair<uint64_t, uint64_t>>& intervals)
{
    ...
    interval_bdd diagram(support_context_);
...
size_t bdd_b3_pair_exclusion_uncovered_count(
...
{
    support_context support_context_(dimension);
    interval_bdd supports(support_context_);
...
size_t bdd_b3_high_filter_uncovered_count(
