...
public:
    explicit interval_bdd(size_t dimension)
        ...
        , cardinality_roots_(dimension + 1, missing)
    ...
    {
        size_t upper_size = 0;
        for (size_t index = 0; index < dimension_; ++index) upper_size += upper.contains(index);
    ...
    void add_pair_upward_closure(size_t first, size_t second)
    {
        ...
        lower.set(first);
        lower.set(second);
        for (size_t index = 0; index < dimension_; ++index) upper.set(index);
...
private:
    ...
    size_t interval_family(const support& lower, const support& upper)
    {
        ...
        for (size_t variable_value = dimension_; variable_value-- > 0;) {
            ...
            if (lower.contains(bit)) root = make_node(variable_value, empty, root);
            else if (!upper.contains(bit)) root = make_node(variable_value, root, empty);
        }
        return root;
    }
...
{
    interval_bdd supports(dimension);
