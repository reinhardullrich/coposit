...
matrix_integer narrow_two_coordinate_witness()
{
    matrix_integer matrix = constant_off_diagonal(4, 1, 1);
    matrix(2, 2) = integer(2);
    matrix(3, 3) = integer(3);
    if (fmpz_set_str(matrix(1, 1).native_handle(), "100000000000000000000000000", 10) != 0
        || fmpz_set_str(matrix(0, 1).native_handle(), "-10000000000001", 10) != 0) {
        throw std::runtime_error("invalid test integer");
    }
    matrix(1, 0) = matrix(0, 1);
    return matrix;
}

TEST(SupportPolishedFrankWolfeDickinsonModelTest, FindsAnExactHigherOrderNegativeWitness)
{
...
TEST(SupportPolishedFrankWolfeDickinsonModelTest, CombinedModeClassifiesTheBoundary)
{
...
TEST(SupportPolishedFrankWolfeDickinsonModelTest, PolishesAProposalWhoseSmallCoordinateIsLostByQuantization)
{
    EXPECT_FALSE(model::solve(narrow_two_coordinate_witness()));
    EXPECT_TRUE(model::frank_wolfe_witness_found_for_testing());
    EXPECT_TRUE(model::iterative_frank_wolfe_witness_found_for_testing());
    EXPECT_TRUE(model::support_polishing_witness_found_for_testing());
}

TEST(SupportPolishedFrankWolfeDickinsonModelTest, FallsBackToDickinson)
{
...
TEST(SupportPolishedFrankWolfeDickinsonModelTest, PreservesArbitraryPrecisionScaling)
{
    integer scale;
...
TEST(SupportPolishedFrankWolfeDickinsonModelTest, DistinguishesLowDimensionalCases)
{
