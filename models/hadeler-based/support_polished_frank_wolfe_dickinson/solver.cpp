...
#ifdef COPOSIT_SUPPORT_POLISHED_FRANK_WOLFE_DICKINSON_TESTING
bool last_frank_wolfe_witness = false;
bool last_iterative_frank_wolfe_witness = false;
bool last_support_polishing_witness = false;
#endif

class frank_wolfe_witness_search {
public:
    explicit frank_wolfe_witness_search(size_t dimension)
    ...
    bool find(const matrix_integer& matrix)
    {
        ...
        iterative_witness_ = best_value <= support_polishing_threshold && exact_nonpositive(matrix, best_x_);
        return iterative_witness_;
    }

    bool used_iterative_witness() const noexcept { return iterative_witness_; }
    bool used_support_polishing() const noexcept { return support_polishing_witness_; }
...
private:
    ...
    static constexpr double support_polishing_threshold = 1.0e-12;
    ...
    bool exact_nonpositive(const matrix_integer& matrix, const std::vector<double>& candidate)
    {
        ...
        for (size_t coordinate = 0; coordinate < candidate.size(); ++coordinate) {
            ...
            if (std::isfinite(value) && value > 0.0) polishing_support_.push_back(coordinate);
        ...
        if (polishing_support_.empty()) polishing_support_.push_back(largest);
        ...
        return exact_support_nonpositive(matrix, polishing_support_);
    }

    bool exact_support_nonpositive(const matrix_integer& matrix, const std::vector<size_t>& indices)
    {
        if (indices.size() <= 3) {
            const copositivity_classification result = small_copositivity::classify_principal(matrix, indices.data(), indices.size());
            support_polishing_witness_ = !result.is_strictly_copositive;
            if (support_polishing_witness_) witness_sign_ = result.is_copositive ? 0 : -1;
            return support_polishing_witness_;
        }
        ...
        for (size_t row = 0; row < indices.size(); ++row) {
            timeout_checkpoint();
            for (size_t column = 0; column <= row; ++column) principal_(row, column) = matrix(indices[row], indices[column]);
        }
        ...
        if (factorization_.factorize_inplace(principal_) != 0) {
            ...
            support_polishing_witness_ = true;
            for (size_t row = 0; row < indices.size(); ++row) support_polishing_witness_ &= solution_(row, 0).sign() <= 0;
            if (support_polishing_witness_) witness_sign_ = -1;
            return support_polishing_witness_;
        }
        ...
        for (size_t row = 0; row < indices.size(); ++row) {
            nonnegative &= solution_(row, 0).sign() >= 0;
            nonpositive &= solution_(row, 0).sign() <= 0;
        }
        support_polishing_witness_ = nonnegative || nonpositive;
        if (support_polishing_witness_) witness_sign_ = 0;
        return support_polishing_witness_;
    }
    ...
    std::vector<size_t> polishing_support_;
    ...
    bool support_polishing_witness_ = false;
    int witness_sign_ = 1;
};
...
bool solve(const matrix_integer& matrix, copositivity_mode mode)
{
    ...
    last_support_polishing_witness = false;
#endif
    if (dimension <= 3) return small_copositivity::check(matrix, mode);

    frank_wolfe_witness_search witness_search(dimension);
    if (witness_search.find(matrix)) {
        ...
        last_support_polishing_witness = witness_search.used_support_polishing();
#endif
...
bool support_polishing_witness_found_for_testing() noexcept
{
    return last_support_polishing_witness;
}
#endif
