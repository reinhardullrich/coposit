...
TEST(ZddDickinsonTest, PreservesStrictDickinsonDecisions)
{
...
TEST(ZddDickinsonTest, ClassifiesCopositiveBoundary)
{
...
TEST(ZddDickinsonTest, SubtractsTheUnionOfBoundedIntervals)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011}, {0b100, 0b110}};
    EXPECT_EQ(model::interval_zdd_uncovered_count(3, 1, intervals).first, 1U);
    EXPECT_EQ(model::interval_zdd_uncovered_count(3, 2, intervals).first, 1U);
}

TEST(ZddDickinsonTest, DeletesExpiredIntervalsWithoutDamagingOverlappingLiveCoverage)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b0001, 0b0011}, {0b0001, 0b1111}};
    EXPECT_EQ(model::interval_zdd_expired_bucket_count(4, 3, intervals), 1U);
    EXPECT_EQ(model::interval_zdd_uncovered_count(4, 3, intervals).first, brute_uncovered_count(4, 3, intervals));
}

TEST(ZddDickinsonTest, MatchesBruteForceForEveryPairOfFourDimensionalIntervals)
{
    ...
    for (const auto& first : valid_intervals) {
        for (const auto& second : valid_intervals) {
            ...
            for (size_t cardinality = 1; cardinality <= dimension; ++cardinality)
                EXPECT_EQ(model::interval_zdd_uncovered_count(dimension, cardinality, intervals).first,
                          brute_uncovered_count(dimension, cardinality, intervals));
        }
    }
}

TEST(ZddDickinsonTest, DoesNotEmitSupportsAlreadyCoveredByIdentityCertificates)
{
    matrix_integer identity;
    identity.set_identity(4);
    zdd_dickinson_diagnostics::clear();
    EXPECT_TRUE(model::solve(identity));

    ASSERT_EQ(zdd_dickinson_diagnostics::events.size(), 4U);
    for (const auto& event : zdd_dickinson_diagnostics::events)
        EXPECT_EQ(event, (zdd_dickinson_diagnostics::event{"process", 1}));
}

TEST(ZddDickinsonTest, KeepsPackedSupportsBeyondOneWord)
{
...
TEST(ZddDickinsonTest, CooperativeTimeoutEscapesDiagramOperations)
{
    request_timeout();
    EXPECT_THROW(model::interval_zdd_uncovered_count(15, 7, {}), timeout_requested);
    reset_timeout();
}
