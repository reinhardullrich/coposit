...
bool recorded(std::string_view name) {
  return std::any_of(coposit::dual_frontier_nbc_diagnostics::events.begin(),
                     coposit::dual_frontier_nbc_diagnostics::events.end(),
                     [&](const auto &event) { return event.name == name; });
}

TEST(DualFrontierNbcTest, ClassifiesStrictCopositivityInOneTraversal) {
  coposit::dual_frontier_nbc_diagnostics::clear();
...
TEST(DualFrontierNbcTest, RetainsOnlyCeilingDickinsonPruning) {
  coposit::dual_frontier_nbc_diagnostics::clear();
  const auto matrix = symmetric_matrix(2, {1, -1, 1});

  EXPECT_TRUE(
      model::dual_frontier_nbc_process_support_for_testing(matrix, {0}));
  EXPECT_TRUE(recorded("dickinson_partial_discarded"));
  EXPECT_FALSE(recorded("dickinson_ceiling"));
}

TEST(DualFrontierNbcTest, FindsAnExactNegativeWitnessAfterTheFloatWalk) {
...
TEST(DualFrontierNbcTest, KeepsPairCurvaturePruning) {
  coposit::dual_frontier_nbc_diagnostics::clear();
  const auto result = model::classify(symmetric_matrix(2, {1, 2, 1}));
...
TEST(DualFrontierNbcTest, PreservesTheEmittedSeedAcrossWalkVerification) {
  const auto parsed = parsers::fracessa_matrix_parser::parse(
      "9#18843461267942957920810105202638666044164228350784975569055276078649597,"
