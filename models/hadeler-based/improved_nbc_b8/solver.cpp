...
public:
  ...
  bool check(const matrix_integer &matrix) {
    ...
    while (low <= matrix.rows()) {
      if (!process_low_support(matrix, low))
        return finish(false);
...
private:
  ...
  void record_interval_certificate(const support &lower, const support &upper) {
    ...
    event << "model=improved_nbc_b8 n=" << product_.size()
  ...
  void record_closure_certificate(std::string_view kind,
    ...
    append_indices(event, source);
    event
        << " upper=all exclude_empty=no floating_checked=no exact_checked=yes";
    diagnostics::record_history_event("certificate", event.str());
  }

  void record_pair_curvature_certificate(size_t first, size_t second) {
    record_closure_certificate("pair_curvature", "initial", {first, second});
  }

  bool process_low_support(const matrix_integer &matrix, size_t &low) {
    while (low <= matrix.rows()) {
      diagnostics_.stage(low);
      COPOSIT_IMPROVED_NBC_B8_DIAGNOSTICS("stage_low", low);
  ...
  bool process_selected_support(const matrix_integer &matrix) {
    ...
    COPOSIT_IMPROVED_NBC_B8_DIAGNOSTICS("process_low", indices_.size());
  ...
  void install_pair_curvature_exclusions(const matrix_integer &matrix) {
    ...
    for (size_t first = 0; first < matrix.rows(); ++first) {
      ...
      for (size_t second = first + 1; second < matrix.rows(); ++second) {
        ...
        COPOSIT_IMPROVED_NBC_B8_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_IMPROVED_NBC_B8_TESTING
...
#endif
      COPOSIT_IMPROVED_NBC_B8_DIAGNOSTICS("combined_ray", ray + 1);
  ...
  void add_upward_closure() {
    ...
    if (diagnostics_.active())
      diagnostics_.certificate(product_.size() - indices_.size(),
                               product_.size());
    record_closure_certificate("support_curvature", "low", indices_);
    COPOSIT_IMPROVED_NBC_B8_DIAGNOSTICS("support_upward", indices_.size());
  }
...
#ifdef COPOSIT_IMPROVED_NBC_B8_TESTING
bool improved_nbc_b8_prefers_negative_singular_orientation_for_testing(
...
size_t improved_nbc_b8_combined_ray_sweep_count_for_testing() noexcept {
  return last_combined_ray_sweep_count;
}

size_t improved_nbc_b8_combined_ray_improvement_count_for_testing() noexcept {
  return last_combined_ray_improvement_count;
}

size_t improved_nbc_b8_pair_curvature_exclusion_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t
improved_nbc_b8_support_curvature_exclusion_count_for_testing() noexcept {
...
bool improved_nbc_b8_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
size_t improved_nbc_b8_shortlist_limit_for_testing(size_t matrix_dimension,
                                                   size_t support_dimension) {
  return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool improved_nbc_b8_prefers_ray_candidate_for_testing(
...
size_t improved_nbc_b8_fixed_support_upper_size_for_testing() noexcept {
...
size_t improved_nbc_b8_pair_exclusion_uncovered_count(size_t dimension,
                                                      size_t cardinality,
                                                      size_t first,
                                                      size_t second) {
  improved_nbc_upward_supports diagram(dimension);
...
size_t improved_nbc_b8_interval_clause_size(size_t dimension,
                                            uint64_t lower_mask,
                                            uint64_t upper_mask) {
  size_t lower_size = 0;
  size_t upper_size = 0;
