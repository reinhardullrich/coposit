...
private:
    ...
    bool process_low_support(const matrix_integer& matrix, size_t& low, size_t high)
    {
        while (low <= high) {
            ...
            COPOSIT_NBC_B7_DIAGNOSTICS("stage_low", low);
            ...
            if (supports_->all_future_covered()) return true;
        }
        return true;
    }
    ...
    bool process_high_support(const matrix_integer& matrix, size_t low, size_t& high)
    {
        while (low <= high) {
            ...
            COPOSIT_NBC_B7_DIAGNOSTICS("stage_high", high);
    ...
    bool process_selected_support(const matrix_integer& matrix, pruning_direction direction, bool from_low_frontier)
    {
        ...
        COPOSIT_NBC_B7_DIAGNOSTICS(from_low_frontier ? "process_low" : "process_high", indices_.size());
    ...
    void install_pair_curvature_exclusions(const matrix_integer& matrix)
    {
        ...
        for (size_t first = 0; first < matrix.rows(); ++first) {
            ...
            for (size_t second = first + 1; second < matrix.rows(); ++second) {
                ...
                COPOSIT_NBC_B7_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_NBC_B7_TESTING
        ...
        const matrix_integer& matrix, pruning_direction direction = pruning_direction::upward, bool from_low_frontier = true)
    {
        if (!from_low_frontier) {
            ...
            if (!floating_filter_.looks_positive_semidefinite(indices_)) {
                COPOSIT_NBC_B7_DIAGNOSTICS("high_float_reject", indices_.size());
#ifdef COPOSIT_NBC_B7_TESTING
    ...
    bool process_curvature_only_nonsingular(pruning_direction direction)
    {
        if (factorization_.is_positive_definite()) {
            if (direction != pruning_direction::upward) add_downward_closure();
            return true;
        }
...
#endif
            COPOSIT_NBC_B7_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    bool add_certificate()
    {
        ...
        COPOSIT_NBC_B7_DIAGNOSTICS("dickinson", indices_.size());
        return true;
    }
    ...
    void add_upward_closure()
    {
        ...
        COPOSIT_NBC_B7_DIAGNOSTICS("support_upward", indices_.size());
    }
    ...
    void add_downward_closure()
    {
        ...
        support upper = support_context_.make();
        for (const size_t index : indices_) support_context_.set(upper, index);
        supports_->add_interval(floor, upper);
        support_context_.release(std::move(floor));
        support_context_.release(std::move(upper));
        diagnostics_.certificate();
        COPOSIT_NBC_B7_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_NBC_B7_TESTING
        ++downward_count_;
#endif
    }
...
#ifdef COPOSIT_NBC_B7_TESTING
bool nbc_b7_prefers_negative_singular_orientation_for_testing(size_t positive_products,
...
size_t nbc_b7_combined_ray_sweep_count_for_testing() noexcept
{
    return last_combined_ray_sweep_count;
}

size_t nbc_b7_combined_ray_improvement_count_for_testing() noexcept
{
    return last_combined_ray_improvement_count;
}

size_t nbc_b7_pair_curvature_exclusion_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t nbc_b7_support_curvature_exclusion_count_for_testing() noexcept
{
...
bool nbc_b7_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer& matrix, const std::vector<size_t>& indices)
{
...
size_t nbc_b7_shortlist_limit_for_testing(size_t matrix_dimension, size_t support_dimension)
{
    return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool nbc_b7_prefers_ray_candidate_for_testing(size_t candidate_upper, size_t candidate_width, size_t candidate_gains,
...
size_t nbc_b7_fixed_support_upper_size_for_testing() noexcept
{
    ...
    size_t dimension, size_t cardinality, const std::vector<std::pair<uint64_t, uint64_t>>& intervals)
{
    ...
    nbc_upward_supports diagram(context);
    ...
    size_t count = 0;
    const auto result = diagram.enumerate_cardinality(cardinality, &count, &count_uncovered_support);
    assert(result == nbc_upward_supports::enumeration_result::exhausted);
    return count;
}

size_t nbc_b7_pair_exclusion_uncovered_count(
    ...
    const std::vector<std::vector<size_t>>& downward, const std::vector<std::vector<size_t>>& exact)
{
    ...
    for (const auto& indices : upward) {
        support lower = context.make();
        for (const size_t index : indices) context.set(lower, index);
    ...
    for (const auto& indices : downward) {
        ...
        support upper = context.make();
        for (const size_t index : indices) context.set(upper, index);
    ...
    for (const auto& indices : exact) {
        ...
        for (const size_t index : indices) context.set(singleton, index);
    ...
    size_t count = 0;
    const auto result = supports.enumerate_cardinality(cardinality, &count, &count_uncovered_support);
    assert(result == nbc_upward_supports::enumeration_result::exhausted);
    return count;
}

size_t nbc_b7_interval_clause_size(size_t dimension, uint64_t lower_mask, uint64_t upper_mask)
{
    size_t lower_size = 0;
    size_t upper_size = 0;
    for (size_t bit = 0; bit < dimension; ++bit) {
