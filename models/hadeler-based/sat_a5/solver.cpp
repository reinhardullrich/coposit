...
size_t last_monotone_first_improvement_upper_size = 0;
size_t last_monotone_first_selected_upper_size = 0;
...
public:
    ...
    bool check(const matrix_integer& matrix)
    {
        ...
        monotone_first_improvement_upper_size_ = 0;
        monotone_first_selected_upper_size_ = 0;
#endif
        ...
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            while (supports_->take_first(indices_)) {
                ...
                COPOSIT_SAT_A5_DIAGNOSTICS("process", subset_dimension);
...
private:
    ...
    bool try_monotone_enlargement()
    {
        ...
        while (current_score_.upper_size < product_.size()) {
            ...
            size_t first_improvement_upper_size = 0;
            ...
            for (size_t target = 0; target < outside_dimension; ++target) {
                ...
                const exact_lp_candidate_result result =
                    consider_lp_point(proposal.point, lp_required_rows_, selected_score, selected);
                if (result == exact_lp_candidate_result::negative_witness) return false;
                if (result == exact_lp_candidate_result::improved) {
                    if (first_improvement_upper_size == 0) first_improvement_upper_size = selected_score.upper_size;
                    if (selected_score.upper_size == product_.size()) break;
                    continue;
                }
#ifdef COPOSIT_SAT_A5_TESTING
...
#ifdef COPOSIT_SAT_A5_TESTING
            if (monotone_first_improvement_upper_size_ == 0) {
                monotone_first_improvement_upper_size_ = first_improvement_upper_size;
                monotone_first_selected_upper_size_ = selected_score.upper_size;
            }
    ...
    exact_lp_candidate_result consider_lp_point(const std::vector<double>& point,
                                                ...
                                                coverage_score& selected_score, bool& selected)
    {
        ...
        for (const uint64_t scale :
            ...
            if (!better_score(candidate_score, current_score_)) continue;
            improved = true;
        ...
        return improved ? exact_lp_candidate_result::improved : exact_lp_candidate_result::unchanged;
    }
...
#endif
            COPOSIT_SAT_A5_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    void publish_test_counters() const noexcept
    {
        ...
        last_monotone_first_improvement_upper_size = monotone_first_improvement_upper_size_;
        last_monotone_first_selected_upper_size = monotone_first_selected_upper_size_;
    }
#endif
    ...
    size_t monotone_first_improvement_upper_size_ = 0;
    size_t monotone_first_selected_upper_size_ = 0;
...
size_t sat_a5_monotone_lp_attempt_count_for_testing() noexcept { return last_monotone_lp_attempt_count; }
size_t sat_a5_monotone_lp_feasible_count_for_testing() noexcept { return last_monotone_lp_feasible_count; }
size_t sat_a5_monotone_extension_count_for_testing() noexcept { return last_monotone_extension_count; }
size_t sat_a5_monotone_exact_rejection_count_for_testing() noexcept { return last_monotone_exact_rejection_count; }
size_t sat_a5_monotone_incumbent_upper_size_for_testing() noexcept { return last_monotone_incumbent_upper_size; }
size_t sat_a5_monotone_final_upper_size_for_testing() noexcept { return last_monotone_final_upper_size; }
size_t sat_a5_monotone_first_improvement_upper_size_for_testing() noexcept
{
    return last_monotone_first_improvement_upper_size;
}
size_t sat_a5_monotone_first_selected_upper_size_for_testing() noexcept
{
    return last_monotone_first_selected_upper_size;
}
