...
SAT-A5 first runs that complete Rays procedure unchanged. If its exact result is $u_{\rm rays}$, it fixes
...
that index. It evaluates every target reached before the shared local deadline against the same incumbent and installs the strongest
exactly verified improvement, maximizing $|U|$ first and the interval width second. The chosen set becomes the new $F$, and the
process repeats. SAT-A5 therefore searches one best-improvement monotone chain of upper endpoints; it never exchanges a Rays index
for a different index and never claims that the final endpoint is globally maximum.

## Name, Sources, And Classification

The identifier is `sat_a5`.

- **SAT** names the persistent Boolean representation of the supports not yet covered by certificates.
- **A5** identifies the fifth bounded experiment derived from the fixed-support right-hand-side certificate-complex analysis. A1
  retains an antichain of endpoints, A2 uses a maximum-halfspace relaxation, A3 accepts the first exact monotone improvement, and A4
  applies monotone enlargement without Halfspace-Rays. A5 changes only A3's monotone target selection.
...
$U$. For an omitted outside index $q$, SAT-A5 asks whether there is a vector $b>0$ satisfying
...
$10^{15}$. Those coefficients are then multiplied by the exact direction and product matrices. SAT-A5 retains a candidate only if the
...
All omitted targets in one pass are tested against the same $F$. Among the exactly verified proposals reached before the local
deadline, SAT-A5 chooses the lexicographically largest $(|U|,d)$ rather than the first improvement in target-index order. Only that
endpoint becomes the new $F$, after which a new pass begins. Thus the model builds a strictly increasing chain
...
million entries. If the deadline interrupts a pass after an exact improvement has already been found, SAT-A5 installs the best such
improvement before stopping the optional search. Otherwise interruption, numerical infeasibility, or failed exact reconstruction
leaves the current exact incumbent unchanged. These outcomes mean only “no verified enlargement was found within this bounded
numerical search,” not “no enlargement exists.” SAT-A5 performs no MILP branching, index exchange, or conflict extraction.
...
7. Let $F$ be the exact Rays upper endpoint. For each omitted index reached before the shared deadline, solve the bounded monotone
   feasibility probe preserving the same $F$.
8. Exactly verify every proposed strict enlargement, install the best $(|U|,d)$ from that pass as the new $F$, and repeat; otherwise
...
- A stronger attainable endpoint may require losing a Rays index before gaining several others. The monotone constraint forbids that
  exchange, so SAT-A5 can stop at a non-maximal endpoint.
- A feasible monotone enlargement can be numerically missed or can lie on a boundary that the finite reconstruction scales fail to
  recover. SAT-A5 never treats that miss as an infeasibility proof.
