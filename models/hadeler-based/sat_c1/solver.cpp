        ...
        : variable_by_index_(dimension)
    {
        for (size_t index = 0; index < dimension; ++index) {
            if (lower.contains(index)) lower_.push_back(index);
                ...
                variable_by_index_[index] = static_cast<int>(optional_.size());
            }
        }
        ...
        if (!solver_.set("ilb", 2)) throw std::runtime_error("CaDiCaL lacks incremental lazy backtracking");
        solver_.connect_terminator(&terminator_);
        solver_.reserve(static_cast<int>(optional_.size()));

        for (const auto& core : known_bad_cores) {
            if (!is_subset_of_upper(core, upper)) continue;
            add_bad(core);
        }
        for (size_t local = 0; local < optional_.size(); ++local) solver_.phase(static_cast<int>(local) + 1);
    }

    bool take(std::vector<size_t>& indices)
    {
        const int status = solver_.solve();
        if (status == CaDiCaL::UNSATISFIABLE) return false;
        if (status != CaDiCaL::SATISFIABLE) {
        ...
        for (size_t local = 0; local < optional_.size(); ++local)
            if (solver_.val(static_cast<int>(local) + 1) > 0) indices.push_back(optional_[local]);
        std::sort(indices.begin(), indices.end());
        return true;
    }

    bool take_large(std::vector<size_t>& indices)
    {
        for (size_t local = 0; local < optional_.size(); ++local) solver_.assume(static_cast<int>(local) + 1);
        int status = solver_.solve();
        if (status != CaDiCaL::SATISFIABLE) {
            if (status != CaDiCaL::UNSATISFIABLE) require_satisfiable(status);
            status = solver_.solve();
            if (status == CaDiCaL::UNSATISFIABLE) return false;
            require_satisfiable(status);
        }

        indices = lower_;
        for (size_t local = 0; local < optional_.size(); ++local) {
            const int value = solver_.val(static_cast<int>(local) + 1);
            if (value >= 0) indices.push_back(optional_[local]);
        }
        std::sort(indices.begin(), indices.end());
        return true;
    }

    void add_bad(const std::vector<size_t>& core)
    {
        for (const size_t index : core) {
            const int variable = variable_by_index_[index];
            if (variable != 0) solver_.add(-variable);
        }
        solver_.add(0);
    }

    void add_good(const std::vector<size_t>& maximal_good)
    {
        size_t local = 0;
        for (const size_t index : optional_) {
            while (local < maximal_good.size() && maximal_good[local] < index) ++local;
            if (local == maximal_good.size() || maximal_good[local] != index) solver_.add(variable_by_index_[index]);
        }
        solver_.add(0);
    }

private:
    static void require_satisfiable(int status)
    {
        if (status == CaDiCaL::SATISFIABLE) return;
        timeout_checkpoint();
        throw std::runtime_error("CaDiCaL returned an inconclusive local curvature result without a coposit timeout");
    }
    ...
    std::vector<int> variable_by_index_;
};
...
public:
    ...
    bool is_lower_index(size_t index) const noexcept
    {
        return std::binary_search(lower_.begin(), lower_.end(), index);
    }
...
private:
    ...
    bool process_low_support(const matrix_integer& matrix, size_t& low, size_t high)
    {
        while (low <= high) {
            ...
            COPOSIT_SAT_C1_DIAGNOSTICS("stage_low", low);
    ...
    bool process_high_support(const matrix_integer& matrix, size_t low, size_t& high)
    {
        while (low <= high) {
            ...
            COPOSIT_SAT_C1_DIAGNOSTICS("stage_high", high);
    ...
    bool process_selected_support(const matrix_integer& matrix, pruning_direction direction, bool from_low_frontier)
    {
        ...
        COPOSIT_SAT_C1_DIAGNOSTICS(from_low_frontier ? "process_low" : "process_high", indices_.size());
    ...
    void install_pair_curvature_exclusions(const matrix_integer& matrix)
    {
        ...
        for (size_t first = 0; first < matrix.rows(); ++first) {
            ...
            for (size_t second = first + 1; second < matrix.rows(); ++second) {
                ...
                COPOSIT_SAT_C1_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_SAT_C1_TESTING
        ...
        const matrix_integer& matrix, pruning_direction direction = pruning_direction::upward, bool from_low_frontier = true)
    {
        if (!from_low_frontier) {
            ...
            if (!floating_filter_.looks_positive_semidefinite(indices_)) {
                ...
                COPOSIT_SAT_C1_DIAGNOSTICS("high_float_reject", indices_.size());
#ifdef COPOSIT_SAT_C1_TESTING
...
#endif
            COPOSIT_SAT_C1_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    void add_existing_vector_curvature_cores(
        const matrix_integer& matrix, const support& lower, const support& upper, const integer& quadratic)
    {
        integer sum;
        for (size_t local = 0; local < indices_.size(); ++local) sum += solution_(local, 0);
        ...
        for (size_t index = 0; index < matrix.rows(); ++index) {
            ...
            auto child = core;
            child.insert(std::lower_bound(child.begin(), child.end(), index), index);
            if (!add_global_curvature_core(child)) continue;
            if (diagnostics_.active()) diagnostics_.certificate(matrix.rows() - child.size(), matrix.rows());
            COPOSIT_SAT_C1_DIAGNOSTICS("certificate_vector_upward", child.size());
#ifdef COPOSIT_SAT_C1_TESTING
    ...
    void search_interval_curvature(const matrix_integer& matrix, const support& lower, const support& upper)
    {
        ...
        if (!contains_known_curvature_core(upper)) {
            curvature_residual_.build(matrix, lower, upper);
            residual_ready = true;
            if (residual_is_positive_definite(candidate)) return;
        }
        ...
        do {
            ...
            if (candidate_is_bad) {
                shrink_bad_curvature_support(candidate);
                local.add_bad(candidate);
                if (!add_global_curvature_core(candidate)) continue;
                if (diagnostics_.active()) diagnostics_.certificate(matrix.rows() - candidate.size(), matrix.rows());
                COPOSIT_SAT_C1_DIAGNOSTICS("local_curvature_upward", candidate.size());
#ifdef COPOSIT_SAT_C1_TESTING
                ++local_curvature_core_count_;
#endif
            } else {
                grow_good_curvature_support(candidate, upper);
                local.add_good(candidate);
                COPOSIT_SAT_C1_DIAGNOSTICS("local_curvature_good", candidate.size());
            }
        } while (local.take(candidate));
    }
    ...
    void shrink_bad_curvature_support(std::vector<size_t>& indices)
    {
        for (size_t position = 0; position < indices.size();) {
            timeout_checkpoint();
            if (curvature_residual_.is_lower_index(indices[position])) {
                ++position;
                continue;
            }
            auto smaller = indices;
            smaller.erase(smaller.begin() + static_cast<std::ptrdiff_t>(position));
            if (!residual_is_positive_definite(smaller)) indices.swap(smaller);
            else ++position;
        }
    }
    ...
    void add_upward_closure()
    {
        ...
        COPOSIT_SAT_C1_DIAGNOSTICS("support_upward", indices_.size());
    }
    ...
    void add_downward_closure()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_SAT_C1_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_SAT_C1_TESTING
        ++downward_count_;
#endif
    }
    ...
    void add_exact_support()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_SAT_C1_DIAGNOSTICS("exact", indices_.size());
#ifdef COPOSIT_SAT_C1_TESTING
        ++exact_count_;
#endif
    }
...
#ifdef COPOSIT_SAT_C1_TESTING
bool sat_c1_prefers_negative_singular_orientation_for_testing(size_t positive_products,
...
size_t sat_c1_combined_ray_sweep_count_for_testing() noexcept
{
    return last_combined_ray_sweep_count;
}

size_t sat_c1_combined_ray_improvement_count_for_testing() noexcept
{
    return last_combined_ray_improvement_count;
}

size_t sat_c1_pair_curvature_exclusion_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t sat_c1_support_curvature_exclusion_count_for_testing() noexcept
{
...
size_t sat_c1_shortlist_limit_for_testing(size_t matrix_dimension, size_t support_dimension)
{
    return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool sat_c1_prefers_ray_candidate_for_testing(size_t candidate_upper, size_t candidate_width, size_t candidate_gains,
...
size_t sat_c1_fixed_support_upper_size_for_testing() noexcept
{
