...
Classification: coposit-created exact CP/SCP experiment. Improved NBC-B7 alternates between low and high cardinalities. The low frontier uses
...
Improved NBC-B7 uses that observation as a search certificate.
...
The identifier is `improved_nbc_b7`.
...
The model is an independent copy of [`nbc_b7`](../nbc_b7/ALGORITHM.md). It preserves NBC-B7's two-frontier traversal, floating
...
interval compaction. It changes no mathematical pruning rule. Its only algorithm-engine change is that the copied NBC source has a
defined repeated-call boundary and a permanent-inconsistency latch. The original `nbc_b7` model and original wrapper remain available
unchanged for direct comparison. Dickinson intervals come from Peter J. C.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Improved NBC-B7 solves
...
When low-frontier curvature does not already remove the upward closure, Improved NBC-B7 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Improved NBC-B7 performs the inherited exact
...
Improved NBC-B7 retains SAT-B3's singular case. Suppose
...
Improved NBC-B7 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
