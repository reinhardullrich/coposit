...
size_t improved_nbc_b7_pair_upward_count_for_testing() noexcept;
size_t improved_nbc_b7_support_upward_count_for_testing() noexcept;
size_t improved_nbc_b7_downward_count_for_testing() noexcept;
size_t improved_nbc_b7_high_float_rejection_count_for_testing() noexcept;
size_t improved_nbc_b7_optimized_certificate_count_for_testing() noexcept;
bool improved_nbc_b7_floating_psd_candidate_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices);
bool improved_nbc_b7_check_support_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices);
bool improved_nbc_b7_certificate_for_testing(const matrix_integer &matrix,
                                             const std::vector<size_t> &indices,
                                             support &lower, support &upper);
size_t improved_nbc_b7_uncovered_count(
...
TEST(ImprovedNbcB7Test, ResumesOneModelCallsWithoutRepeatingSupports) {
...
TEST(ImprovedNbcB7Test, LatchesCollectiveRootInconsistencyAcrossCalls) {
...
TEST(ImprovedNbcB7Test, ExhaustivelyMatchesSingleIntervalCoverage) {
...
TEST(ImprovedNbcB7Test, ExhaustivelyMatchesIntervalsAddedBetweenModelCalls) {
...
TEST(ImprovedNbcB7Test, AlternatesFromOneLowSupportToTheNextOpenHighSupport) {
  improved_nbc_b7_diagnostics::clear();
  ...
  EXPECT_EQ(model::improved_nbc_b7_pair_upward_count_for_testing(), 0U);
  EXPECT_EQ(model::improved_nbc_b7_support_upward_count_for_testing(), 0U);
  EXPECT_EQ(model::improved_nbc_b7_downward_count_for_testing(), 1U);
  const auto low =
      std::find(improved_nbc_b7_diagnostics::events.begin(),
                improved_nbc_b7_diagnostics::events.end(),
                improved_nbc_b7_diagnostics::event{"process_low", 1});
  ASSERT_NE(low, improved_nbc_b7_diagnostics::events.end());
  const auto high = std::find_if(
      std::next(low), improved_nbc_b7_diagnostics::events.end(),
      [](const auto &event) {
        return event.name == "process_low" || event.name == "process_high";
      });
  ASSERT_NE(high, improved_nbc_b7_diagnostics::events.end());
  EXPECT_EQ(high->name, "process_high");
  EXPECT_NE(std::find(improved_nbc_b7_diagnostics::events.begin(),
                      improved_nbc_b7_diagnostics::events.end(),
                      improved_nbc_b7_diagnostics::event{"dickinson", 1}),
            improved_nbc_b7_diagnostics::events.end());
}

TEST(ImprovedNbcB7Test, RecordsChronologicalCertificatesForTheRenderer) {
  ...
  const size_t dickinson =
      history.find("event=certificate sequence=1 model=improved_nbc_b7 n=3 "
                   "frontier=low kind=dickinson source=[");
  const size_t downward =
      history.find("event=certificate sequence=2 model=improved_nbc_b7 n=3 "
...
TEST(ImprovedNbcB7Test, UsesTheFloatingFilterBeforeExactHighFrontierWork) {
  improved_nbc_b7_diagnostics::clear();
  std::ostringstream ignored_output;
  {
    diagnostics::reporter reporter(false, ignored_output, true, true);
  ...
  const auto high =
      std::find(improved_nbc_b7_diagnostics::events.begin(),
                improved_nbc_b7_diagnostics::events.end(),
                improved_nbc_b7_diagnostics::event{"process_high", 3});
  ASSERT_NE(high, improved_nbc_b7_diagnostics::events.end());
  ASSERT_NE(std::next(high), improved_nbc_b7_diagnostics::events.end());
  EXPECT_EQ(*std::next(high),
            (improved_nbc_b7_diagnostics::event{"high_float_reject", 3}));
  EXPECT_GT(model::improved_nbc_b7_high_float_rejection_count_for_testing(),
            0U);
  EXPECT_NE(diagnostics::detail::load_diagnostics().find(
                "model=improved_nbc_b7 n=3 frontier=high source=[1,2,3] "
                "floating_checked=yes exact_checked=no"),
            std::string::npos);
}

TEST(ImprovedNbcB7Test,
     FloatingFilterOnlyProposesExactPositiveSemidefinitenessChecks) {
  EXPECT_TRUE(model::improved_nbc_b7_floating_psd_candidate_for_testing(
      symmetric_matrix(3, {2, 0, 0, 3, 0, 4}), {0, 1, 2}));
  EXPECT_TRUE(model::improved_nbc_b7_floating_psd_candidate_for_testing(
      symmetric_matrix(3, {5, -1, 2, 5, 2, 2}), {0, 1, 2}));
  EXPECT_FALSE(model::improved_nbc_b7_floating_psd_candidate_for_testing(
      symmetric_matrix(3, {1, 0, 0, -1, 0, 1}), {0, 1, 2}));
  EXPECT_FALSE(model::improved_nbc_b7_floating_psd_candidate_for_testing(
      symmetric_matrix(2, {0, 1, 0}), {0, 1}));
}

TEST(ImprovedNbcB7Test, FloatingFilterUsesTheSelectedSubmatrixScale) {
...
TEST(ImprovedNbcB7Test,
  ...
  EXPECT_NE(std::find(improved_nbc_b7_diagnostics::events.begin(),
                      improved_nbc_b7_diagnostics::events.end(),
                      improved_nbc_b7_diagnostics::event{"downward", 4}),
            improved_nbc_b7_diagnostics::events.end());
}

TEST(ImprovedNbcB7Test, BuildsAnExactDickinsonIntervalOnTheLowPath) {
  ...
  EXPECT_TRUE(model::improved_nbc_b7_certificate_for_testing(identity, {0},
                                                             lower, upper));
...
TEST(ImprovedNbcB7Test, RetainsTheExactHalfspaceRayOptimization) {
  const matrix_integer matrix =
  ...
  for (uint64_t mask = 1; mask < 16 && !improved; ++mask) {
    ...
    (void)model::improved_nbc_b7_check_support_for_testing(matrix, indices);
    improved =
        model::improved_nbc_b7_optimized_certificate_count_for_testing() > 0;
  }
  EXPECT_TRUE(improved);
}

TEST(ImprovedNbcB7Test, StoresDickinsonIntervalsInTheBufferedNbcState) {
  const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011},
                                                             {0b100, 0b110}};
  EXPECT_EQ(model::improved_nbc_b7_uncovered_count(3, 1, intervals), 1U);
  EXPECT_EQ(model::improved_nbc_b7_uncovered_count(3, 2, intervals), 1U);

  const std::vector<std::pair<uint64_t, uint64_t>> padded_network_interval{
      {0b00001, 0b01111}};
  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(5, 2, padded_network_interval),
      7U);

  const std::vector<std::pair<uint64_t, uint64_t>> singular_pair_interval{
      {0b00011, 0b01111}};
  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(5, 2, singular_pair_interval), 9U);
}

TEST(ImprovedNbcB7Test, EncodesUpwardDownwardAndExactSupportBlocks) {
  const std::vector<std::vector<size_t>> none;
  const std::vector<std::vector<size_t>> support_01{{0, 1}};

  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(3, 1, support_01, none, none), 3U);
  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(3, 2, support_01, none, none), 2U);
  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(3, 3, support_01, none, none), 0U);

  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(3, 1, none, support_01, none), 1U);
  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(3, 2, none, support_01, none), 2U);
  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(3, 3, none, support_01, none), 1U);

  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(3, 1, none, none, support_01), 3U);
  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(3, 2, none, none, support_01), 2U);
  EXPECT_EQ(
      model::improved_nbc_b7_uncovered_count(3, 3, none, none, support_01), 1U);
}

TEST(ImprovedNbcB7Test, PreservesExactCombinedClassifications) {
...
TEST(ImprovedNbcB7Test, DoesNotRepeatSupportsAcrossCompactedCardinalityLayers) {
...
TEST(ImprovedNbcB7Test, ExhaustivelyClassifiesSmallTwoByTwoIntegerMatrices) {
...
TEST(ImprovedNbcB7Test, MatchesTheIndependentExactThreeByThreeCriterion) {
...
TEST(ImprovedNbcB7Test, AppliesPairCurvaturePrepass) {
  ...
  EXPECT_EQ(model::improved_nbc_b7_pair_upward_count_for_testing(), 1U);
}
