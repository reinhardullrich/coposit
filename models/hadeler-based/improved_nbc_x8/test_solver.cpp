...
TEST(ImprovedNbcX8Test, ResumesOneModelCallsWithoutRepeatingSupports) {
...
TEST(ImprovedNbcX8Test, LatchesCollectiveRootInconsistencyAcrossCalls) {
...
TEST(ImprovedNbcX8Test, ExhaustivelyMatchesSingleIntervalCoverage) {
...
TEST(ImprovedNbcX8Test, ExhaustivelyMatchesIntervalsAddedBetweenModelCalls) {
...
TEST(ImprovedNbcX8Test, AlternatesFromOneLowSupportToTheNextOpenHighSupport) {
  improved_nbc_x8_diagnostics::clear();
  ...
  EXPECT_EQ(model::improved_nbc_x8_pair_upward_count_for_testing(), 0U);
  EXPECT_EQ(model::improved_nbc_x8_support_upward_count_for_testing(), 0U);
  EXPECT_EQ(model::improved_nbc_x8_downward_count_for_testing(), 1U);
  const auto low =
      std::find(improved_nbc_x8_diagnostics::events.begin(),
                improved_nbc_x8_diagnostics::events.end(),
                improved_nbc_x8_diagnostics::event{"process_low", 1});
  ASSERT_NE(low, improved_nbc_x8_diagnostics::events.end());
  const auto high = std::find_if(
      std::next(low), improved_nbc_x8_diagnostics::events.end(),
      [](const auto &event) {
        return event.name == "process_low" || event.name == "process_high";
      });
  ASSERT_NE(high, improved_nbc_x8_diagnostics::events.end());
  EXPECT_EQ(high->name, "process_high");
  EXPECT_NE(std::find(improved_nbc_x8_diagnostics::events.begin(),
                      improved_nbc_x8_diagnostics::events.end(),
                      improved_nbc_x8_diagnostics::event{"dickinson", 1}),
            improved_nbc_x8_diagnostics::events.end());
}

TEST(ImprovedNbcX8Test, RecordsChronologicalCertificatesForTheRenderer) {
  ...
  const size_t dickinson =
      history.find("event=certificate sequence=1 model=improved_nbc_x8 n=3 "
                   "frontier=low kind=dickinson source=[");
  const size_t downward =
      history.find("event=certificate sequence=2 model=improved_nbc_x8 n=3 "
...
TEST(ImprovedNbcX8Test, UsesTheFloatingFilterBeforeExactHighFrontierWork) {
  improved_nbc_x8_diagnostics::clear();
  std::ostringstream ignored_output;
  {
    diagnostics::reporter reporter(false, ignored_output, true, true);
  ...
  const auto high =
      std::find(improved_nbc_x8_diagnostics::events.begin(),
                improved_nbc_x8_diagnostics::events.end(),
                improved_nbc_x8_diagnostics::event{"process_high", 3});
  ASSERT_NE(high, improved_nbc_x8_diagnostics::events.end());
  ASSERT_NE(std::next(high), improved_nbc_x8_diagnostics::events.end());
  EXPECT_EQ(*std::next(high),
            (improved_nbc_x8_diagnostics::event{"high_float_reject", 3}));
  EXPECT_GT(model::improved_nbc_x8_high_float_rejection_count_for_testing(),
            0U);
  EXPECT_NE(diagnostics::detail::load_diagnostics().find(
                "model=improved_nbc_x8 n=3 frontier=high source=[1,2,3] "
                "floating_checked=yes exact_checked=no"),
            std::string::npos);
}

TEST(ImprovedNbcX8Test,
     FloatingFilterOnlyProposesExactPositiveSemidefinitenessChecks) {
  EXPECT_TRUE(model::improved_nbc_x8_floating_psd_candidate_for_testing(
      symmetric_matrix(3, {2, 0, 0, 3, 0, 4}), {0, 1, 2}));
  EXPECT_TRUE(model::improved_nbc_x8_floating_psd_candidate_for_testing(
      symmetric_matrix(3, {5, -1, 2, 5, 2, 2}), {0, 1, 2}));
  EXPECT_FALSE(model::improved_nbc_x8_floating_psd_candidate_for_testing(
      symmetric_matrix(3, {1, 0, 0, -1, 0, 1}), {0, 1, 2}));
  EXPECT_FALSE(model::improved_nbc_x8_floating_psd_candidate_for_testing(
      symmetric_matrix(2, {0, 1, 0}), {0, 1}));
}

TEST(ImprovedNbcX8Test, FloatingFilterUsesTheSelectedSubmatrixScale) {
...
TEST(ImprovedNbcX8Test,
  ...
  EXPECT_NE(std::find(improved_nbc_x8_diagnostics::events.begin(),
                      improved_nbc_x8_diagnostics::events.end(),
                      improved_nbc_x8_diagnostics::event{"downward", 4}),
            improved_nbc_x8_diagnostics::events.end());
}

TEST(ImprovedNbcX8Test, BuildsAnExactDickinsonIntervalOnTheLowPath) {
  ...
  EXPECT_TRUE(model::improved_nbc_x8_certificate_for_testing(identity, {0},
                                                             lower, upper));
...
TEST(ImprovedNbcX8Test, SkipsTheMilpWhenAllOnesAlreadyReachesTheCeiling) {
  matrix_integer identity;
  identity.set_identity(3);
  support_context context(3);
  support lower = context.make();
  ...
  ASSERT_TRUE(model::improved_nbc_x8_certificate_for_testing(identity, {0},
                                                             lower, upper));
  EXPECT_EQ(model::improved_nbc_x8_root_relaxation_attempt_count_for_testing(),
            0U);
  for (size_t index = 0; index < 3; ++index)
    EXPECT_TRUE(context.contains(upper, index));
}

TEST(ImprovedNbcX8Test, AcceptsAnExactlyBetterRootRelaxationCertificate) {
  const matrix_integer matrix =
  ...
  for (uint64_t mask = 1; mask < 16 && !improved; ++mask) {
    ...
    (void)model::improved_nbc_x8_check_support_for_testing(matrix, indices);
    improved =
        model::improved_nbc_x8_root_relaxation_improvement_count_for_testing() >
        0;
  }
  EXPECT_TRUE(improved);
  EXPECT_EQ(model::improved_nbc_x8_root_relaxation_node_count_for_testing(),
            model::improved_nbc_x8_root_relaxation_attempt_count_for_testing());
}

TEST(ImprovedNbcX8Test, CanImproveWithoutFindingTheIntegerBoundaryOptimum) {
  const matrix_integer matrix =
      symmetric_matrix(3, {1, 0, 1, 1, -2, 1});
  support_context context(3);
  support lower = context.make();
  support upper = context.make();

  ASSERT_TRUE(model::improved_nbc_x8_certificate_for_testing(
      matrix, {0, 1}, lower, upper));
  EXPECT_TRUE(context.contains(lower, 0));
...
TEST(ImprovedNbcX8Test, StoresDickinsonIntervalsInTheBufferedNbcState) {
  const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011},
                                                             {0b100, 0b110}};
  EXPECT_EQ(model::improved_nbc_x8_uncovered_count(3, 1, intervals), 1U);
  EXPECT_EQ(model::improved_nbc_x8_uncovered_count(3, 2, intervals), 1U);

  const std::vector<std::pair<uint64_t, uint64_t>> padded_network_interval{
      {0b00001, 0b01111}};
  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(5, 2, padded_network_interval),
      7U);

  const std::vector<std::pair<uint64_t, uint64_t>> singular_pair_interval{
      {0b00011, 0b01111}};
  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(5, 2, singular_pair_interval), 9U);
}

TEST(ImprovedNbcX8Test, EncodesUpwardDownwardAndExactSupportBlocks) {
  const std::vector<std::vector<size_t>> none;
  const std::vector<std::vector<size_t>> support_01{{0, 1}};

  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(3, 1, support_01, none, none), 3U);
  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(3, 2, support_01, none, none), 2U);
  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(3, 3, support_01, none, none), 0U);

  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(3, 1, none, support_01, none), 1U);
  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(3, 2, none, support_01, none), 2U);
  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(3, 3, none, support_01, none), 1U);

  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(3, 1, none, none, support_01), 3U);
  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(3, 2, none, none, support_01), 2U);
  EXPECT_EQ(
      model::improved_nbc_x8_uncovered_count(3, 3, none, none, support_01), 1U);
}

TEST(ImprovedNbcX8Test, PreservesExactCombinedClassifications) {
...
TEST(ImprovedNbcX8Test, DoesNotRepeatSupportsAcrossCompactedCardinalityLayers) {
...
TEST(ImprovedNbcX8Test, ExhaustivelyClassifiesSmallTwoByTwoIntegerMatrices) {
...
TEST(ImprovedNbcX8Test, MatchesTheIndependentExactThreeByThreeCriterion) {
...
TEST(ImprovedNbcX8Test, AppliesPairCurvaturePrepass) {
  ...
  EXPECT_EQ(model::improved_nbc_x8_pair_upward_count_for_testing(), 1U);
}
