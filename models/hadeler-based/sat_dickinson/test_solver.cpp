...
TEST(SatDickinsonTest, PreservesStrictDickinsonDecisions)
{
...
TEST(SatDickinsonTest, DistinguishesOrdinaryFromStrictCopositivity)
{
...
TEST(SatDickinsonTest, ClassifiesBothPredicatesInOneTraversal)
{
...
TEST(SatDickinsonTest, SubtractsTheUnionOfBoundedIntervals)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011}, {0b100, 0b110}};
    EXPECT_EQ(model::sat_uncovered_count(3, 1, intervals), 1U);
    EXPECT_EQ(model::sat_uncovered_count(3, 2, intervals), 1U);
}

TEST(SatDickinsonTest, StoresOneBlockingClausePerInterval)
{
...
TEST(SatDickinsonTest, RepresentsAWholeDownsetWithAnEmptyLowerEndpoint)
{
    ...
    EXPECT_EQ(model::sat_uncovered_count(3, 1, downset), 1U);
    EXPECT_EQ(model::sat_uncovered_count(3, 2, downset), 2U);
    EXPECT_EQ(model::sat_uncovered_count(3, 3, downset), 1U);
}

TEST(SatDickinsonTest, MatchesBruteForceForEveryPairOfFourDimensionalIntervals)
{
    ...
    for (const auto& first : valid_intervals) {
        for (const auto& second : valid_intervals) {
            ...
            for (size_t cardinality = 1; cardinality <= dimension; ++cardinality)
                EXPECT_EQ(model::sat_uncovered_count(dimension, cardinality, intervals),
                          brute_uncovered_count(dimension, cardinality, intervals));
        }
    }
}

TEST(SatDickinsonTest, PublishesCertificateFreeIndexAndUpperSizeDistributionOnlyWithDiagnostics)
{
...
TEST(SatDickinsonTest, CooperativeTimeoutInterruptsSatSolving)
{
    request_timeout();
    EXPECT_THROW(model::sat_uncovered_count(15, 7, {}), timeout_requested);
    reset_timeout();
}
