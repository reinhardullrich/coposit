...
class timeout_post final : public Clasp::PostPropagator {
public:
    Clasp::uint32 priority() const override { return priority_reserved_msg; }

    bool propagateFixpoint(Clasp::Solver& solver, Clasp::PostPropagator*) override
    {
        if (!timeout_pending()) return true;
        solver.setStopConflict();
        return false;
    }
};

class timeout_configurator final : public Clasp::ClaspConfig::Configurator {
public:
    bool applyConfig(Clasp::Solver& solver) override { return solver.addPost(new timeout_post); }
};

class clasp_frontier {
public:
    void initialize(int variable_count, const std::vector<std::vector<int>>& clauses)
    {
        config_.solve.numModels = 0;
        config_.solve.enumMode = Clasp::EnumOptions::enum_bt;
        config_.addSolver(0).restartOnModel = 1;
        config_.addSearch(0).reduce.disable();
        config_.addConfigurator(new timeout_configurator, Clasp::Ownership_t::Acquire, true);

        Clasp::SatBuilder& builder = facade_.startSat(config_);
        facade_.ctx.setSolveMode(Clasp::SharedContext::solve_multi);
        builder.prepareProblem(static_cast<Clasp::uint32>(variable_count), 0, static_cast<Clasp::uint32>(clauses.size()));
        for (const auto& clause : clauses) {
            Clasp::LitVec literals = clasp_literals(clause);
            if (!builder.addClause(literals)) throw std::runtime_error("clasp rejected the initial support formula");
        }
        facade_.prepare(Clasp::ClaspFacade::enum_volatile);
    }

    bool take(size_t cardinality, const std::vector<int>& cardinality_outputs, std::vector<size_t>& indices, size_t dimension)
    {
        if (!handle_ || active_cardinality_ != cardinality) {
            handle_.reset();
            Clasp::LitVec assumptions;
            assumptions.push_back(Clasp::posLit(static_cast<Clasp::Var>(cardinality_outputs[cardinality - 1])));
            if (cardinality < dimension)
                assumptions.push_back(Clasp::negLit(static_cast<Clasp::Var>(cardinality_outputs[cardinality])));
            handle_ = std::make_unique<Clasp::ClaspFacade::SolveHandle>(
                facade_.solve(Clasp::SolveMode_t::Yield, assumptions));
            active_cardinality_ = cardinality;
        }

        if (!handle_->next()) {
            const Clasp::SolveResult result = handle_->get();
            handle_.reset();
            timeout_checkpoint();
            if (!result.exhausted()) throw std::runtime_error("clasp stopped without exhausting the support layer");
            return false;
        }

        const Clasp::Model* model = handle_->model();
        if (!model) throw std::runtime_error("clasp yielded no model after reporting one");
        indices.clear();
        for (size_t index = 0; index < dimension; ++index)
            if (model->isTrue(Clasp::posLit(static_cast<Clasp::Var>(index + 1)))) indices.push_back(index);
        if (indices.size() != cardinality) throw std::runtime_error("clasp violated the exact-cardinality assumptions");
        return true;
    }
    ...
    {
        Clasp::LitVec literals = clasp_literals(clause);
        Clasp::ConstraintInfo info(Clasp::Constraint_t::Conflict);
        info.setActivity(Clasp::ACT_MAX).setLbd(1);
        static_cast<void>(Clasp::ClauseCreator::create(
            *facade_.ctx.master(), literals, Clasp::ClauseCreator::clause_force_simplify, info));
    }

private:
    static Clasp::LitVec clasp_literals(const std::vector<int>& clause)
    {
        Clasp::LitVec result;
        result.reserve(clause.size());
        for (const int literal : clause) {
            const auto variable = static_cast<Clasp::Var>(std::abs(literal));
            result.push_back(literal < 0 ? Clasp::negLit(variable) : Clasp::posLit(variable));
        }
        return result;
    }
...
public:
        ...
        : dimension_(dimension)
    {
        if (dimension_ > static_cast<size_t>(std::numeric_limits<int>::max() - 1))
            throw std::overflow_error("Clasp variable count exceeds its integer literal range");
        ...
        low_frontier_.initialize(next_variable_ - 1, base_clauses_);
        high_frontier_.initialize(next_variable_ - 1, base_clauses_);
    }
    ...
    bool take_first(std::vector<size_t>& indices, bool high_frontier = false)
    {
        ...
        return (high_frontier ? high_frontier_ : low_frontier_)
            .take(cardinality_, cardinality_outputs_, indices, dimension_);
    }
    ...
    void add_interval(const support& lower, const support& upper)
    {
        ...
        for (size_t index = 0; index < dimension_; ++index) {
            ...
            if (lower.contains(index)) {
                clause.push_back(-variable(index));
#ifdef COPOSIT_SAT_B3_TESTING
        ...
        if (upper_size < dimension_) {
            clause.push_back(cardinality_outputs_[upper_size]);
#ifdef COPOSIT_SAT_B3_TESTING
            ++last_interval_clause_size_;
#endif
        }
    ...
    {
        add_clause({-variable(first), -variable(second)});
        ++interval_count_;
    }
    ...
    {
        std::vector<int> clause;
        clause.reserve(indices.size());
        for (const size_t index : indices) clause.push_back(-variable(index));
        add_clause(clause);
        ++interval_count_;
    }
    ...
    void add_downward_closure(const std::vector<size_t>& indices)
    {
        ...
        for (size_t index = 0; index < dimension_; ++index) {
            ...
            else clause.push_back(variable(index));
        }
        add_clause(clause);
    ...
    {
        std::vector<int> clause;
        clause.reserve(indices.size() + 1);
        for (const size_t index : indices) clause.push_back(-variable(index));
        if (indices.size() < dimension_) clause.push_back(cardinality_outputs_[indices.size()]);
        add_clause(clause);
        ++interval_count_;
    }
    ...
    void reject_from_high_frontier(const std::vector<size_t>& indices)
    {
        ...
        for (const size_t index : indices) clause.push_back(-variable(index));
        if (indices.size() < dimension_) clause.push_back(cardinality_outputs_[indices.size()]);
        high_frontier_.add_clause(clause);
    }
...
private:
    ...
    int new_variable()
    {
        if (next_variable_ == std::numeric_limits<int>::max())
            throw std::overflow_error("Clasp cardinality network exceeds its integer literal range");
    ...
    {
        base_clauses_.emplace_back(literals);
    }

    void add_clause(const std::vector<int>& literals)
    {
        low_frontier_.add_clause(literals);
        high_frontier_.add_clause(literals);
    }
...
#endif
    clasp_frontier low_frontier_;
    clasp_frontier high_frontier_;
...
private:
    ...
    bool process_low_support(const matrix_integer& matrix, size_t& low, size_t high)
    {
        while (low <= high) {
            ...
            COPOSIT_SAT_B3_DIAGNOSTICS("stage_low", low);
    ...
    bool process_high_support(const matrix_integer& matrix, size_t low, size_t& high)
    {
        while (low <= high) {
            ...
            COPOSIT_SAT_B3_DIAGNOSTICS("stage_high", high);
...
#endif
            COPOSIT_SAT_B3_DIAGNOSTICS("combined_ray", ray + 1);
...
#ifdef COPOSIT_SAT_B3_TESTING
bool sat_b3_prefers_negative_singular_orientation_for_testing(size_t positive_products,
                                                               size_t negative_products) noexcept
{
    return negative_orientation_has_larger_upper(positive_products, negative_products);
}

size_t sat_b3_optimized_certificate_count_for_testing() noexcept
{
    return last_optimized_certificate_count;
}

size_t sat_b3_combined_ray_sweep_count_for_testing() noexcept
{
    return last_combined_ray_sweep_count;
}

size_t sat_b3_combined_ray_improvement_count_for_testing() noexcept
{
    return last_combined_ray_improvement_count;
}

size_t sat_b3_pair_curvature_exclusion_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t sat_b3_support_curvature_exclusion_count_for_testing() noexcept
{
    return last_support_curvature_exclusion_count;
}

size_t sat_b3_pair_upward_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t sat_b3_support_upward_count_for_testing() noexcept
{
    return last_support_curvature_exclusion_count;
}

size_t sat_b3_downward_count_for_testing() noexcept
{
    return last_downward_count;
}

size_t sat_b3_exact_count_for_testing() noexcept
{
    return last_exact_count;
}

size_t sat_b3_high_float_rejection_count_for_testing() noexcept
{
    return last_high_float_rejection_count;
}

bool sat_b3_floating_psd_candidate_for_testing(const matrix_integer& matrix, const std::vector<size_t>& indices)
{
...
bool sat_b3_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer& matrix, const std::vector<size_t>& indices)
{
...
size_t sat_b3_shortlist_limit_for_testing(size_t matrix_dimension, size_t support_dimension)
{
    return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool sat_b3_prefers_ray_candidate_for_testing(size_t candidate_upper, size_t candidate_width, size_t candidate_gains,
...
bool sat_b3_check_support_for_testing(const matrix_integer& matrix, const std::vector<size_t>& indices)
{
    return dickinson_checker(matrix.rows(), copositivity_mode::strictly_copositive).check_support_for_testing(matrix, indices);
}

bool sat_b3_certificate_for_testing(
    const matrix_integer& matrix, const std::vector<size_t>& indices, support& lower, support& upper)
{
    return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
...
size_t sat_b3_fixed_support_upper_size_for_testing() noexcept
{
    return last_fixed_support_upper_size;
}

size_t sat_b3_uncovered_count(
...
size_t clasp_b3_native_enumeration_count(size_t dimension, size_t cardinality)
{
    interval_sat supports(dimension);
    supports.start_cardinality(cardinality);
    std::vector<size_t> indices;
    size_t count = 0;
    while (supports.take_first(indices)) ++count;
    return count;
}

size_t sat_b3_pair_exclusion_uncovered_count(
...
size_t sat_b3_high_filter_uncovered_count(
...
size_t sat_b3_interval_clause_size(size_t dimension, uint64_t lower_mask, uint64_t upper_mask)
{
    interval_sat diagram(dimension);
    support lower(dimension);
    support upper(dimension);
