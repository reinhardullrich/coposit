...
Classification: coposit-created exact CP/SCP experiment. Clasp-B3 alternates between low and high cardinalities. The low frontier uses
...
Clasp-B3 uses that observation as a search certificate.
...
The remaining supports are represented by two direct `libclasp` model enumerators, one for each frontier. Two cardinality frontiers start at $1$ and $n$. The model
...
The identifier is `clasp_b3`.

- **Clasp** names the direct low-level `libclasp` Boolean engine. The model does not invoke Clingo and does not ground an ASP program.
- **B3** identifies the unchanged mathematical pruning and low/high traversal inherited from SAT-B3.

The model is an independent experiment copied from [`sat_b3`](../sat_b3/ALGORITHM.md). Its curvature rules, exact arithmetic,
Halfspace-Rays certificates, high-frontier filter, and alternating traversal are unchanged. Only the Boolean engine is replaced.
Each frontier uses Clasp's restricted-backtracking model enumerator, so successive supports of one cardinality are generated without
adding an exact-model blocking clause. Clasp's native restart-on-model option returns the search to its root after every yielded
support while retaining learnt and pruning clauses; the restricted-backtracking enumerator still determines the next branch.
Genuine pruning clauses are sent to both frontier solvers; high-filter-only rejections are
sent only to the high solver. Learnt-clause deletion is disabled so these dynamically inserted exact clauses remain active. The
sorting network itself is inherited from [`sat_b3`](../sat_b3/ALGORITHM.md). Dickinson intervals come from Peter J. C.
...
Using these curvature facts as Boolean clauses, alternating the cardinality order, and using the search as a complete exact CP/SCP
classifier are coposit experiments rather than algorithms from those papers. Clasp 3.4.1, obtained through the Clingo 5.8.2 source
dependency, supplies direct SAT model enumeration; no Clingo control, backend, grounding, or ASP representation is used.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Clasp-B3 solves
...
When low-frontier curvature does not already remove the upward closure, Clasp-B3 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Clasp-B3 performs the inherited exact
...
## Boolean Clauses And Direct Clasp Enumeration
...
sorting network supplies exact-cardinality assumptions for any requested layer. The low and high frontiers keep separate paused
Clasp enumerations so that the algorithm can alternate one support at a time. Clasp's backtracking enumerator remembers the explored
part of the active cardinality layer without recording one extra clause per returned support.
...
Clasp-B3 adds one singular case. Suppose
...
matrix. The rejection clause at $k=|I|$ is sent only to the high-frontier Clasp instance:

$$
\left(\bigvee_{i\in I}\neg s_i\right)\lor c_{k+1},
$$

where $c_{k+1}$ is the sorting-network output meaning that at least $k+1$ indices are selected. At cardinality $k$, $c_{k+1}$ is
false, so the clause skips exactly $I$ in future high scans. The low-frontier instance never receives that clause. Thus the support
remains available to the exact low-frontier proof.

## Complete Decision Flow

1. Build identical direct-Clasp instances for the low and high frontiers, each with the same exact-cardinality sorting network.
2. Install every failed pair-curvature clause.
3. Start a low frontier at cardinality $1$ and a high frontier at cardinality $n$.
4. Ask the low Clasp enumerator for one unresolved low-frontier support. Advance across empty low layers, but do not exhaust a nonempty layer.
...
6. Ask the high Clasp enumerator for one unresolved high-frontier support. Descend across high layers after every support
...
Runtime diagnostics report the current low or high cardinality, selected and processed supports, installed Boolean exclusions, and the
...
Clasp-B3 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
...
would have removed those supports. Boolean clauses remain compact, but a large family of high-only rejections can still make Clasp search
expensive.

Even with restart-on-model enabled, Clasp's retained heuristic state can select supports whose Dickinson intervals overlap heavily.
Such an order can install many low-value certificates before reaching a support that would prune the region broadly. The direct
Clasp interface can therefore have low Boolean overhead per returned support while still taking a worse proof path than another SAT
heuristic.
