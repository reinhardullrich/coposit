...
TEST(SatB3Test, AlternatesIndividualLowAndHighSupportsWithDirectionalPruning)
{
    ...
    EXPECT_EQ(model::sat_b3_pair_upward_count_for_testing(), 0U);
    EXPECT_EQ(model::sat_b3_support_upward_count_for_testing(), 0U);
    EXPECT_EQ(model::sat_b3_downward_count_for_testing(), 1U);
    EXPECT_EQ(model::sat_b3_exact_count_for_testing(), 0U);
    EXPECT_NE(std::find(sat_b3_diagnostics::events.begin(), sat_b3_diagnostics::events.end(),
                       sat_b3_diagnostics::event{"dickinson", 1}),
...
TEST(SatB3Test, UsesTheFloatingFilterBeforeExactHighFrontierWork)
{
    ...
    ASSERT_NE(high, sat_b3_diagnostics::events.end());
...
{
    EXPECT_TRUE(model::sat_b3_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {2, 0, 0, 3, 0, 4}), {0, 1, 2}));
    EXPECT_TRUE(model::sat_b3_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {5, -1, 2, 5, 2, 2}), {0, 1, 2}));
    EXPECT_FALSE(model::sat_b3_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {1, 0, 0, -1, 0, 1}), {0, 1, 2}));
    EXPECT_FALSE(model::sat_b3_floating_psd_candidate_for_testing(
        symmetric_matrix(2, {0, 1, 0}), {0, 1}));
}

TEST(SatB3Test, FloatingFilterUsesTheSelectedSubmatrixScale)
{
    const matrix_integer matrix = symmetric_matrix(3, {1'000'000'000'000'000'000L, 0, 0, 1, 0, 1});
    EXPECT_TRUE(model::sat_b3_floating_psd_candidate_for_testing(matrix, {1, 2}));
}

TEST(SatB3Test, UsesConsistentAllOnesSystemToPruneSingularPsdSupportDownward)
{
    sat_b3_diagnostics::clear();
    ...
    EXPECT_EQ(model::sat_b3_downward_count_for_testing(), 1U);
    EXPECT_NE(std::find(sat_b3_diagnostics::events.begin(), sat_b3_diagnostics::events.end(),
                       sat_b3_diagnostics::event{"downward", 4}),
              sat_b3_diagnostics::events.end());
}

TEST(SatB3Test, FloatingRejectionsRemainVisibleToTheExactLowFrontier)
{
    const std::vector<std::vector<size_t>> rejected{{0, 1}};
    EXPECT_EQ(model::sat_b3_high_filter_uncovered_count(3, 2, rejected, true), 2U);
    EXPECT_EQ(model::sat_b3_high_filter_uncovered_count(3, 2, rejected, false), 3U);
}

TEST(SatB3Test, BuildsAnExactDickinsonIntervalOnTheLowPath)
{
    ...
    EXPECT_TRUE(model::sat_b3_certificate_for_testing(identity, {0}, lower, upper));
...
TEST(SatB3Test, RetainsTheExactHalfspaceRayOptimization)
{
    ...
    for (uint64_t mask = 1; mask < 16 && !improved; ++mask) {
        ...
        (void)model::sat_b3_check_support_for_testing(matrix, indices);
        improved = model::sat_b3_optimized_certificate_count_for_testing() > 0;
    }
    EXPECT_TRUE(improved);
}

TEST(SatB3Test, StoresDickinsonIntervalsInTheSharedSatState)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011}, {0b100, 0b110}};
    EXPECT_EQ(model::sat_b3_uncovered_count(3, 1, intervals), 1U);
    EXPECT_EQ(model::sat_b3_uncovered_count(3, 2, intervals), 1U);
}

TEST(SatB3Test, EnumeratesOneCardinalityWithoutExactSupportClauses)
{
    EXPECT_EQ(model::clasp_b3_native_enumeration_count(6, 3), 20U);
}

TEST(SatB3Test, EncodesUpwardDownwardAndExactSupportBlocks)
{
    const std::vector<std::vector<size_t>> none;
    const std::vector<std::vector<size_t>> support_01{{0, 1}};

    EXPECT_EQ(model::sat_b3_uncovered_count(3, 1, support_01, none, none), 3U);
    EXPECT_EQ(model::sat_b3_uncovered_count(3, 2, support_01, none, none), 2U);
    EXPECT_EQ(model::sat_b3_uncovered_count(3, 3, support_01, none, none), 0U);

    EXPECT_EQ(model::sat_b3_uncovered_count(3, 1, none, support_01, none), 1U);
    EXPECT_EQ(model::sat_b3_uncovered_count(3, 2, none, support_01, none), 2U);
    EXPECT_EQ(model::sat_b3_uncovered_count(3, 3, none, support_01, none), 1U);

    EXPECT_EQ(model::sat_b3_uncovered_count(3, 1, none, none, support_01), 3U);
    EXPECT_EQ(model::sat_b3_uncovered_count(3, 2, none, none, support_01), 2U);
    EXPECT_EQ(model::sat_b3_uncovered_count(3, 3, none, none, support_01), 1U);
}

TEST(SatB3Test, PreservesExactCombinedClassifications)
{
...
TEST(SatB3Test, ExhaustivelyClassifiesSmallTwoByTwoIntegerMatrices)
{
...
TEST(SatB3Test, MatchesTheIndependentExactThreeByThreeCriterion)
{
...
TEST(SatB3Test, AppliesPairCurvaturePrepass)
{
    ...
    EXPECT_EQ(model::sat_b3_pair_upward_count_for_testing(), 1U);
}
