...
class cardinality_bdd {
public:
    cardinality_bdd(size_t dimension, size_t cardinality)
...
public:
    ...
    bool check(const matrix_integer& matrix)
    {
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            cardinality_bdd supports(matrix.rows(), subset_dimension);
            ...
            while (supports.take_first(indices_)) {
                ...
                COPOSIT_CARDINALITY_BDD_DIAGNOSTICS("process", subset_dimension);
...
private:
    bool process_subset(const matrix_integer& matrix, cardinality_bdd& supports)
    {
    ...
    void add_certificate(const matrix_integer& matrix, cardinality_bdd& supports)
    {
...
#ifdef COPOSIT_CARDINALITY_BDD_DICKINSON_TESTING
...
{
    cardinality_bdd diagram(dimension, cardinality);
