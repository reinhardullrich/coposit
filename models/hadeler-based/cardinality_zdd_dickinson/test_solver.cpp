...
TEST(CardinalityZddDickinsonTest, PreservesStrictDickinsonDecisions)
{
...
TEST(CardinalityZddDickinsonTest, ClassifiesCopositiveBoundary)
{
...
TEST(CardinalityZddDickinsonTest, SubtractsTheUnionOfBoundedIntervals)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011}, {0b100, 0b110}};
    EXPECT_EQ(model::cardinality_zdd_uncovered_count(3, 1, intervals).first, 1U);
    EXPECT_EQ(model::cardinality_zdd_uncovered_count(3, 2, intervals).first, 1U);
}

TEST(CardinalityZddDickinsonTest, MatchesBruteForceForEveryPairOfFourDimensionalIntervals)
{
    ...
    for (const auto& first : valid_intervals) {
        for (const auto& second : valid_intervals) {
            ...
            for (size_t cardinality = 1; cardinality <= dimension; ++cardinality)
                EXPECT_EQ(model::cardinality_zdd_uncovered_count(dimension, cardinality, intervals).first,
                          brute_uncovered_count(dimension, cardinality, intervals));
        }
    }
}

TEST(CardinalityZddDickinsonTest, DoesNotEmitSupportsAlreadyCoveredByIdentityCertificates)
{
    matrix_integer identity;
    identity.set_identity(4);
    cardinality_zdd_dickinson_diagnostics::clear();
    EXPECT_TRUE(model::solve(identity));

    ASSERT_EQ(cardinality_zdd_dickinson_diagnostics::events.size(), 4U);
    for (const auto& event : cardinality_zdd_dickinson_diagnostics::events)
        EXPECT_EQ(event, (cardinality_zdd_dickinson_diagnostics::event{"process", 1}));
}

TEST(CardinalityZddDickinsonTest, KeepsPackedSupportsBeyondOneWord)
{
...
TEST(CardinalityZddDickinsonTest, CooperativeTimeoutEscapesDiagramOperations)
{
    request_timeout();
    EXPECT_THROW(model::cardinality_zdd_uncovered_count(15, 7, {}), timeout_requested);
    reset_timeout();
}
