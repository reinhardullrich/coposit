...
size_t last_targeted_lp_attempt_count = 0;
size_t last_targeted_lp_feasible_count = 0;
size_t last_targeted_lp_exact_candidate_count = 0;
size_t last_targeted_lp_accepted_count = 0;
size_t last_targeted_lp_prepare_count = 0;
size_t last_targeted_lp_two_candidate_round_count = 0;
size_t last_targeted_lp_later_candidate_selected_count = 0;
bool last_traditional_interval_available = false;
size_t last_traditional_lower_size = 0;
size_t last_traditional_upper_size = 0;
std::uint64_t last_traditional_interval_ns = 0;
std::uint64_t last_x7_interval_ns = 0;
#endif
...
public:
  ...
  bool check(const matrix_integer &matrix) {
    ...
    targeted_lp_attempt_count_ = 0;
    targeted_lp_feasible_count_ = 0;
    targeted_lp_exact_candidate_count_ = 0;
    targeted_lp_accepted_count_ = 0;
    targeted_lp_prepare_count_ = 0;
    targeted_lp_two_candidate_round_count_ = 0;
    targeted_lp_later_candidate_selected_count_ = 0;
#endif
  ...
  bool optimize_support_for_testing(const matrix_integer &matrix,
    ...
    targeted_lp_two_candidate_round_count_ = 0;
    targeted_lp_later_candidate_selected_count_ = 0;
    last_traditional_interval_available = false;
    last_traditional_lower_size = 0;
    last_traditional_upper_size = 0;
    last_traditional_interval_ns = 0;
    last_x7_interval_ns = 0;
    measure_support_for_testing_ = true;
    test_support_started_ = std::chrono::steady_clock::now();
    indices_ = indices;
    captured_lower_ = &lower;
    captured_upper_ = &upper;
    const bool result = process_subset(matrix);
    last_x7_interval_ns = static_cast<std::uint64_t>(
        std::chrono::duration_cast<std::chrono::nanoseconds>(
            std::chrono::steady_clock::now() - test_support_started_)
            .count());
    measure_support_for_testing_ = false;
...
private:
  ...
  void record_interval_certificate(const support &lower, const support &upper) {
    ...
    event << "model=improved_nbc_x7 n=" << product_.size()
  ...
  bool process_low_support(const matrix_integer &matrix, size_t &low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X7_DIAGNOSTICS("stage_low", low);
  ...
  bool process_high_support(const matrix_integer &matrix, size_t low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X7_DIAGNOSTICS("stage_high", high);
  ...
  bool process_selected_support(const matrix_integer &matrix,
    ...
    COPOSIT_IMPROVED_NBC_X7_DIAGNOSTICS(
  ...
  void install_pair_curvature_exclusions(const matrix_integer &matrix) {
    ...
    for (size_t first = 0; first < matrix.rows(); ++first) {
      ...
      for (size_t second = first + 1; second < matrix.rows(); ++second) {
        ...
        COPOSIT_IMPROVED_NBC_X7_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_IMPROVED_NBC_X7_TESTING
  ...
  bool process_subset(const matrix_integer &matrix,
    ...
    if (!from_low_frontier) {
      ...
      if (!floating_filter_.looks_positive_semidefinite(indices_)) {
        ...
        COPOSIT_IMPROVED_NBC_X7_DIAGNOSTICS("high_float_reject",
                                            indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X7_TESTING
  ...
  bool process_nonsingular_subset(const matrix_integer &matrix) {
    ...
    rays_product_ = product_;
    prepare_targeted_lp_coefficients();
    bool lp_improved = false;
    while (has_targeted_lp_problem()) {
      const exact_lp_candidate_result lp_result = try_targeted_enlargement();
      if (lp_result == exact_lp_candidate_result::negative_witness)
        return false;
      if (lp_result == exact_lp_candidate_result::unchanged)
        break;
      lp_improved = true;
    }
    if (!lp_improved) {
    ...
    shrink_lower_preserving_upper(matrix);
    targeted_solution_ = solution_;
    targeted_product_ = product_;

    solution_ = rays_solution_;
    product_ = rays_product_;
    current_score_ = score(solution_, 0, product_);
    shrink_lower_preserving_upper(matrix);
    if (interval_strictly_dominates(solution_, product_, targeted_solution_,
                                    targeted_product_)) {
  ...
  enum class exact_lp_candidate_result {
    unchanged,
    improved,
    negative_witness
  };

  bool has_targeted_lp_problem() const noexcept {
    ...
    if (outside_dimension == 0 ||
        !lp_problem_fits(support_dimension, outside_dimension))
      return false;
  ...
  exact_lp_candidate_result try_targeted_enlargement() {
    timeout_checkpoint();
    const size_t support_dimension = indices_.size();
    const size_t outside_dimension = product_.size() - support_dimension;
    assert(lp_outside_rows_.size() == outside_dimension);
    assert(lp_scaled_rows_.size() == outside_dimension * support_dimension);

    lp_required_rows_.clear();
    for (size_t row = 0; row < outside_dimension; ++row)
      if (product_[lp_outside_rows_[row]].sign() >= 0)
        lp_required_rows_.push_back(row);

    const size_t preserved_count = lp_required_rows_.size();
    constexpr size_t candidate_limit = 2;
    size_t exact_candidate_count = 0;
    bool best_candidate_found = false;
    bool later_candidate_selected = false;
    coverage_score best_candidate_score{};
    for (size_t target = 0; target < outside_dimension; ++target) {
      timeout_checkpoint();
      if (product_[lp_outside_rows_[target]].sign() >= 0)
        continue;
      lp_required_rows_.resize(preserved_count);
      lp_required_rows_.push_back(target);
      const double positive_floor =
          std::min(1e-7, 0.5 / static_cast<double>(support_dimension));
      const double simplex_scale =
          1.0 - positive_floor * static_cast<double>(support_dimension);
      const size_t reduced_dimension = support_dimension - 1;
      const size_t margin_column = reduced_dimension;
      detail::tiny_simplex lp(lp_required_rows_.size() + 1, support_dimension);
      size_t constraint = 0;
      for (const size_t required : lp_required_rows_) {
        const double *const coefficients =
            lp_scaled_rows_.data() + required * support_dimension;
        double sum = 0.0;
        for (size_t column = 0; column < support_dimension; ++column)
          sum += coefficients[column];
        const double eliminated = coefficients[support_dimension - 1];
        for (size_t column = 0; column < reduced_dimension; ++column) {
          lp.set_coefficient(constraint, column,
                             -simplex_scale *
                                 (coefficients[column] - eliminated));
        }
        lp.set_coefficient(constraint, margin_column, 1.0);
        lp.set_rhs(constraint,
                   positive_floor * sum + simplex_scale * eliminated);
        ++constraint;
      }
      for (size_t column = 0; column < reduced_dimension; ++column)
        lp.set_coefficient(constraint, column, 1.0);
      lp.set_rhs(constraint, 1.0);
      lp.set_objective(margin_column, 1.0);

      const detail::tiny_simplex::status status = lp.solve(lp_proposal_);
#ifdef COPOSIT_IMPROVED_NBC_X7_TESTING
      ++targeted_lp_attempt_count_;
#endif
      if (status != detail::tiny_simplex::status::optimal ||
          lp_proposal_[margin_column] < -1e-9)
        continue;
#ifdef COPOSIT_IMPROVED_NBC_X7_TESTING
      ...
      for (size_t column = 0; column < reduced_dimension; ++column) {
        simplex_sum += lp_proposal_[column];
        lp_proposal_[column] =
            positive_floor + simplex_scale * lp_proposal_[column];
      }
      lp_proposal_[reduced_dimension] =
          positive_floor + simplex_scale * (1.0 - simplex_sum);
      coverage_score candidate_score{};
      const exact_lp_candidate_result result =
          consider_lp_point(lp_proposal_, lp_required_rows_, candidate_score);
      if (result == exact_lp_candidate_result::negative_witness)
        return result;
      if (result == exact_lp_candidate_result::unchanged)
        continue;

      ++exact_candidate_count;
      if (!best_candidate_found ||
          better_score(candidate_score, best_candidate_score)) {
        later_candidate_selected = best_candidate_found;
        lp_best_solution_.swap(lp_solution_);
        lp_best_product_.swap(lp_product_);
        best_candidate_score = candidate_score;
        best_candidate_found = true;
      }
      if (exact_candidate_count == candidate_limit)
        break;
    }
    if (!best_candidate_found)
      return exact_lp_candidate_result::unchanged;

    solution_.swap(lp_best_solution_);
    product_.swap(lp_best_product_);
    current_score_ = best_candidate_score;
#ifdef COPOSIT_IMPROVED_NBC_X7_TESTING
    if (exact_candidate_count == candidate_limit)
      ++targeted_lp_two_candidate_round_count_;
    if (later_candidate_selected)
      ++targeted_lp_later_candidate_selected_count_;
#endif
    return exact_lp_candidate_result::improved;
  }

  void prepare_targeted_lp_coefficients() {
    const size_t support_dimension = indices_.size();
    const size_t outside_dimension = product_.size() - support_dimension;
    lp_outside_rows_.clear();
    if (outside_dimension > lp_scaled_rows_.max_size() / support_dimension)
      throw std::overflow_error("targeted LP coefficient matrix is too large");
    lp_scaled_rows_.resize(outside_dimension * support_dimension);
    lp_exponents_.resize(support_dimension);
  ...
  exact_lp_candidate_result
  consider_lp_point(const std::vector<double> &point,
    ...
    if (all_nonpositive(lp_solution_, 0))
      return exact_lp_candidate_result::negative_witness;
    for (const size_t row : required_outside_rows) {
      if (lp_product_[lp_outside_rows_[row]].sign() < 0)
        return exact_lp_candidate_result::unchanged;
    }
    candidate_score = score(lp_solution_, 0, lp_product_);
    if (!better_score(candidate_score, current_score_))
      return exact_lp_candidate_result::unchanged;

    remove_common_content(lp_solution_, lp_product_);
#ifdef COPOSIT_IMPROVED_NBC_X7_TESTING
  ...
  static bool lp_problem_fits(size_t support_dimension,
                              size_t outside_dimension) noexcept {
    constexpr size_t maximum_tableau_cells = size_t{8} * 1024 * 1024;
    if (support_dimension > maximum_tableau_cells - 2 ||
        outside_dimension > maximum_tableau_cells - 3)
      return false;
    return outside_dimension + 3 <=
           maximum_tableau_cells / (support_dimension + 2);
  }

  static bool
  interval_strictly_dominates(const matrix_integer &baseline_solution,
                              const std::vector<integer> &baseline_product,
                              const matrix_integer &candidate_solution,
                              const std::vector<integer> &candidate_product) {
    bool strict = false;
    for (size_t row = 0; row < baseline_solution.rows(); ++row) {
      if (!candidate_solution(row, 0).is_zero() &&
          baseline_solution(row, 0).is_zero())
        return false;
      strict |= !baseline_solution(row, 0).is_zero() &&
                candidate_solution(row, 0).is_zero();
    }
    for (size_t row = 0; row < baseline_product.size(); ++row) {
      if (baseline_product[row].sign() >= 0 &&
          candidate_product[row].sign() < 0)
        return false;
      strict |= baseline_product[row].sign() < 0 &&
                candidate_product[row].sign() >= 0;
    }
    return strict;
  }
...
#endif
      COPOSIT_IMPROVED_NBC_X7_DIAGNOSTICS("combined_ray", ray + 1);
  ...
  static void remove_common_content(matrix_integer &solution,
                                    std::vector<integer> &product) {
    integer content;
    integer next;
    for (size_t row = 0; row < solution.rows(); ++row) {
      fmpz_gcd(next.native_handle(), content.native_handle(),
               solution(row, 0).native_handle());
      content = next;
      if (content.is_one())
        return;
    }
    if (content.is_zero() || content.is_one())
      return;
    for (size_t row = 0; row < solution.rows(); ++row)
      solution(row, 0).divide_exact(content);
    for (integer &value : product)
      value.divide_exact(content);
  }

  void remove_common_content() { remove_common_content(solution_, product_); }
  ...
  void shrink_lower_preserving_upper(const matrix_integer &matrix) {
    size_t positive_entries = 0;
    for (size_t local = 0; local < solution_.rows(); ++local)
      positive_entries += solution_(local, 0).sign() > 0;
    ...
    do {
      ...
      for (size_t local = 0; local < solution_.rows(); ++local) {
        timeout_checkpoint();
        const auto coefficient = solution_(local, 0);
        if (coefficient.is_zero() ||
            (coefficient.sign() > 0 && positive_entries == 1))
          continue;

        bool preserves_upper = true;
        for (size_t row = 0; row < product_.size(); ++row) {
          candidate_product_[row] = product_[row];
          candidate_product_[row].submul(matrix(row, indices_[local]),
                                         coefficient);
          if (product_[row].sign() >= 0 && candidate_product_[row].sign() < 0) {
            preserves_upper = false;
            break;
          }
        }
        if (!preserves_upper)
        ...
        product_.swap(candidate_product_);
        positive_entries -= coefficient.sign() > 0;
        solution_(local, 0).set_zero();
        ++current_score_.width;
        changed = true;
        removed = true;
        break;
      }
  ...
  bool add_certificate() {
    ...
    support_context_.release(std::move(lower));
    support_context_.release(std::move(upper));
    COPOSIT_IMPROVED_NBC_X7_DIAGNOSTICS("dickinson", indices_.size());
    return true;
  }
  ...
  void add_upward_closure() {
    ...
    COPOSIT_IMPROVED_NBC_X7_DIAGNOSTICS("support_upward", indices_.size());
  }
  ...
  void add_downward_closure(std::string_view kind) {
    ...
    record_closure_certificate(kind, "downward", certificate_frontier_,
                               indices_);
    COPOSIT_IMPROVED_NBC_X7_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X7_TESTING
    ++downward_count_;
#endif
  }
  ...
  void publish_test_counters() const noexcept {
    ...
    last_targeted_lp_attempt_count = targeted_lp_attempt_count_;
    last_targeted_lp_feasible_count = targeted_lp_feasible_count_;
    last_targeted_lp_exact_candidate_count = targeted_lp_exact_candidate_count_;
    last_targeted_lp_accepted_count = targeted_lp_accepted_count_;
    last_targeted_lp_prepare_count = targeted_lp_prepare_count_;
    last_targeted_lp_two_candidate_round_count =
        targeted_lp_two_candidate_round_count_;
    last_targeted_lp_later_candidate_selected_count =
        targeted_lp_later_candidate_selected_count_;
  }
#endif
  ...
  matrix_integer lp_best_solution_;
  ...
  std::vector<integer> lp_best_product_;
  ...
  std::vector<double> lp_proposal_;
  std::vector<slong> lp_exponents_;
  ...
  size_t targeted_lp_attempt_count_ = 0;
  size_t targeted_lp_feasible_count_ = 0;
  size_t targeted_lp_exact_candidate_count_ = 0;
  size_t targeted_lp_accepted_count_ = 0;
  size_t targeted_lp_prepare_count_ = 0;
  size_t targeted_lp_two_candidate_round_count_ = 0;
  size_t targeted_lp_later_candidate_selected_count_ = 0;
  bool measure_support_for_testing_ = false;
  std::chrono::steady_clock::time_point test_support_started_;
...
#ifdef COPOSIT_IMPROVED_NBC_X7_TESTING
bool improved_nbc_x7_prefers_negative_singular_orientation_for_testing(
    size_t positive_products, size_t negative_products) noexcept {
  return negative_orientation_has_larger_upper(positive_products,
                                               negative_products);
}

size_t improved_nbc_x7_optimized_certificate_count_for_testing() noexcept {
  return last_optimized_certificate_count;
}

size_t improved_nbc_x7_combined_ray_sweep_count_for_testing() noexcept {
  return last_combined_ray_sweep_count;
}

size_t improved_nbc_x7_combined_ray_improvement_count_for_testing() noexcept {
  return last_combined_ray_improvement_count;
}

size_t improved_nbc_x7_pair_curvature_exclusion_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t
improved_nbc_x7_support_curvature_exclusion_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t improved_nbc_x7_pair_upward_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t improved_nbc_x7_support_upward_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t improved_nbc_x7_downward_count_for_testing() noexcept {
  return last_downward_count;
}

size_t improved_nbc_x7_high_float_rejection_count_for_testing() noexcept {
  return last_high_float_rejection_count;
}

size_t improved_nbc_x7_targeted_lp_attempt_count_for_testing() noexcept {
  return last_targeted_lp_attempt_count;
}

size_t improved_nbc_x7_targeted_lp_feasible_count_for_testing() noexcept {
  return last_targeted_lp_feasible_count;
}

size_t
improved_nbc_x7_targeted_lp_exact_candidate_count_for_testing() noexcept {
  return last_targeted_lp_exact_candidate_count;
}

size_t improved_nbc_x7_targeted_lp_accepted_count_for_testing() noexcept {
  return last_targeted_lp_accepted_count;
}

size_t improved_nbc_x7_targeted_lp_prepare_count_for_testing() noexcept {
  return last_targeted_lp_prepare_count;
}

size_t
improved_nbc_x7_targeted_lp_two_candidate_round_count_for_testing() noexcept {
  return last_targeted_lp_two_candidate_round_count;
}

size_t improved_nbc_x7_targeted_lp_later_candidate_selected_count_for_testing()
    noexcept {
  return last_targeted_lp_later_candidate_selected_count;
}

bool improved_nbc_x7_floating_psd_candidate_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool improved_nbc_x7_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
size_t improved_nbc_x7_shortlist_limit_for_testing(size_t matrix_dimension,
                                                   size_t support_dimension) {
  return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool improved_nbc_x7_prefers_ray_candidate_for_testing(
...
bool improved_nbc_x7_check_support_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool improved_nbc_x7_certificate_for_testing(const matrix_integer &matrix,
                                             const std::vector<size_t> &indices,
                                             support &lower, support &upper) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
      .optimize_support_for_testing(matrix, indices, lower, upper);
...
size_t improved_nbc_x7_fixed_support_upper_size_for_testing() noexcept {
  return last_fixed_support_upper_size;
}

bool improved_nbc_x7_traditional_interval_available_for_testing() noexcept {
  return last_traditional_interval_available;
}

size_t improved_nbc_x7_traditional_lower_size_for_testing() noexcept {
  return last_traditional_lower_size;
}

size_t improved_nbc_x7_traditional_upper_size_for_testing() noexcept {
  return last_traditional_upper_size;
}

std::uint64_t improved_nbc_x7_traditional_interval_ns_for_testing() noexcept {
  return last_traditional_interval_ns;
}

std::uint64_t improved_nbc_x7_interval_ns_for_testing() noexcept {
  return last_x7_interval_ns;
}
...
size_t improved_nbc_x7_uncovered_count(
...
size_t improved_nbc_x7_pair_exclusion_uncovered_count(size_t dimension,
                                                      size_t cardinality,
                                                      size_t first,
                                                      size_t second) {
  support_context context(dimension);
...
size_t improved_nbc_x7_interval_clause_size(size_t dimension,
                                            uint64_t lower_mask,
                                            uint64_t upper_mask) {
  size_t lower_size = 0;
  size_t upper_size = 0;
