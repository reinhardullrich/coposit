...
Classification: coposit-created exact CP/SCP experiment. Improved NBC-X7 alternates between low and high cardinalities. The low frontier uses
...
existing one. Every numerical LP point is only a proposal. X7 compares up to two exact improvements in each continuation round,
continues from the stronger one, and shrinks the lower endpoint only after that chain stops. It keeps the LP branch only when its
final exact interval strictly contains the ordinary Rays interval. The high
...
Improved NBC-X7 uses that observation as a search certificate.
...
- If a low-frontier face is strictly convex, the retained factorization is reused to build and optimize a Dickinson interval. This
  replaces the former exact-support block. X7 next asks a small LP whether a different positive right-hand side could retain the
  current upper endpoint and add one chosen missing index. A proposed vector is reconstructed exactly. X7 compares at most two exact
  improvements and uses the one with the larger upper endpoint, breaking a tie by interval width. Only after no further exact
  enlargement is found does X7 shrink the lower endpoint. The final chain interval is used only when it exactly contains the
  separately shrunken ordinary Rays interval.
...
The identifier is `improved_nbc_x7`. It is an experiment derived from the current `improved_nbc_x6` incumbent.

- **Improved NBC** names the separately owned resumable derivative of NBC MiniSat All.
- **X7** names X6 with bounded two-candidate target selection in each LP continuation round.
...
floating high-frontier screen, exact nonsingular and singular downward tests, low-frontier Halfspace-Rays machinery, repeated
targeted LP, exact lower shrinking, Boolean clauses, and interval compaction. Its only mathematical policy change is that an LP round
does not stop after its first exact improvement: it compares at most two and retains the stronger one. `improved_nbc_x6` remains
unchanged as X7's direct comparison baseline.
The dense simplex code is a private reduced copy of the implementation in
[`sat_halfspace_milp_dickinson`](../sat_halfspace_milp_dickinson/ALGORITHM.md); X7 uses no MILP variables or branch-and-bound.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Improved NBC-X7 solves
...
When low-frontier curvature does not already remove the upward closure, Improved NBC-X7 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Improved NBC-X7 performs the inherited exact
...
For one outside target $j\notin U_R$, X7 numerically solves
...
with $t\ge0$ are useful, $t$ is one nonnegative variable rather than the difference of two nonnegative variables. X7 also eliminates
...
region that X7 can accept, this is equivalent to the unreduced formulation, but with two fewer variables and one fewer constraint.
Targets are tried in increasing index order. The first exactly verified improvement is retained provisionally. The scan continues
until it finds a second exact improvement or exhausts the targets. X7 keeps the candidate with larger $|U|$, breaking a tie by larger
$|U|-|L|$; an exact tie keeps the earlier target. The retained candidate starts a new scan that preserves its enlarged upper endpoint.
The chain stops when the upper endpoint is full or a complete scan finds no exact improvement. Every successful round adds at least
one upper index, so at most $n-|U_R|$ rounds can succeed. The LP
...
fixed. X7 therefore constructs its binary64 representation once and reuses it across all continuation rounds for that support.

The LP is deliberately non-authoritative. If its floating solution has a nonnegative numerical margin, X7 normalizes it by its
...
nonnegative. Numerical infeasibility or failed reconstruction merely forgoes that optional candidate; the complete exact X6 branch
is unchanged. If the dense tableau would exceed 8,388,608 cells, X7 also skips this optional stage and uses the ordinary Rays
certificate.
...
Let $u$ be the optimized embedded vector and let $U=U(u)$. For each nonzero coordinate $u_i$, X7 tests the simpler vector
...
Intuitively, Halfspace-Rays first decides how high the certificate reaches. X7 then asks whether some baggage at its lower end can
...
X7 first completes the entire LP enlargement chain and then applies that same shrink independently to the original Rays vector and
the final exactly verified LP vector. Let their final intervals be
$[L_X,U_X]$ and $[L_P,U_P]$. The LP branch replaces the ordinary Rays branch only if exact signs prove
...
carry a worse lower endpoint after shrinking. X7 inserts only one interval, never two nearly overlapping alternatives.
...
Improved NBC-X7 retains SAT-B3's singular case. Suppose
...
3. Ask the low frontier for one open support. Copy and exactly factor its principal matrix. Install an upward closure immediately
   ...
   lower shrinking once to the final LP vector and independently to the original Rays vector. Insert the LP interval only if exact
   containment proves that it strictly contains the Rays interval; otherwise insert the ordinary Rays interval.
...
the exact X6 branch or final classification.
...
Improved NBC-X7 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
...
Each targeted LP round still tries one missing upper index at a time. Comparing only the first two exact improvements can miss a
larger interval from a later target, or one that requires losing an old upper index temporarily. When fewer than two targets produce
an exact improvement, X7 scans every missing target; this extra dense LP work can outweigh its coverage gain. Near-boundary numerical
proposals often fail exact reconstruction; in that case X7 deliberately retains the ordinary Rays certificate unless the final chain
interval already strictly dominates it.
