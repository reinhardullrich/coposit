...
Classification: coposit-created exact CP/SCP experiment. F2 preserves SAT-B3's mathematics and alternating low/high traversal, but
replaces its single SAT coverage structure with two simpler structures. The low frontier uses
...
F2 uses that observation as a search certificate.
...
Full upward curvature closures are stored in a FracESSA-style recursive support generator. Bounded Dickinson intervals, including
Dickinson intervals that reach the ceiling, downward closures, and exact-support exclusions are stored in a separate exact interval
index. The generator first proposes a support; the interval index then rejects it if one of those extra certificates covers it.

Two resumable cardinality cursors start at $1$ and $n$. The model
...
The identifier is `f2`.

- **F** denotes the FracESSA-style support-generator line.
- **2** distinguishes this split-coverage experiment from F1, which retains only ceiling-reaching certificates.

The model is an independent experiment copied from [`sat_b3`](../sat_b3/ALGORITHM.md). It retains B3's factorization, curvature
tests, Halfspace-Rays optimization, witnesses, directional pruning rules, and individual low/high alternation. Its only intended
algorithmic change is the representation and selection of unresolved supports. The recursive generator is adapted from
[`fracessa`](../fracessa/ALGORITHM.md), whose source reconstruction used FracESSA revision
`95e0ec019cf11a60c6423508e8768536a0b88860`, principally its `NonCircularSupportGenerator`. Dickinson intervals come from Peter J. C.
...
Using these curvature facts as permanent support exclusions, alternating the cardinality order, and using the search as a complete
exact CP/SCP classifier are coposit experiments rather than algorithms from those papers.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, F2 solves
...
positive-semidefinite singular support proceeds to exact verification. If the all-ones system is inconsistent, no downward interval is
...
When low-frontier curvature does not already remove the upward closure, F2 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. F2 performs the inherited exact
...
## Split Coverage Representation

F2 stores a certificate according to its shape.

### Full upward curvature closures

If the reduced Hessian on $I$ is not positive definite, the recursive generator stores the forbidden root $I$. While constructing a
candidate, it abandons a recursion branch as soon as the partial support contains a forbidden root. This removes $I$ and every
support containing it. The rule is sound because strict convexity is inherited by subfaces: if a larger face containing $I$ were
strictly convex, its restriction to $\mathcal T_I$ would also be strictly convex.
...
Every failing pair is installed as a forbidden generator root.

### Extra exact intervals

Every Dickinson certificate is stored as its exact interval $[L,U]$, even when $U=[n]$. Downward closures are stored as
$[\varnothing,I]$, and one-support exclusions as $[I,I]$. After the generator proposes $J$, the extra index rejects it exactly when

$$
L\subseteq J\subseteq U
$$

for at least one stored interval. Nonempty lower endpoints are bucketed by one required index, so a query inspects only buckets named
by indices present in $J$. Downward closures have an empty lower endpoint and are kept in a separate list. The index records both
endpoint cardinalities and skips intervals that cannot contain the candidate cardinality. When the exact low frontier advances to
cardinality $k$, it permanently discards every interval with $|U|<k$; the optional descending high frontier never triggers this
retirement because the low cursor may still need those intervals. Long scans contain periodic timeout checkpoints.

This split is deliberate: the generator represents only the monotone rule “forbid every superset of this root.” It is never asked to
represent bounded or downward intervals whose shape it cannot express.

### Downward closure

If $B=A_I$ is positive definite, every principal submatrix indexed by a nonempty subset of $I$ is positive definite. F2 therefore
stores $[\varnothing,I]$. This removes $I$ and every nonempty subset of $I$. For $I=[n]$, no nonempty support remains and the proof is
complete.

F2 retains B3's singular case. Suppose
...
certificate: the integer principal matrix is then copied and factorized exactly before any downward interval is installed.
...
matrix. F2 therefore does not store a floating rejection in either global coverage structure. The resumable high cursor simply
continues after that candidate, while the independent low cursor remains free to select and process the same support exactly later.

## Complete Decision Flow

1. Build one recursive upward-root generator, two resumable layer cursors, and one exact extra-interval index.
2. Install every failed pair-curvature test as an upward generator root.
3. Start a low frontier at cardinality $1$ and a high frontier at cardinality $n$.
4. Ask the low cursor for one support not covered by an upward root, then reject it if the extra interval index covers it. Advance
   across empty low layers, but do not exhaust a nonempty layer.
...
6. Ask the independent high cursor for one support that survives the same upward roots and extra intervals. Descend across high
   layers after every support in a layer has either been globally pruned or passed by the cursor.
...
The proof search is finite because each cursor is a resumable finite traversal, every low-selected support is covered by the exact
certificate it produces, and the Boolean lattice contains $2^n-1$ nonempty supports. Floating high rejections affect only the
optional high cursor and cannot terminate the proof.
...
positive-semidefiniteness verification. A floating rejection advances only the high cursor and cannot affect the low proof or final
classification.

Runtime diagnostics report the current low or high cardinality, selected and processed supports, installed exclusions, and the
...
and exact-support certificates in the focused model tests.

## Known Difficult Inputs

F2 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
...
produces no downward interval. The low Halfspace-Rays fallback can prune sideways and upward, but it may be expensive when many exact
...
would have removed those supports.

The extra interval index is intentionally simple. Its required-bit buckets make bounded and exact queries selective, but downward
closures have empty lower endpoints and are scanned linearly. Inputs producing many overlapping downward closures or many weak
Dickinson intervals can therefore spend substantial time in post-generation membership checks. Newly inserted upward roots can
also arrive while a layer cursor is already inside a recursion branch; final root membership remains exact, but subtree skipping is
best at the start of the next layer.
