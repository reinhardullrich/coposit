...
class upward_support_generator {
public:
    struct cursor {
        struct frame {
    ...
    bool take_first(cursor& current, size_t cardinality, support& result)
    {
        ...
        while (!current.stack.empty()) {
            timeout_checkpoint();
            cursor::frame& frame = current.stack.back();
            if (frame.needed == 0) {
                if (frame.stage == 0) {
                    frame.stage = 1;
            ...
            if (frame.needed > frame.bits_remaining) {
                current.stack.pop_back();
                continue;
            }

            const size_t bit = frame.bits_remaining - 1;
            if (frame.stage == 0) {
                frame.stage = 1;
                if (frame.needed < frame.bits_remaining)
                    current.stack.push_back({bit, frame.needed, 0});
                continue;
            }
            if (frame.stage == 1) {
                frame.stage = 2;
...
#ifdef COPOSIT_F2_TESTING
    size_t forbidden_count_for_testing() const noexcept { return forbidden_count_; }
#endif
...
public:
    ...
    {
        if (cardinality <= minimum_upper_size_) return;
...
#endif
        minimum_upper_size_ = next_minimum_upper_size;
    }

    bool contains(const support& candidate, size_t cardinality) const
    {
        size_t probes_until_timeout = timeout_probe_period;
        for (const upper_bound& interval : downward_) {
            if (--probes_until_timeout == 0) {
                timeout_checkpoint();
                probes_until_timeout = timeout_probe_period;
            }
...
#ifdef COPOSIT_F2_TESTING
    size_t interval_count_for_testing() const noexcept { return interval_count_; }
#endif
...
private:
    ...
    size_t minimum_upper_size_ = std::numeric_limits<size_t>::max();
...
public:
    ...
    void start_cardinality(size_t cardinality, bool high_frontier = false)
    {
        ...
        if (!high_frontier && cardinality > low_cardinality_) {
            extras_.expire_before(cardinality);
            low_cardinality_ = cardinality;
        }
    }
    ...
    void add_interval(support lower, support upper)
    {
    ...
    // A resumable high cursor advances past the rejected candidate. No global block is installed, so the exact low cursor can still
    // visit it.
    void reject_from_high_frontier(const std::vector<size_t>&) noexcept {}

    bool globally_empty() const noexcept { return extras_.globally_empty(); }

#ifdef COPOSIT_F2_TESTING
    std::pair<size_t, size_t> storage_counts_for_testing() const noexcept
    {
        return {generator_.forbidden_count_for_testing(), extras_.interval_count_for_testing()};
    }
#endif
...
private:
    ...
    upward_support_generator generator_;
    upward_support_generator::cursor low_cursor_;
    upward_support_generator::cursor high_cursor_;
    support candidate_;
    extra_interval_index extras_;
};
...
private:
    ...
    bool process_low_support(const matrix_integer& matrix, size_t& low, size_t high)
    {
        while (low <= high) {
            ...
            COPOSIT_F2_DIAGNOSTICS("stage_low", low);
    ...
    bool process_high_support(const matrix_integer& matrix, size_t low, size_t& high)
    {
        while (low <= high) {
            ...
            COPOSIT_F2_DIAGNOSTICS("stage_high", high);
    ...
    bool process_selected_support(const matrix_integer& matrix, pruning_direction direction, bool from_low_frontier)
    {
        ...
        COPOSIT_F2_DIAGNOSTICS(from_low_frontier ? "process_low" : "process_high", indices_.size());
    ...
    void install_pair_curvature_exclusions(const matrix_integer& matrix)
    {
        ...
        for (size_t first = 0; first < matrix.rows(); ++first) {
            ...
            for (size_t second = first + 1; second < matrix.rows(); ++second) {
                ...
                COPOSIT_F2_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_F2_TESTING
        ...
        const matrix_integer& matrix, pruning_direction direction = pruning_direction::upward, bool from_low_frontier = true)
    {
        if (!from_low_frontier) {
            ...
            if (!floating_filter_.looks_positive_semidefinite(indices_)) {
                ...
                COPOSIT_F2_DIAGNOSTICS("high_float_reject", indices_.size());
#ifdef COPOSIT_F2_TESTING
...
#endif
            COPOSIT_F2_DIAGNOSTICS("combined_ray", ray + 1);
...
#endif
        supports_.add_interval(std::move(lower), std::move(upper));
        if (diagnostics_.active()) diagnostics_.certificate(upper_size - lower_size, upper_size);
        COPOSIT_F2_DIAGNOSTICS("dickinson", indices_.size());
        return true;
    }
    ...
    void add_upward_closure()
    {
        ...
        COPOSIT_F2_DIAGNOSTICS("support_upward", indices_.size());
    }
    ...
    void add_downward_closure()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_F2_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_F2_TESTING
        ++downward_count_;
#endif
    }
    ...
    void add_exact_support()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_F2_DIAGNOSTICS("exact", indices_.size());
#ifdef COPOSIT_F2_TESTING
        ++exact_count_;
#endif
    }
...
#ifdef COPOSIT_F2_TESTING
bool f2_prefers_negative_singular_orientation_for_testing(size_t positive_products,
...
size_t f2_combined_ray_sweep_count_for_testing() noexcept
{
    return last_combined_ray_sweep_count;
}

size_t f2_combined_ray_improvement_count_for_testing() noexcept
{
    return last_combined_ray_improvement_count;
}

size_t f2_pair_curvature_exclusion_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t f2_support_curvature_exclusion_count_for_testing() noexcept
{
...
bool f2_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer& matrix, const std::vector<size_t>& indices)
{
...
size_t f2_shortlist_limit_for_testing(size_t matrix_dimension, size_t support_dimension)
{
    return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool f2_prefers_ray_candidate_for_testing(size_t candidate_upper, size_t candidate_width, size_t candidate_gains,
...
size_t f2_fixed_support_upper_size_for_testing() noexcept
{
...
std::pair<size_t, size_t> f2_interval_retirement_counts_for_testing()
{
    ...
    const size_t after_high_frontier = supports.storage_counts_for_testing().second;
    supports.start_cardinality(3);
    return {after_high_frontier, supports.storage_counts_for_testing().second};
}
    ...
    size_t dimension, size_t cardinality, const std::vector<std::vector<size_t>>& rejected, bool high_frontier)
{
    ...
    while (supports.take_first(indices, high_frontier)) {
        const bool is_rejected = std::find(rejected.begin(), rejected.end(), indices) != rejected.end();
        if (high_frontier && is_rejected) continue;
        supports.add_exact_support(indices);
        ++count;
    }
    return count;
