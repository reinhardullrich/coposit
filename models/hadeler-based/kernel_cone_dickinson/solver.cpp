...
namespace {
    ...
    public:
        ...
        bool check(const matrix_integer& matrix)
        {
            const bool result = supports_.generate([&](const support& current, size_t cardinality) {
                ...
                COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("process", cardinality);
    ...
    private:
        ...
        bool search_root_kernel(const matrix_integer& matrix)
        {
            ...
            root_basis_.resize(root_dimension, nullity);
            ...
            COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("persistent-kernel", persistent_nullity);
        ...
        void reduce_projected_rows()
        {
            ...
            COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("projected-constraints", constraint_rows_.rows());
        }
        ...
        bool add_persistent_circuit(const matrix_integer& matrix)
        {
            ...
            COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("persistent-certificate", circuit_locals_.size());
            return true;
        }
        ...
        bool enumerate_active_rays(const matrix_integer& matrix, size_t nullity)
        {
            ...
            do {
                timeout_checkpoint();
                COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("active-set", nullity);
                ...
                if (exact_nullspace(active_rows_, active_basis_) == 1) {
                    ...
                    if (!seen_rays_.insert(ray_key(coefficient_ray_)).second) {
                        COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("duplicate-ray", nullity);
        ...
        void build_active_hyperplanes()
        {
            ...
            COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("active-hyperplanes", active_hyperplane_order_.size());
        }
        ...
        bool enumerate_planar_rays(const matrix_integer& matrix)
        {
            ...
            if (!boundary_rays.insert(ray_key(coefficient_ray_)).second) {
                COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("duplicate-ray", 2);
                return true;
            }
        ...
        bool consider_active_ray(const matrix_integer& matrix)
        {
            ...
            if (products_nonnegative && has_positive) {
                add_known_ceiling_certificate(matrix.rows(), candidate_);
                COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("active-ray-certificate", support_size(candidate_));
        ...
        void add_ceiling_certificate(const matrix_integer& matrix, const matrix_integer& vector)
        {
            ...
            for (size_t row = 0; row < matrix.rows(); ++row) {
                ...
                if (product_.sign() < 0) {
                    COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("discard-certificate", indices_.size());
                    return;
                }
            }
        ...
        void add_known_ceiling_certificate(size_t matrix_dimension, const matrix_integer& vector)
        {
            ...
            COPOSIT_KERNEL_CONE_DICKINSON_DIAGNOSTICS("ceiling-certificate", lower_size);
        }
...
std::vector<uint64_t> kernel_cone_generated_masks(size_t dimension, uint64_t forbidden_trigger)
{
    assert(dimension <= 64);
