...
public:
    hadeler_checker(size_t dimension, copositivity_mode mode)
        : factorization_(dimension), mode_(mode), diagnostics_(diagnostics::metric::support, dimension)
    {
    }
    ...
    bool check(const matrix_integer& matrix)
    {
        ...
        for (size_t subset_dimension = 1; subset_dimension <= matrix_dimension; ++subset_dimension) {
            ...
            do {
                ...
                if (subset_dimension == 2) COPOSIT_SOURCE_DIAGNOSTICS("pair", indices[0], indices[1]);
