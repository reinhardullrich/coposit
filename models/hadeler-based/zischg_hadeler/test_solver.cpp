...
TEST(ZischgHadelerModelTest, UsesLevelTwoWithoutLevelOne)
{
    EXPECT_TRUE(model::solve(path_matrix(5)));
    EXPECT_GT(model::level_two_skips_for_testing(), 0U);
}

TEST(ZischgHadelerModelTest, CombinedModeClassifiesTheBoundary)
{
...
TEST(ZischgHadelerModelTest, PreservesStrictBoundaryDecision)
{
