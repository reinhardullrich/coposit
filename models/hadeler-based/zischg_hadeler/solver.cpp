...
private:
    ...
    bool check_subset(const matrix_integer& matrix, const std::vector<size_t>& indices)
    {
        ...
        if (dimension <= 3 && classification_ != nullptr) {
            const copositivity_classification face = small_copositivity::classify_principal(matrix, indices.data(), dimension);
            classification_->is_strictly_copositive &= face.is_strictly_copositive;
            return face.is_copositive;
        }
...
bool solve(const matrix_integer& matrix, copositivity_mode mode)
{
    ...
    level_two_skips = 0;
#endif
    return hadeler_checker(matrix, mode).check(matrix);
}
...
copositivity_classification classify(const matrix_integer& matrix)
{
    ...
    if (!hadeler_checker(matrix, result).check(matrix)) result = {false, false};
    return result;
}
