...
TEST(SatC4Test, AlternatesIndividualLowAndHighSupportsWithDirectionalPruning)
{
    sat_c4_diagnostics::clear();
    ...
    EXPECT_EQ(model::sat_c4_pair_upward_count_for_testing(), 0U);
    EXPECT_EQ(model::sat_c4_support_upward_count_for_testing(), 0U);
    EXPECT_EQ(model::sat_c4_downward_count_for_testing(), 1U);
    EXPECT_EQ(model::sat_c4_exact_count_for_testing(), 0U);
    EXPECT_NE(std::find(sat_c4_diagnostics::events.begin(), sat_c4_diagnostics::events.end(),
                       sat_c4_diagnostics::event{"dickinson", 1}),
              sat_c4_diagnostics::events.end());
}

TEST(SatC4Test, UsesTheFloatingFilterBeforeExactHighFrontierWork)
{
    sat_c4_diagnostics::clear();
    (void)model::classify(symmetric_matrix(3, {1, -4, -2, 1, 0, 1}));

    const auto high = std::find(sat_c4_diagnostics::events.begin(), sat_c4_diagnostics::events.end(),
                                sat_c4_diagnostics::event{"process_high", 3});
    ASSERT_NE(high, sat_c4_diagnostics::events.end());
    ASSERT_NE(std::next(high), sat_c4_diagnostics::events.end());
    EXPECT_EQ(*std::next(high), (sat_c4_diagnostics::event{"high_float_reject", 3}));
    EXPECT_GT(model::sat_c4_high_float_rejection_count_for_testing(), 0U);
}

TEST(SatC4Test, FloatingFilterOnlyProposesExactPositiveSemidefinitenessChecks)
{
    EXPECT_TRUE(model::sat_c4_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {2, 0, 0, 3, 0, 4}), {0, 1, 2}));
    EXPECT_TRUE(model::sat_c4_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {5, -1, 2, 5, 2, 2}), {0, 1, 2}));
    EXPECT_FALSE(model::sat_c4_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {1, 0, 0, -1, 0, 1}), {0, 1, 2}));
    EXPECT_FALSE(model::sat_c4_floating_psd_candidate_for_testing(
        symmetric_matrix(2, {0, 1, 0}), {0, 1}));
}

TEST(SatC4Test, FloatingFilterUsesTheSelectedSubmatrixScale)
{
    const matrix_integer matrix = symmetric_matrix(3, {1'000'000'000'000'000'000L, 0, 0, 1, 0, 1});
    EXPECT_TRUE(model::sat_c4_floating_psd_candidate_for_testing(matrix, {1, 2}));
}

TEST(SatC4Test, UsesConsistentAllOnesSystemToPruneSingularPsdSupportDownward)
{
    sat_c4_diagnostics::clear();
    ...
    EXPECT_EQ(model::sat_c4_downward_count_for_testing(), 1U);
    EXPECT_NE(std::find(sat_c4_diagnostics::events.begin(), sat_c4_diagnostics::events.end(),
                       sat_c4_diagnostics::event{"downward", 4}),
              sat_c4_diagnostics::events.end());
}

TEST(SatC4Test, FloatingRejectionsRemainVisibleToTheExactLowFrontier)
{
    const std::vector<std::vector<size_t>> rejected{{0, 1}};
    EXPECT_EQ(model::sat_c4_high_filter_uncovered_count(3, 2, rejected, true), 2U);
    EXPECT_EQ(model::sat_c4_high_filter_uncovered_count(3, 2, rejected, false), 3U);
}

TEST(SatC4Test, BuildsAnExactDickinsonIntervalOnTheLowPath)
{
    ...
    EXPECT_TRUE(model::sat_c4_certificate_for_testing(identity, {0}, lower, upper));
...
TEST(SatC4Test, RetainsTheExactHalfspaceRayOptimization)
{
    ...
    for (uint64_t mask = 1; mask < 16 && !improved; ++mask) {
        ...
        (void)model::sat_c4_check_support_for_testing(matrix, indices);
        improved = model::sat_c4_optimized_certificate_count_for_testing() > 0;
    }
    EXPECT_TRUE(improved);
}

TEST(SatC4Test, KeepsTheTraditionalCurvatureClosureAndFinishesDickinsonOptimization)
{
    sat_c4_diagnostics::clear();
    const matrix_integer matrix = symmetric_matrix(4, {2, 3, 1, 0, 10, 0, 1, 0, 0, 1});

    EXPECT_TRUE(model::sat_c4_check_endpoint_stages_for_testing(matrix, {0, 1}));
    EXPECT_EQ(model::sat_c4_endpoint_curvature_exclusion_count_for_testing(0), 1U);
    EXPECT_GT(model::sat_c4_optimized_certificate_count_for_testing(), 0U);
    const auto endpoint = std::find(sat_c4_diagnostics::events.begin(), sat_c4_diagnostics::events.end(),
                                    sat_c4_diagnostics::event{"endpoint_traditional_upward", 3});
    ASSERT_NE(endpoint, sat_c4_diagnostics::events.end());
    EXPECT_NE(std::find(std::next(endpoint), sat_c4_diagnostics::events.end(), sat_c4_diagnostics::event{"dickinson", 2}),
              sat_c4_diagnostics::events.end());
}

TEST(SatC4Test, StoresDickinsonIntervalsInTheSharedSatState)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011}, {0b100, 0b110}};
    EXPECT_EQ(model::sat_c4_uncovered_count(3, 1, intervals), 1U);
    EXPECT_EQ(model::sat_c4_uncovered_count(3, 2, intervals), 1U);
}

TEST(SatC4Test, EncodesUpwardDownwardAndExactSupportBlocks)
{
    const std::vector<std::vector<size_t>> none;
    const std::vector<std::vector<size_t>> support_01{{0, 1}};

    EXPECT_EQ(model::sat_c4_uncovered_count(3, 1, support_01, none, none), 3U);
    EXPECT_EQ(model::sat_c4_uncovered_count(3, 2, support_01, none, none), 2U);
    EXPECT_EQ(model::sat_c4_uncovered_count(3, 3, support_01, none, none), 0U);

    EXPECT_EQ(model::sat_c4_uncovered_count(3, 1, none, support_01, none), 1U);
    EXPECT_EQ(model::sat_c4_uncovered_count(3, 2, none, support_01, none), 2U);
    EXPECT_EQ(model::sat_c4_uncovered_count(3, 3, none, support_01, none), 1U);

    EXPECT_EQ(model::sat_c4_uncovered_count(3, 1, none, none, support_01), 3U);
    EXPECT_EQ(model::sat_c4_uncovered_count(3, 2, none, none, support_01), 2U);
    EXPECT_EQ(model::sat_c4_uncovered_count(3, 3, none, none, support_01), 1U);
}

TEST(SatC4Test, PreservesExactCombinedClassifications)
{
...
TEST(SatC4Test, ExhaustivelyClassifiesSmallTwoByTwoIntegerMatrices)
{
...
TEST(SatC4Test, MatchesTheIndependentExactThreeByThreeCriterion)
{
...
TEST(SatC4Test, AppliesPairCurvaturePrepass)
{
    ...
    EXPECT_EQ(model::sat_c4_pair_upward_count_for_testing(), 1U);
}
