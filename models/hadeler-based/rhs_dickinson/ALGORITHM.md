...
Classification: coposit-created exact CP/SCP variant of the maintained `dickinson_2019` adaptation.
...
This model was copied from coposit's [`dickinson_2019`](../dickinson_2019/ALGORITHM.md) implementation. The local authoritative source
...
## Strict Copositivity Within Classification

Dickinson's published construction certifies non-strict copositivity. To classify SCP in the same traversal, coposit additionally
checks every generated vector $u$ for
...
As in the maintained `dickinson_2019` model, finding a nonnegative zero ends a selected SCP query immediately. In CP or combined
mode it records that SCP is false, retains the vector's certificate, and continues the CP proof. If the finite traversal completes
...
For every visited principal support of order at most three, the shared exact classifier runs before certificate coverage. It checks
the exact order-one, order-two, and order-three formulas and updates CP and SCP independently.
...
The baseline `dickinson_2019` remains separately maintained; both models use the same selected-predicate and combined zero handling.

## CP and SCP classification

The right-hand-side search changes only certificate coverage; it does not change the CP/SCP decision rule. A componentwise
nonpositive nonsingular solution rejects CP and SCP. A nonnegative vector with exact zero quadratic value proves that SCP is false.
Strict-only mode stops at that zero; CP and combined mode retain its interval and continue because a later support may still contain
a negative witness. A completed traversal proves CP and proves SCP exactly when no zero was found. Thus `both` is one traversal, not
two calls with different right-hand-side searches.
