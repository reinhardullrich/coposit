...
TEST(RhsDickinsonModelTest, DistinguishesStrictMatricesZerosAndNegativeWitnesses)
{
...
TEST(RhsDickinsonModelTest, CombinedModeClassifiesTheBoundary)
{
...
TEST(RhsDickinsonModelTest, HandlesSingularCertificatesExactly)
{
...
TEST(RhsDickinsonModelTest, PreservesArbitraryPrecisionScaling)
{
    integer scale;
...
TEST(RhsDickinsonModelTest, ClassifiesBoundaryStressMatrix9161)
{
    EXPECT_FALSE(model::solve(symmetric_matrix(5, {1, -1, 1, 2, -3, 2, -3, -3, 4, 5, 6, -4, 5, -8, 16})));
}

TEST(RhsDickinsonModelTest, SelectsAnExactNonconstantRightHandSide)
{
    EXPECT_TRUE(model::solve(symmetric_matrix(4, {6, -4, 5, -5, 6, -7, 5, 10, -10, 14})));
    EXPECT_GT(model::rhs_dickinson_optimized_certificate_count_for_testing(), 0U);
}

TEST(RhsDickinsonModelTest, UsesPackedCoverageBeyondOneWord)
{
