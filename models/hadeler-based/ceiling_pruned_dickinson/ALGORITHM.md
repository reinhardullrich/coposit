...
75(2), 1992, 313–329. The local coposit source used for the copy is `models/hadeler-based/fracessa/solver.cpp`.

The model began as an isolated copy of `models/hadeler-based/cbdd_dickinson`. Its exact Dickinson solve, singular-vector rule, and
copositivity decisions were preserved. The CBDD was removed and the FracESSA generator inserted. No external source describes this
exact combination or its ceiling-only retention policy.
...
When diagnostics or diagnostic capture is enabled, the generator reports:
...
diagnostics history and the final sparse distribution through the standard Python results path.

## Exactness And Termination

All matrix arithmetic and sign decisions use arbitrary-precision integers. Every principal solve uses the shared exact
fraction-free LDLT implementation. No floating-point proposal or tolerance enters the decision.
...
principal-support enumeration. This can require nearly all $2^n-1$ supports even though standard Dickinson intervals would have
