...
{
    ceiling_pruned_dickinson_diagnostics::clear();
    EXPECT_TRUE(model::solve(symmetric_matrix(2, {2, -1, 2})));

    EXPECT_EQ(std::count(ceiling_pruned_dickinson_diagnostics::events.begin(), ceiling_pruned_dickinson_diagnostics::events.end(),
                         ceiling_pruned_dickinson_diagnostics::event{"discard-certificate", 1}),
              2);
    EXPECT_EQ(std::count(ceiling_pruned_dickinson_diagnostics::events.begin(), ceiling_pruned_dickinson_diagnostics::events.end(),
                         ceiling_pruned_dickinson_diagnostics::event{"ceiling-certificate", 2}),
              1);
}

TEST(CeilingPrunedDickinsonTest, PublishesDiagnosticsAndCeilingCertificateDiagnostics)
{
