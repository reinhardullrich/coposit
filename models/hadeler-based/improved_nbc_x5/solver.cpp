...
#ifdef COPOSIT_IMPROVED_NBC_X5_TESTING
...
size_t last_targeted_lp_bound_skip_count = 0;
...
std::uint64_t last_x5_interval_ns = 0;
#endif
...
constexpr double targeted_lp_margin_tolerance = 1e-9;
...
public:
  ...
  bool check(const matrix_integer &matrix) {
    ...
    targeted_lp_bound_skip_count_ = 0;
#endif
  ...
  bool optimize_support_for_testing(const matrix_integer &matrix,
    ...
    last_x5_interval_ns = static_cast<std::uint64_t>(
...
private:
  ...
  void record_interval_certificate(const support &lower, const support &upper) {
    ...
    event << "model=improved_nbc_x5 n=" << product_.size()
  ...
  bool process_low_support(const matrix_integer &matrix, size_t &low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X5_DIAGNOSTICS("stage_low", low);
  ...
  bool process_high_support(const matrix_integer &matrix, size_t low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X5_DIAGNOSTICS("stage_high", high);
  ...
  bool process_selected_support(const matrix_integer &matrix,
    ...
    COPOSIT_IMPROVED_NBC_X5_DIAGNOSTICS(
  ...
  void install_pair_curvature_exclusions(const matrix_integer &matrix) {
    ...
    for (size_t first = 0; first < matrix.rows(); ++first) {
      ...
      for (size_t second = first + 1; second < matrix.rows(); ++second) {
        ...
        COPOSIT_IMPROVED_NBC_X5_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_IMPROVED_NBC_X5_TESTING
  ...
  bool process_subset(const matrix_integer &matrix,
    ...
    if (!from_low_frontier) {
      ...
      if (!floating_filter_.looks_positive_semidefinite(indices_)) {
        ...
        COPOSIT_IMPROVED_NBC_X5_DIAGNOSTICS("high_float_reject",
                                            indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X5_TESTING
  ...
  exact_lp_candidate_result try_targeted_enlargement() {
    ...
    for (size_t target = 0; target < outside_dimension; ++target) {
      ...
      if (product_[lp_outside_rows_[target]].sign() >= 0)
        continue;
      const double *const target_coefficients =
          lp_scaled_rows_.data() + target * support_dimension;
      double target_sum = 0.0;
      double target_maximum = target_coefficients[0];
      for (size_t column = 0; column < support_dimension; ++column) {
        target_sum += target_coefficients[column];
        target_maximum = std::max(target_maximum, target_coefficients[column]);
      }
      if (positive_floor * target_sum + simplex_scale * target_maximum <
          -targeted_lp_margin_tolerance) {
#ifdef COPOSIT_IMPROVED_NBC_X5_TESTING
        ++targeted_lp_bound_skip_count_;
#endif
        continue;
      }
      ...
      if (status != detail::tiny_simplex::status::optimal ||
          proposal[margin_column] < -targeted_lp_margin_tolerance)
        continue;
#ifdef COPOSIT_IMPROVED_NBC_X5_TESTING
      ...
      for (size_t column = 0; column < reduced_dimension; ++column) {
        simplex_sum += proposal[column];
        proposal[column] = positive_floor + simplex_scale * proposal[column];
      }
      proposal[reduced_dimension] =
          positive_floor + simplex_scale * (1.0 - simplex_sum);
      const exact_lp_candidate_result result =
          consider_lp_point(proposal, lp_required_rows_);
...
#endif
      COPOSIT_IMPROVED_NBC_X5_DIAGNOSTICS("combined_ray", ray + 1);
  ...
  bool add_certificate() {
    ...
    support_context_.release(std::move(lower));
    support_context_.release(std::move(upper));
    COPOSIT_IMPROVED_NBC_X5_DIAGNOSTICS("dickinson", indices_.size());
    return true;
  }
  ...
  void add_upward_closure() {
    ...
    COPOSIT_IMPROVED_NBC_X5_DIAGNOSTICS("support_upward", indices_.size());
  }
  ...
  void add_downward_closure(std::string_view kind) {
    ...
    record_closure_certificate(kind, "downward", certificate_frontier_,
                               indices_);
    COPOSIT_IMPROVED_NBC_X5_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X5_TESTING
    ++downward_count_;
#endif
  }
  ...
  void publish_test_counters() const noexcept {
    ...
    last_targeted_lp_bound_skip_count = targeted_lp_bound_skip_count_;
  }
#endif
  ...
  size_t targeted_lp_bound_skip_count_ = 0;
...
#ifdef COPOSIT_IMPROVED_NBC_X5_TESTING
bool improved_nbc_x5_prefers_negative_singular_orientation_for_testing(
    size_t positive_products, size_t negative_products) noexcept {
  return negative_orientation_has_larger_upper(positive_products,
                                               negative_products);
}

size_t improved_nbc_x5_optimized_certificate_count_for_testing() noexcept {
  return last_optimized_certificate_count;
}

size_t improved_nbc_x5_combined_ray_sweep_count_for_testing() noexcept {
  return last_combined_ray_sweep_count;
}

size_t improved_nbc_x5_combined_ray_improvement_count_for_testing() noexcept {
  return last_combined_ray_improvement_count;
}

size_t improved_nbc_x5_pair_curvature_exclusion_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t
improved_nbc_x5_support_curvature_exclusion_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t improved_nbc_x5_pair_upward_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t improved_nbc_x5_support_upward_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t improved_nbc_x5_downward_count_for_testing() noexcept {
  return last_downward_count;
}

size_t improved_nbc_x5_high_float_rejection_count_for_testing() noexcept {
  return last_high_float_rejection_count;
}

size_t improved_nbc_x5_targeted_lp_attempt_count_for_testing() noexcept {
  return last_targeted_lp_attempt_count;
}

size_t improved_nbc_x5_targeted_lp_feasible_count_for_testing() noexcept {
  return last_targeted_lp_feasible_count;
}

size_t
improved_nbc_x5_targeted_lp_exact_candidate_count_for_testing() noexcept {
  return last_targeted_lp_exact_candidate_count;
}

size_t improved_nbc_x5_targeted_lp_accepted_count_for_testing() noexcept {
  return last_targeted_lp_accepted_count;
}

size_t improved_nbc_x5_targeted_lp_bound_skip_count_for_testing() noexcept {
  return last_targeted_lp_bound_skip_count;
}

bool improved_nbc_x5_floating_psd_candidate_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool improved_nbc_x5_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
size_t improved_nbc_x5_shortlist_limit_for_testing(size_t matrix_dimension,
                                                   size_t support_dimension) {
  return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool improved_nbc_x5_prefers_ray_candidate_for_testing(
...
bool improved_nbc_x5_check_support_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool improved_nbc_x5_certificate_for_testing(const matrix_integer &matrix,
                                             const std::vector<size_t> &indices,
                                             support &lower, support &upper) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
      .optimize_support_for_testing(matrix, indices, lower, upper);
...
size_t improved_nbc_x5_fixed_support_upper_size_for_testing() noexcept {
  return last_fixed_support_upper_size;
}

bool improved_nbc_x5_traditional_interval_available_for_testing() noexcept {
  return last_traditional_interval_available;
}

size_t improved_nbc_x5_traditional_lower_size_for_testing() noexcept {
  return last_traditional_lower_size;
}

size_t improved_nbc_x5_traditional_upper_size_for_testing() noexcept {
  return last_traditional_upper_size;
}

std::uint64_t improved_nbc_x5_traditional_interval_ns_for_testing() noexcept {
  return last_traditional_interval_ns;
}

std::uint64_t improved_nbc_x5_interval_ns_for_testing() noexcept {
  return last_x5_interval_ns;
}
...
size_t improved_nbc_x5_uncovered_count(
...
size_t improved_nbc_x5_pair_exclusion_uncovered_count(size_t dimension,
                                                      size_t cardinality,
                                                      size_t first,
                                                      size_t second) {
  support_context context(dimension);
...
size_t improved_nbc_x5_interval_clause_size(size_t dimension,
                                            uint64_t lower_mask,
                                            uint64_t upper_mask) {
  size_t lower_size = 0;
  size_t upper_size = 0;
