...
Classification: coposit-created CP/SCP variant with a bounded Frank–Wolfe witness search in front of the maintained
...
does not produce such an exact witness, the model runs the maintained CP/SCP Dickinson traversal. Therefore Frank–Wolfe can add an
...
10. If no decisive negative witness was verified, run the maintained Dickinson subset traversal in the selected or combined mode.
11. Return the exact selected predicate or CP/SCP classification.
...
Every floating Frank–Wolfe proposal is reconstructed and checked exactly before it affects the answer. An exact negative quadratic
...
Dickinson traversal because another support may still be negative. The Dickinson phase applies the same rule to its singular
...
Frank–Wolfe search followed by at most one Dickinson traversal.
