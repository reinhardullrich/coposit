...
public:
  ...
  bool process_support_for_testing(const matrix_integer &matrix,
    ...
    seed_dickinson_upper_.reset();
    retain_partial_dickinson_ = true;
    const bool result = process_subset(matrix, pruning_direction::upward, true);
    retain_partial_dickinson_ = false;
    if (!result || !seed_dickinson_upper_)
      return result;
    COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("walk_from_dickinson_upper",
                                               seed_dickinson_upper_->size());
    return process_frontier_walk(matrix, std::move(*seed_dickinson_upper_));
  }
...
private:
  ...
  void record_interval_certificate(const support &lower, const support &upper) {
    ...
    event << "model=dual_frontier_nbc_four n=" << product_.size()
  ...
  bool process_low_support(const matrix_integer &matrix, size_t &low,
    ...
    while (low <= high) {
      ...
      COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("stage_low", low);
  ...
  bool process_selected_support(const matrix_integer &matrix) {
    ...
    COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("process_low", indices_.size());
    seed_dickinson_upper_.reset();
    retain_partial_dickinson_ = true;
    const bool seed_result =
        process_subset(matrix, pruning_direction::upward, true);
    retain_partial_dickinson_ = false;
    if (!seed_result || !seed_dickinson_upper_)
      return seed_result;
    COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("walk_from_dickinson_upper",
                                               seed_dickinson_upper_->size());
    return process_frontier_walk(matrix, std::move(*seed_dickinson_upper_));
  ...
  bool extension_is_covered(support &candidate_support, size_t candidate) {
    support_context_.set(candidate_support, candidate);
    const bool covered = supports_->covers(candidate_support);
    support_context_.reset(candidate_support, candidate);
    return covered;
  }

  std::optional<size_t>
  best_principal_extension(const std::vector<size_t> &current,
                           bool &has_uncovered_extension) {
    has_uncovered_extension = false;
    if (!floating_filter_.prepare_principal_extensions(current))
      return std::nullopt;
    support candidate_support = support_context_.make();
    for (const size_t index : current)
      support_context_.set(candidate_support, index);
    ...
    for (size_t candidate = 0; candidate < product_.size(); ++candidate) {
      if (std::binary_search(current.begin(), current.end(), candidate))
        continue;
      if (extension_is_covered(candidate_support, candidate))
        continue;
      has_uncovered_extension = true;
  ...
  bool process_frontier_walk(const matrix_integer &matrix,
                             ...
                             bool seed_already_processed = false) {
    std::vector<std::vector<size_t>> path;
    std::vector<std::vector<size_t>> ceiling_candidates;
    ...
    bool blocked_by_coverage = false;
    remember_walk_support(current, path, ceiling_candidates);

    while (current.size() < product_.size()) {
      timeout_checkpoint();
      bool has_uncovered_extension = false;
      const auto next =
          best_principal_extension(current, has_uncovered_extension);
      if (!next) {
        if (floating_filter_.prepare_principal_extensions(current))
          downward_candidate = current;
        blocked_by_coverage = !has_uncovered_extension;
        break;
      }
    ...
    while (!blocked_by_coverage && current.size() < product_.size()) {
      timeout_checkpoint();
      bool has_uncovered_extension = false;
      const auto next =
          best_tangent_extension(current, has_uncovered_extension);
      if (!next) {
        blocked_by_coverage = !has_uncovered_extension;
        break;
      }
    ...
    const std::optional<std::vector<size_t>> terminal_candidate =
        blocked_by_coverage ? std::optional<std::vector<size_t>>(current)
                            : std::nullopt;
    ...
    if (downward_candidate) {
      if (!terminal_candidate || *downward_candidate != *terminal_candidate)
        verify_downward_frontier(matrix, *downward_candidate);
    }
    if (upward_candidate)
      verify_upward_frontier(matrix, *upward_candidate);
    if (terminal_candidate &&
        (!seed_already_processed || *terminal_candidate != seed)) {
      verify_downward_frontier(matrix, *terminal_candidate);
      verify_upward_frontier(matrix, *terminal_candidate);
    }
    ...
    if (covered || seed_already_processed)
      return true;
    indices_ = seed;
    certificate_frontier_ = "low";
  ...
  bool verify_ceiling_dickinson(const matrix_integer &matrix,
    ...
    COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("dickinson_ceiling",
                                               candidate.size());
    support_context_.release(std::move(lower));
    support_context_.release(std::move(ceiling));
    return true;
  ...
  void install_pair_curvature_exclusions(const matrix_integer &matrix) {
    ...
    for (size_t first = 0; first < matrix.rows(); ++first) {
      ...
      for (size_t second = first + 1; second < matrix.rows(); ++second) {
        ...
        COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_DUAL_FRONTIER_NBC_FOUR_TESTING
  ...
  bool process_subset(const matrix_integer &matrix,
    ...
    if (!from_low_frontier) {
      ...
      if (!floating_filter_.looks_positive_semidefinite(indices_)) {
        ...
        COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("high_float_reject",
                                                   indices_.size());
#ifdef COPOSIT_DUAL_FRONTIER_NBC_FOUR_TESTING
...
#endif
      COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("combined_ray", ray + 1);
...
#endif
    if (retain_partial_dickinson_) {
      seed_dickinson_upper_.emplace();
      seed_dickinson_upper_->reserve(upper_size);
      for (size_t index = 0; index < product_.size(); ++index) {
        if (support_context_.contains(upper, index))
          seed_dickinson_upper_->push_back(index);
      }
    }
    if (upper_size == product_.size() || retain_partial_dickinson_) {
      ...
      COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS(
          upper_size == product_.size() ? "dickinson_ceiling"
                                        : "dickinson_seed_interval",
          indices_.size());
    } else {
      record_low_support_without_certificate();
      COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("dickinson_partial_discarded",
                                                 indices_.size());
    }
    support_context_.release(std::move(lower));
    support_context_.release(std::move(upper));
  ...
  void add_upward_closure() {
    ...
    COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("support_upward",
                                               indices_.size());
  }
  ...
  void add_downward_closure(std::string_view kind) {
    ...
    record_closure_certificate(kind, "downward", certificate_frontier_,
                               indices_);
    COPOSIT_DUAL_FRONTIER_NBC_FOUR_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_DUAL_FRONTIER_NBC_FOUR_TESTING
    ++downward_count_;
#endif
  }
  ...
  bool retain_partial_dickinson_ = false;
  std::optional<std::vector<size_t>> seed_dickinson_upper_;
...
#ifdef COPOSIT_DUAL_FRONTIER_NBC_FOUR_TESTING
bool dual_frontier_nbc_four_prefers_negative_singular_orientation_for_testing(
...
size_t
dual_frontier_nbc_four_optimized_certificate_count_for_testing() noexcept {
  return last_optimized_certificate_count;
}

size_t dual_frontier_nbc_four_combined_ray_sweep_count_for_testing() noexcept {
  return last_combined_ray_sweep_count;
}

size_t
dual_frontier_nbc_four_combined_ray_improvement_count_for_testing() noexcept {
  return last_combined_ray_improvement_count;
}

size_t
dual_frontier_nbc_four_pair_curvature_exclusion_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t
dual_frontier_nbc_four_support_curvature_exclusion_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t dual_frontier_nbc_four_pair_upward_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t dual_frontier_nbc_four_support_upward_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t dual_frontier_nbc_four_downward_count_for_testing() noexcept {
  return last_downward_count;
}

size_t
dual_frontier_nbc_four_high_float_rejection_count_for_testing() noexcept {
  return last_high_float_rejection_count;
}

bool dual_frontier_nbc_four_floating_psd_candidate_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool dual_frontier_nbc_four_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
size_t
dual_frontier_nbc_four_shortlist_limit_for_testing(size_t matrix_dimension,
                                                   size_t support_dimension) {
  return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool dual_frontier_nbc_four_prefers_ray_candidate_for_testing(
...
bool dual_frontier_nbc_four_check_support_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool dual_frontier_nbc_four_process_support_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
      .process_support_for_testing(matrix, indices);
}

bool dual_frontier_nbc_four_process_walk_with_upward_closure_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &seed,
    const std::vector<size_t> &covered_root) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
...
bool dual_frontier_nbc_four_certificate_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices,
    support &lower, support &upper) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
      .optimize_support_for_testing(matrix, indices, lower, upper);
...
size_t dual_frontier_nbc_four_fixed_support_upper_size_for_testing() noexcept {
...
size_t dual_frontier_nbc_four_uncovered_count(
...
size_t dual_frontier_nbc_four_pair_exclusion_uncovered_count(size_t dimension,
                                                             size_t cardinality,
                                                             size_t first,
                                                             size_t second) {
  support_context context(dimension);
...
size_t dual_frontier_nbc_four_interval_clause_size(size_t dimension,
                                                   uint64_t lower_mask,
                                                   uint64_t upper_mask) {
  size_t lower_size = 0;
  size_t upper_size = 0;
