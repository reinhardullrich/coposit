...
TEST(DenseBitsetDickinsonTest, JumpsOverIdentitySupersets)
{
    ...
    dense_bitset_dickinson_diagnostics::clear();
    EXPECT_TRUE(model::solve(identity));

    ASSERT_EQ(dense_bitset_dickinson_diagnostics::events.size(), 4U);
    for (const auto& event : dense_bitset_dickinson_diagnostics::events)
        EXPECT_EQ(event, (dense_bitset_dickinson_diagnostics::event{"process", 1}));
}
