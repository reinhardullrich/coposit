...
Classification: coposit-created exact copositivity and strict-copositivity experiment copied from `dickinson_2019`.
...
“Dense-Bitset” states the only mathematical-control-flow change from Dickinson 2019: the explicit $2^n$-bit representation of the
...
termination are copied from Dickinson 2019.
...
agrees with increasing numeric support-mask order, so the exact calculations have the same deterministic order as Dickinson 2019
...
the current layer. Exact diagnostics reporting counts the processed support once and every newly covered future bit as a skipped
support.
