...
private:
  ...
  void record_interval_certificate(const support &lower, const support &upper) {
    ...
    event << "model=dual_frontier_nbc_two n=" << product_.size()
  ...
  bool process_low_support(const matrix_integer &matrix, size_t &low,
    ...
    while (low <= high) {
      ...
      COPOSIT_DUAL_FRONTIER_NBC_TWO_DIAGNOSTICS("stage_low", low);
  ...
  bool process_selected_support(const matrix_integer &matrix) {
    ...
    COPOSIT_DUAL_FRONTIER_NBC_TWO_DIAGNOSTICS("process_low", indices_.size());
  ...
  bool verify_ceiling_dickinson(const matrix_integer &matrix,
    ...
    COPOSIT_DUAL_FRONTIER_NBC_TWO_DIAGNOSTICS("dickinson_ceiling",
                                              candidate.size());
    support_context_.release(std::move(lower));
    support_context_.release(std::move(ceiling));
    return true;
  ...
  void install_pair_curvature_exclusions(const matrix_integer &matrix) {
    ...
    for (size_t first = 0; first < matrix.rows(); ++first) {
      ...
      for (size_t second = first + 1; second < matrix.rows(); ++second) {
        ...
        COPOSIT_DUAL_FRONTIER_NBC_TWO_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_DUAL_FRONTIER_NBC_TWO_TESTING
  ...
  bool process_subset(const matrix_integer &matrix,
    ...
    if (!from_low_frontier) {
      ...
      if (!floating_filter_.looks_positive_semidefinite(indices_)) {
        ...
        COPOSIT_DUAL_FRONTIER_NBC_TWO_DIAGNOSTICS("high_float_reject",
                                                  indices_.size());
#ifdef COPOSIT_DUAL_FRONTIER_NBC_TWO_TESTING
...
#endif
      COPOSIT_DUAL_FRONTIER_NBC_TWO_DIAGNOSTICS("combined_ray", ray + 1);
  ...
  bool add_certificate() {
    ...
    if (upper_size == product_.size()) {
      ...
      COPOSIT_DUAL_FRONTIER_NBC_TWO_DIAGNOSTICS("dickinson_partial_discarded",
                                                indices_.size());
    }
    support_context_.release(std::move(lower));
    support_context_.release(std::move(upper));
  ...
  void add_upward_closure() {
    ...
    COPOSIT_DUAL_FRONTIER_NBC_TWO_DIAGNOSTICS("support_upward",
                                              indices_.size());
  }
  ...
  void add_downward_closure(std::string_view kind) {
    ...
    record_closure_certificate(kind, "downward", certificate_frontier_,
                               indices_);
    COPOSIT_DUAL_FRONTIER_NBC_TWO_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_DUAL_FRONTIER_NBC_TWO_TESTING
    ++downward_count_;
#endif
  }
...
#ifdef COPOSIT_DUAL_FRONTIER_NBC_TWO_TESTING
bool dual_frontier_nbc_two_prefers_negative_singular_orientation_for_testing(
...
size_t
dual_frontier_nbc_two_optimized_certificate_count_for_testing() noexcept {
  return last_optimized_certificate_count;
}

size_t dual_frontier_nbc_two_combined_ray_sweep_count_for_testing() noexcept {
  return last_combined_ray_sweep_count;
}

size_t
dual_frontier_nbc_two_combined_ray_improvement_count_for_testing() noexcept {
  return last_combined_ray_improvement_count;
}

size_t
dual_frontier_nbc_two_pair_curvature_exclusion_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t
dual_frontier_nbc_two_support_curvature_exclusion_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t dual_frontier_nbc_two_pair_upward_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t dual_frontier_nbc_two_support_upward_count_for_testing() noexcept {
  return last_support_curvature_exclusion_count;
}

size_t dual_frontier_nbc_two_downward_count_for_testing() noexcept {
  return last_downward_count;
}

size_t dual_frontier_nbc_two_high_float_rejection_count_for_testing() noexcept {
  return last_high_float_rejection_count;
}

bool dual_frontier_nbc_two_floating_psd_candidate_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool dual_frontier_nbc_two_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
size_t
dual_frontier_nbc_two_shortlist_limit_for_testing(size_t matrix_dimension,
                                                  size_t support_dimension) {
  return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool dual_frontier_nbc_two_prefers_ray_candidate_for_testing(
...
bool dual_frontier_nbc_two_check_support_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
bool dual_frontier_nbc_two_process_support_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
      .process_support_for_testing(matrix, indices);
}

bool dual_frontier_nbc_two_process_walk_with_upward_closure_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &seed,
    const std::vector<size_t> &covered_root) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
...
bool dual_frontier_nbc_two_certificate_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices,
    support &lower, support &upper) {
  return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
      .optimize_support_for_testing(matrix, indices, lower, upper);
...
size_t dual_frontier_nbc_two_fixed_support_upper_size_for_testing() noexcept {
...
size_t dual_frontier_nbc_two_uncovered_count(
...
size_t dual_frontier_nbc_two_pair_exclusion_uncovered_count(size_t dimension,
                                                            size_t cardinality,
                                                            size_t first,
                                                            size_t second) {
  support_context context(dimension);
...
size_t dual_frontier_nbc_two_interval_clause_size(size_t dimension,
                                                  uint64_t lower_mask,
                                                  uint64_t upper_mask) {
  size_t lower_size = 0;
  size_t upper_size = 0;
