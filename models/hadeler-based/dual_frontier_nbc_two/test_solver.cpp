...
bool recorded(std::string_view name) {
  return std::any_of(coposit::dual_frontier_nbc_two_diagnostics::events.begin(),
                     coposit::dual_frontier_nbc_two_diagnostics::events.end(),
                     [&](const auto &event) { return event.name == name; });
}

TEST(DualFrontierNbcTwoTest, ClassifiesStrictCopositivityInOneTraversal) {
  coposit::dual_frontier_nbc_two_diagnostics::clear();
...
TEST(DualFrontierNbcTwoTest, RetainsOnlyCeilingDickinsonPruning) {
  coposit::dual_frontier_nbc_two_diagnostics::clear();
  const auto matrix = symmetric_matrix(2, {1, -1, 1});

  EXPECT_TRUE(
      model::dual_frontier_nbc_two_process_support_for_testing(matrix, {0}));
  EXPECT_TRUE(recorded("dickinson_partial_discarded"));
  EXPECT_FALSE(recorded("dickinson_ceiling"));
}

TEST(DualFrontierNbcTwoTest, FindsAnExactNegativeWitnessAfterTheFloatWalk) {
...
TEST(DualFrontierNbcTwoTest, KeepsPairCurvaturePruning) {
  coposit::dual_frontier_nbc_two_diagnostics::clear();
  const auto result = model::classify(symmetric_matrix(2, {1, 2, 1}));
...
TEST(DualFrontierNbcTwoTest, PreservesTheEmittedSeedAcrossWalkVerification) {
...
TEST(DualFrontierNbcTwoTest, WalkChoosesTheBestUncoveredExtension) {
  coposit::dual_frontier_nbc_two_diagnostics::clear();
  const auto matrix = symmetric_matrix(3, {1, 0, 0, 3, 0, 2});

  EXPECT_TRUE(
      model::dual_frontier_nbc_two_process_walk_with_upward_closure_for_testing(
          matrix, {0}, {1}));
  EXPECT_TRUE(
      std::find(
          coposit::dual_frontier_nbc_two_diagnostics::events.begin(),
          coposit::dual_frontier_nbc_two_diagnostics::events.end(),
          coposit::dual_frontier_nbc_two_diagnostics::event{"downward", 2}) !=
      coposit::dual_frontier_nbc_two_diagnostics::events.end());
}
