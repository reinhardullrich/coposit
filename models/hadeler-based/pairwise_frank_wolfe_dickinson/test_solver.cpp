...
TEST(PairwiseFrankWolfeDickinsonModelTest, UsesAFeasiblePairwiseStep)
{
    EXPECT_TRUE(model::solve(symmetric_matrix(4, {1, 0, 0, 0, 2, 0, 0, 3, 0, 4})));
    EXPECT_TRUE(model::pairwise_step_used_for_testing());
    EXPECT_FALSE(model::frank_wolfe_witness_found_for_testing());
}

TEST(PairwiseFrankWolfeDickinsonModelTest, CombinedModeClassifiesTheBoundary)
{
...
TEST(PairwiseFrankWolfeDickinsonModelTest, FindsAnExactHigherOrderNegativeWitness)
{
...
TEST(PairwiseFrankWolfeDickinsonModelTest, FallsBackToDickinson)
{
...
TEST(PairwiseFrankWolfeDickinsonModelTest, PreservesArbitraryPrecisionScaling)
{
    integer scale;
...
TEST(PairwiseFrankWolfeDickinsonModelTest, DistinguishesLowDimensionalCases)
{
