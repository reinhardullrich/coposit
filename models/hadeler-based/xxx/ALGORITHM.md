...
cardinality order, and persistent SAT representation, but adds at most two synthesized right-hand-side rays after ordinary coordinate
...
Every accepted vector is checked exactly, so the shortlist and pair ranking affect speed and certificate order, not correctness.

## Name, Sources, And Classification

The identifier is `xxx`. **XXX** is the user-selected working name for the new research model. At this preparation stage,
its mathematics is exactly SAT-Halfspace-Rays Dickinson; the name does not yet denote an additional algorithmic change.
...
7. Add the single interval belonging to the final exact vector.
8. Continue until SAT has no uncovered support at that cardinality.
