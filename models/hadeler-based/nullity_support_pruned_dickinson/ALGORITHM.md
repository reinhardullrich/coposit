...
Classification: coposit-created exact CP/SCP variant of `support_pruned_dickinson`.
...
- `models/hadeler-based/support_pruned_dickinson/`, copied as the complete starting implementation. Its recursive support generator comes from
...
solution is componentwise nonpositive rejects CP and SCP. If the nullspace search finds a nonzero componentwise nonnegative vector,
SCP is false. Strict-only mode stops there; CP and combined mode retain that vector's Dickinson certificate and any valid
ceiling-support pruning, then continue because a later support may still contain a negative witness. A completed traversal proves
