...
TEST(BddDickinsonTest, PreservesStrictDickinsonDecisions)
{
...
TEST(BddDickinsonTest, ClassifiesCopositiveBoundary)
{
...
TEST(BddDickinsonTest, SubtractsTheUnionOfBoundedIntervals)
{
...
TEST(BddDickinsonTest, DoesNotEmitSupportsAlreadyCoveredByIdentityCertificates)
{
    matrix_integer identity;
    identity.set_identity(4);
    bdd_dickinson_diagnostics::clear();
    EXPECT_TRUE(model::solve(identity));

    ASSERT_EQ(bdd_dickinson_diagnostics::events.size(), 4U);
    for (const auto& event : bdd_dickinson_diagnostics::events)
        EXPECT_EQ(event, (bdd_dickinson_diagnostics::event{"process", 1}));
}

TEST(BddDickinsonTest, KeepsPackedSupportsBeyondOneWord)
{
...
TEST(BddDickinsonTest, CooperativeTimeoutEscapesDiagramOperations)
{
    request_timeout();
