...
TEST(LayeredSingularLiftDickinsonTest, PreservesStrictAndCopositiveDecisions)
{
...
TEST(LayeredSingularLiftDickinsonTest, ClassifiesBothPredicatesInOneTraversal)
{
...
TEST(LayeredSingularLiftDickinsonTest, LiftsHighNullityDepthFirstAndStoresTheFinalVectorSupport)
{
    ...
    layered_singular_lift_dickinson_diagnostics::clear();
    diagnostics::detail::reset();
    diagnostics::detail::state.enabled.store(true, std::memory_order_relaxed);
    EXPECT_TRUE(model::layered_singular_lift_for_test(matrix, {0, 1}));
    ...
    const auto& events = layered_singular_lift_dickinson_diagnostics::events;
    EXPECT_NE(std::find(events.begin(), events.end(), layered_singular_lift_dickinson_diagnostics::event{"lift", 3}), events.end());
    EXPECT_NE(std::find(events.begin(), events.end(),
                        layered_singular_lift_dickinson_diagnostics::event{"lift-ceiling-certificate", 2}),
              events.end());
...
TEST(LayeredSingularLiftDickinsonTest, ActivatesAFoundLowerSupportOnlyAfterItsOuterLayer)
{
    const std::vector<uint64_t> generated = model::layered_generated_masks(4, 0b0101);
...
TEST(LayeredSingularLiftDickinsonTest, DiagnosesDistinctAndDuplicateLiftedSupports)
{
...
TEST(LayeredSingularLiftDickinsonTest, KeepsPackedSupportsBeyondOneWord)
{
