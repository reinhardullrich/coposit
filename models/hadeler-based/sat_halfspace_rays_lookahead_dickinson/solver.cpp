...
size_t last_lookahead_analysis_count = 0;
size_t last_lookahead_cache_hit_count = 0;
#endif
...
class support_key {
public:
    explicit support_key(size_t word_count)
        : words_(word_count, 0)
    {
    }

    void set(size_t index) noexcept { words_[index / 64] |= uint64_t{1} << (index % 64); }
    void reset(size_t index) noexcept { words_[index / 64] &= ~(uint64_t{1} << (index % 64)); }
    bool contains(size_t index) const noexcept { return (words_[index / 64] & (uint64_t{1} << (index % 64))) != 0; }

    friend bool operator==(const support_key& left, const support_key& right) noexcept { return left.words_ == right.words_; }
...
struct support_key_hash {
    size_t operator()(const support_key& key) const noexcept
    {
        size_t hash = sizeof(size_t) == sizeof(uint64_t) ? size_t{1469598103934665603ULL} : size_t{2166136261U};
        for (const uint64_t word : key.words()) {
            hash ^= static_cast<size_t>(word);
            hash *= sizeof(size_t) == sizeof(uint64_t) ? size_t{1099511628211ULL} : size_t{16777619U};
            if constexpr (sizeof(size_t) < sizeof(uint64_t)) {
                hash ^= static_cast<size_t>(word >> 32);
    ...
    bool check(const matrix_integer& matrix)
    {
        ...
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            while (supports_->take_first(indices_)) {
                ...
                if (!analyze_support(matrix, current_indices, current)) {
                    diagnostics_.finish();
#ifdef COPOSIT_SAT_HALFSPACE_RAYS_LOOKAHEAD_DICKINSON_TESTING
                ...
                if (!process_lookahead(matrix, current_indices, current)) {
                    diagnostics_.finish();
#ifdef COPOSIT_SAT_HALFSPACE_RAYS_LOOKAHEAD_DICKINSON_TESTING
                    publish_test_counters();
#endif
...
private:
    support_key make_key(const std::vector<size_t>& indices) const
    {
        support_key key((product_.size() + 63) / 64);
        for (const size_t index : indices) key.set(index);
        return key;
    }

    bool analyze_support(
        const matrix_integer& matrix, const std::vector<size_t>& indices, cached_certificate& certificate)
    {
        indices_ = indices;
        captured_certificate_ = &certificate;
        const bool result = process_subset(matrix);
        captured_certificate_ = nullptr;
        return result;
    }

    bool commit_certificate(cached_certificate& certificate)
    {
        if (certificate.committed) return true;
        supports_->add_interval(certificate.lower, certificate.upper);
        certificate.committed = true;
        if (diagnostics_.active()) diagnostics_.certificate(certificate.score.width, certificate.score.upper_size);
        return true;
    }

    bool process_lookahead(
        const matrix_integer& matrix, const std::vector<size_t>& current_indices, cached_certificate& current)
    {
        const size_t dimension = matrix.rows();
        if (current_indices.size() == dimension) return commit_certificate(current);

        support_key child_key = make_key(current_indices);
        bool current_is_strictly_contained = false;

        for (size_t added_index = 0; added_index < dimension; ++added_index) {
            timeout_checkpoint();
            if (child_key.contains(added_index)) continue;

            child_key.set(added_index);
            auto candidate = child_cache_.find(child_key);
            if (candidate == child_cache_.end()) {
                ...
                if (!analyze_support(matrix, child_indices, child)) {
                    indices_ = current_indices;
                    return false;
                }
#ifdef COPOSIT_SAT_HALFSPACE_RAYS_LOOKAHEAD_DICKINSON_TESTING
                ++lookahead_analysis_count_;
#endif
                candidate = child_cache_.emplace(child_key, std::move(child)).first;
            } else {
#ifdef COPOSIT_SAT_HALFSPACE_RAYS_LOOKAHEAD_DICKINSON_TESTING
                ++lookahead_cache_hit_count_;
#endif
            }
            ...
            child_key.reset(added_index);
        }

        indices_ = current_indices;
        if (!current_is_strictly_contained) commit_certificate(current);
        return true;
    }
...
#endif
            COPOSIT_SAT_HALFSPACE_RAYS_LOOKAHEAD_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    void publish_test_counters() const noexcept
    {
        ...
        last_lookahead_analysis_count = lookahead_analysis_count_;
    ...
    certificate_cache child_cache_;
    cached_certificate* captured_certificate_ = nullptr;
#ifdef COPOSIT_SAT_HALFSPACE_RAYS_LOOKAHEAD_DICKINSON_TESTING
    size_t optimized_certificate_count_ = 0;
    size_t combined_ray_sweep_count_ = 0;
    size_t combined_ray_improvement_count_ = 0;
    size_t lookahead_analysis_count_ = 0;
    size_t lookahead_cache_hit_count_ = 0;
...
{
    return last_lookahead_analysis_count;
}

size_t sat_halfspace_rays_lookahead_cache_hit_count_for_testing() noexcept
{
    return last_lookahead_cache_hit_count;
}
