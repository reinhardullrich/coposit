...
private:
  bool on_model(Clingo::Model &model) override {
    ...
    COPOSIT_CLINGO_DIAGNOSTICS("process", current_cardinality_);
  ...
  bool process_subset(const matrix_integer &matrix,
    ...
    if (diagnostics_.active()) {
      const auto [free_indices, upper_size] =
          add_certificate<true>(matrix, control);
  ...
  std::pair<size_t, size_t> add_certificate(const matrix_integer &matrix,
    ...
    if (upper_size > current_cardinality_) {
      ...
      if (upper_size < matrix.rows()) {
        ...
        COPOSIT_CLINGO_DIAGNOSTICS("expiry_guard", upper_size);
      }
      pending_clause_ends_.push_back(pending_clause_bodies_.size());
    } else {
      COPOSIT_CLINGO_DIAGNOSTICS("current_layer_only", upper_size);
    }
    if constexpr (CountSizes)
