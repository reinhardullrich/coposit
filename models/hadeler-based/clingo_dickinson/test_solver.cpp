...
TEST(ClingoDickinsonTest, PreservesStrictDickinsonDecisions) {
...
TEST(ClingoDickinsonTest, DistinguishesNonStrictFromStrictCopositivity) {
...
TEST(ClingoDickinsonTest, ClassifiesBothPredicatesInOneTraversal) {
...
TEST(ClingoDickinsonTest, KeepsSingletonCertificatesAcrossLaterCardinalities) {
  matrix_integer identity;
  identity.set_identity(4);
  clingo_dickinson_diagnostics::clear();
  EXPECT_TRUE(model::solve(identity));

  ASSERT_EQ(clingo_dickinson_diagnostics::events.size(), 4U);
  for (const auto &event : clingo_dickinson_diagnostics::events)
    EXPECT_EQ(event, (clingo_dickinson_diagnostics::event{"process", 1}));
}

TEST(ClingoDickinsonTest,
  ...
  EXPECT_NE(std::find(clingo_dickinson_diagnostics::events.begin(),
                      clingo_dickinson_diagnostics::events.end(),
                      clingo_dickinson_diagnostics::event{"expiry_guard", 2}),
            clingo_dickinson_diagnostics::events.end());
}
...
TEST(ClingoDickinsonTest,
  ...
  EXPECT_EQ(processed, (std::map<size_t, size_t>{{1, 8},
                                                 ...
                                                 {7, 8},
                                                 {8, 1}}));
  EXPECT_NE(std::find_if(clingo_dickinson_diagnostics::events.begin(),
                         clingo_dickinson_diagnostics::events.end(),
                         [](const auto &event) {
...
TEST(ClingoDickinsonTest, AcceptsABoundaryMatrixInNonStrictMode) {
  EXPECT_TRUE(model::solve(symmetric_matrix(2, {1, -2, 4}),
                           model::copositivity_mode::copositive));
}

TEST(ClingoDickinsonTest, HandlesSupportAtomsBeyondOneWord) {
