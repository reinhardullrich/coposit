...
public:
    ...
    bool check(const matrix_integer& matrix)
    {
        ...
        while (low <= matrix.rows()) {
            ...
            if (schur_reduction_ready_) return finish(true);
    ...
    bool has_schur_reduction() const noexcept { return schur_reduction_ready_; }

    matrix_integer take_schur_reduction()
    {
        assert(schur_reduction_ready_);
        schur_reduction_ready_ = false;
        return std::move(schur_matrix_);
    }
    ...
    bool schur_reduction_for_testing(
        const matrix_integer& matrix, const std::vector<size_t>& indices, matrix_integer& reduced)
    {
        indices_ = indices;
        principal_.resize(indices.size(), indices.size());
        solution_.resize(indices.size(), 1);
        ones_solution_ready_ = false;
        copy_principal(matrix, indices_, principal_);
        if (factorization_.factorize_inplace(principal_) == 0 || !factorization_.is_positive_definite()) return false;
        return build_schur_reduction(matrix, reduced);
    }
#endif
...
private:
    ...
    bool process_low_support(const matrix_integer& matrix, size_t& low, size_t high)
    {
        while (low <= high) {
            ...
            COPOSIT_SAT_B4_DIAGNOSTICS("stage_low", low);
    ...
    bool process_high_support(const matrix_integer& matrix, size_t low, size_t& high)
    {
        while (low <= high) {
            ...
            COPOSIT_SAT_B4_DIAGNOSTICS("stage_high", high);
    ...
    bool process_selected_support(const matrix_integer& matrix, pruning_direction direction, bool from_low_frontier)
    {
        ...
        COPOSIT_SAT_B4_DIAGNOSTICS(from_low_frontier ? "process_low" : "process_high", indices_.size());
    ...
    void install_pair_curvature_exclusions(const matrix_integer& matrix)
    {
        ...
        for (size_t first = 0; first < matrix.rows(); ++first) {
            ...
            for (size_t second = first + 1; second < matrix.rows(); ++second) {
                ...
                COPOSIT_SAT_B4_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_SAT_B4_TESTING
        ...
        const matrix_integer& matrix, pruning_direction direction = pruning_direction::upward, bool from_low_frontier = true)
    {
        if (!from_low_frontier) {
            ...
            if (!floating_filter_.looks_positive_semidefinite(indices_)) {
                ...
                COPOSIT_SAT_B4_DIAGNOSTICS("high_float_reject", indices_.size());
#ifdef COPOSIT_SAT_B4_TESTING
        ...
        if (!from_low_frontier) {
            ...
            if (factorization_.is_positive_definite() && try_schur_reduction(matrix)) return true;
    ...
    bool build_schur_reduction(const matrix_integer& matrix, matrix_integer& reduced)
    {
        const size_t outside_dimension = matrix.rows() - indices_.size();
        if (outside_dimension == 0) return false;

        outside_indices_.clear();
        outside_indices_.reserve(outside_dimension);
        size_t selected = 0;
        for (size_t index = 0; index < matrix.rows(); ++index) {
            if (selected < indices_.size() && indices_[selected] == index) ++selected;
            else outside_indices_.push_back(index);
        }
        assert(outside_indices_.size() == outside_dimension);

        prepare_ones_solution();
        integer column_sum;
        for (const size_t outside : outside_indices_) {
            timeout_checkpoint();
            column_sum.set_zero();
            for (size_t local = 0; local < indices_.size(); ++local)
                column_sum.addmul(matrix(outside, indices_[local]), solution_(local, 0));
            if (column_sum.sign() > 0) {
                COPOSIT_SAT_B4_DIAGNOSTICS("schur_prefilter_reject", indices_.size());
                return false;
            }
        }

        schur_coupling_.resize(indices_.size(), outside_dimension);
        for (size_t row = 0; row < indices_.size(); ++row) {
            timeout_checkpoint();
            for (size_t column = 0; column < outside_dimension; ++column) {
                schur_coupling_(row, column) = matrix(indices_[row], outside_indices_[column]);
                schur_coupling_(row, column).negate();
            }
        }

        integer denominator;
        factorization_.solve_inplace(schur_coupling_, denominator, principal_);
        assert(denominator.sign() > 0);
        for (size_t row = 0; row < indices_.size(); ++row)
            for (size_t column = 0; column < outside_dimension; ++column)
                if (schur_coupling_(row, column).sign() < 0) return false;

        reduced.resize(outside_dimension, outside_dimension);
        for (size_t row = 0; row < outside_dimension; ++row) {
            timeout_checkpoint();
            for (size_t column = 0; column <= row; ++column) {
                reduced(row, column).set_product(matrix(outside_indices_[row], outside_indices_[column]), denominator);
                for (size_t local = 0; local < indices_.size(); ++local)
                    reduced(row, column).addmul(matrix(outside_indices_[row], indices_[local]), schur_coupling_(local, column));
                reduced(column, row) = reduced(row, column);
            }
        }
        ...
        for (size_t row = 0; row < outside_dimension; ++row) {
            for (size_t column = 0; column <= row; ++column) {
                fmpz_gcd(next.native_handle(), content.native_handle(), reduced(row, column).native_handle());
                content = next;
                if (content.is_one()) return true;
            }
        }
        if (!content.is_zero()) fmpz_mat_scalar_divexact_fmpz(reduced.native_handle(), reduced.native_handle(), content.native_handle());
        return true;
    }

    bool try_schur_reduction(const matrix_integer& matrix)
    {
        if (!build_schur_reduction(matrix, schur_matrix_)) return false;
        COPOSIT_SAT_B4_DIAGNOSTICS("schur_reduction", indices_.size());
        schur_reduction_ready_ = true;
        return true;
    }
    ...
    bool process_nonsingular_subset(const matrix_integer& matrix)
    {
        ...
        calculate_nonsingular_product(matrix, solution_, 0, ones_denominator_, product_);
        ...
        if (dimension > 1 && current_score_.width + 1 < matrix.rows()) {
            ...
            assert(direction_denominator.compare(ones_denominator_) == 0);
    ...
    void prepare_ones_solution()
    {
        if (ones_solution_ready_) return;
        for (size_t row = 0; row < solution_.rows(); ++row) solution_(row, 0).set_one();
        factorization_.solve_inplace(solution_, ones_denominator_, principal_);
        assert(ones_denominator_.sign() > 0);
        ones_solution_ready_ = true;
    }
...
#endif
            COPOSIT_SAT_B4_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    bool add_certificate()
    {
        ...
        COPOSIT_SAT_B4_DIAGNOSTICS("dickinson", indices_.size());
        support_context_.release(std::move(lower));
        support_context_.release(std::move(upper));
        return true;
    }
    ...
    void add_upward_closure()
    {
        ...
        COPOSIT_SAT_B4_DIAGNOSTICS("support_upward", indices_.size());
    }
    ...
    void add_downward_closure()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_SAT_B4_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_SAT_B4_TESTING
        ++downward_count_;
#endif
    }
    ...
    void add_exact_support()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_SAT_B4_DIAGNOSTICS("exact", indices_.size());
#ifdef COPOSIT_SAT_B4_TESTING
        ++exact_count_;
#endif
    }
    ...
    matrix_integer schur_coupling_;
    matrix_integer schur_matrix_;
    ...
    bool schur_reduction_ready_ = false;
    bool ones_solution_ready_ = false;
#ifdef COPOSIT_SAT_B4_TESTING
...
{
    timeout_checkpoint();
    const matrix_integer* current = &matrix;
    matrix_integer reduced;
    while (true) {
        dickinson_checker checker(current->rows(), mode);
        const bool result = checker.check(*current);
        if (!checker.has_schur_reduction()) return result;
        reduced = checker.take_schur_reduction();
        current = &reduced;
    }
}
...
copositivity_classification classify(const matrix_integer& matrix)
{
    ...
    matrix_integer reduced;
    while (true) {
        dickinson_checker checker(current->rows(), result);
        const bool is_copositive = checker.check(*current);
        if (!checker.has_schur_reduction()) {
            if (!is_copositive) result = {false, false};
            return result;
        }
        reduced = checker.take_schur_reduction();
        current = &reduced;
...
#ifdef COPOSIT_SAT_B4_TESTING
bool sat_b4_prefers_negative_singular_orientation_for_testing(size_t positive_products,
...
size_t sat_b4_combined_ray_sweep_count_for_testing() noexcept
{
    return last_combined_ray_sweep_count;
}

size_t sat_b4_combined_ray_improvement_count_for_testing() noexcept
{
    return last_combined_ray_improvement_count;
}

size_t sat_b4_pair_curvature_exclusion_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t sat_b4_support_curvature_exclusion_count_for_testing() noexcept
{
...
bool sat_b4_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer& matrix, const std::vector<size_t>& indices)
{
...
{
    return dickinson_checker(matrix.rows(), copositivity_mode::copositive)
        .schur_reduction_for_testing(matrix, indices, reduced);
}

size_t sat_b4_shortlist_limit_for_testing(size_t matrix_dimension, size_t support_dimension)
{
    return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool sat_b4_prefers_ray_candidate_for_testing(size_t candidate_upper, size_t candidate_width, size_t candidate_gains,
...
size_t sat_b4_fixed_support_upper_size_for_testing() noexcept
{
...
size_t sat_b4_pair_exclusion_uncovered_count(
...
size_t sat_b4_interval_clause_size(size_t dimension, uint64_t lower_mask, uint64_t upper_mask)
{
    support_context support_context_(dimension);
    interval_sat diagram(support_context_);
    support lower = support_context_.make();
