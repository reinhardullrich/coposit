...
Classification: faithful exact implementation of Dickinson's certificate mathematics, extended with an exact strict-copositivity
mode derived from the paper's minimal-zero results.
...
strict copositivity through Lemma 5.2 and Corollary 5.3. The low-level `solve(A, mode)` returns only the selected Boolean and defaults to
...
Analysis and reference-run interfaces default to combined classification when the mode is omitted. The low-level Boolean `solve`
entry point remains a selected-predicate operation; `classify` is the combined entry point.

The isolated model companion serves both direct `coposit` and Python calls after the caller's selected preprocessing
setting. There is no separate selected or public Dickinson copy.
...
choice, and final proof follows Dickinson's Algorithms 1 and 2. The paper reports that the algorithm had not been implemented, so
there is no author executable or author timing path to reproduce. Increasing-cardinality subset order, numeric-mask order, the
admissible null-vector selection, packed signatures, and exact LDLT are documented implementation choices left open by the paper or
lossless representations of its decisions.
...
Diagnostics reporting distinguishes four counts. `visited` is every support reached by enumeration; `covered` is a support skipped by
...
Timed model-companion builds observe a shared signal flag at principal-subset, certificate, factorization, and matrix-row boundaries
