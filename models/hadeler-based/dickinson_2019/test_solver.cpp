...
TEST(Dickinson2019ModelTest, SourceTracePreservesCertificateCoverage)
{
    ...
    ASSERT_EQ(baseline_source_diagnostics::events.size(), 15U);
    for (size_t i = 0; i < 4; ++i) EXPECT_EQ(baseline_source_diagnostics::events[i], (baseline_source_diagnostics::event{"process", 1}));
    for (size_t i = 4; i < 10; ++i) EXPECT_EQ(baseline_source_diagnostics::events[i], (baseline_source_diagnostics::event{"covered", 2}));
    for (size_t i = 10; i < 14; ++i) EXPECT_EQ(baseline_source_diagnostics::events[i], (baseline_source_diagnostics::event{"covered", 3}));
    EXPECT_EQ(baseline_source_diagnostics::events[14], (baseline_source_diagnostics::event{"covered", 4}));
}
...
TEST(Dickinson2019ModelTest, DecidesCopositivity)
{
    ...
    EXPECT_TRUE(model::solve(constant_off_diagonal(4, 3, -1), copositive));
    EXPECT_FALSE(model::solve(constant_off_diagonal(4, 2, -1), copositive));
}
