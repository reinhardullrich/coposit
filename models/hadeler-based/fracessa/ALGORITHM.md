...
Classification: coposit-created exact CP/SCP model adapted from FracESSA's exact safe candidate search.
...
The candidate search classifies both predicates from one support traversal. Because the internal game is $Q=-A$, an accepted KKT
candidate with positive exact payoff has $x^TAx<0$ and rejects CP and SCP. Zero payoff proves that SCP is false. Strict-only mode stops
at the first zero; CP and combined mode continue because a later KKT candidate may have positive $Q$ payoff and therefore be a
negative witness for $A$. Negative payoff is harmless. A completed traversal proves CP and proves SCP exactly when no zero-payoff
candidate was found.
