...
struct closed_cone_extension_result {
  std::vector<double> point;
  bool feasible = false;
};

class tiny_closed_cone_extension {
public:
  tiny_closed_cone_extension(const std::vector<std::vector<double>> &rows,
                             const std::vector<size_t> &required_rows)
      : rows_(rows), dimension_(rows.empty() ? 0 : rows.front().size()),
        required_rows_(required_rows) {
    if (dimension_ == 0)
      throw std::invalid_argument("empty closed-cone extension problem");
    for (const auto &row : rows_)
      if (row.size() != dimension_)
        throw std::invalid_argument("ragged halfspace matrix");
    for (const size_t row : required_rows_)
      if (row >= rows_.size())
        throw std::invalid_argument("invalid required halfspace row");
  }

  closed_cone_extension_result solve() const {
    closed_cone_extension_result result;
    const size_t constraint_count = required_rows_.size() + 2;
    if (constraint_count + 2 > maximum_tableau_cells / (dimension_ + 2))
      return result;

    tiny_simplex lp(constraint_count, dimension_);
    for (size_t constraint = 0; constraint < required_rows_.size();
         ++constraint) {
      ...
      lp.set_rhs(constraint, 0.0);
    }
    for (size_t column = 0; column < dimension_; ++column) {
      lp.set_coefficient(required_rows_.size(), column, 1.0);
      lp.set_coefficient(required_rows_.size() + 1, column, -1.0);
    }
    lp.set_rhs(required_rows_.size(), 1.0);
    lp.set_rhs(required_rows_.size() + 1, -1.0);

    std::vector<double> solution;
    const tiny_simplex::status status = lp.solve(solution);
    if (status != tiny_simplex::status::optimal)
      return result;
    ...
    for (size_t column = 0; column < dimension_; ++column)
      result.point[column] = std::max(0.0, solution[column]);
    result.feasible = true;
    return result;
  }

private:
  static constexpr size_t maximum_tableau_cells = size_t{256} * 1024;
