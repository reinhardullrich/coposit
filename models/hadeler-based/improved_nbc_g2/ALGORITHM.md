...
Classification: coposit-created exact CP/SCP experiment. Improved NBC-G2 copies Improved NBC-B7 and adds one bounded closed-cone
extension after its exact Halfspace-Rays Dickinson search. The extension tries to enlarge the Rays upper endpoint by allowing a
nonnegative, rather than strictly positive, right-hand side. Floating point only proposes candidates; every accepted vector,
certificate, witness, and classification is verified with exact integers.
...
Improved NBC-G2 uses that observation as a search certificate.
...
- If a low-frontier face is strictly convex, the retained factorization is reused to build and optimize a Dickinson interval. This
  replaces the former exact-support block. G2 then tests every omitted index for a monotone boundary extension that keeps every
  index in the Rays upper endpoint and adds another index.
...
The identifier is `improved_nbc_g2`.

- **Improved NBC** names the separately owned resumable derivative of NBC MiniSat All.
- **G2** is the user-selected name for the second G-line experiment. Here it identifies the closed-cone extension added to the
  Improved NBC engine; it is not a literature citation.
...
machinery, resumable Boolean engine, clauses, and interval compaction. G2 changes only the nonsingular low-frontier Dickinson path by
adding the bounded closed-cone extension described below. Improved NBC-B7 remains unchanged for direct comparison. Dickinson
intervals come from Peter J. C.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Improved NBC-G2 solves
...
$-z\geq0$, the embedded kernel vector is an exact copositive zero. It disproves strict copositivity but not copositivity. On
...
When low-frontier curvature does not already remove the upward closure, Improved NBC-G2 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Improved NBC-G2 performs the inherited exact
...
## Cheap Closed-Cone Extension

Let $F$ be the exact upper endpoint left by Halfspace-Rays, let $O=[n]\setminus I$, and define

$$
P=A_{O,I}B^{-1}.
$$

The all-ones and Rays searches use a strictly positive right-hand side $b>0$. G2 closes that domain to $b\geq0$. Positive scaling
does not change any sign, so G2 normalizes the closed cone to the simplex
...
For one omitted outside index $q\notin F$, the proposed extension solves the floating-point feasibility problem

$$
\begin{aligned}
(Pb)_j&\geq0 &&\text{for every }j\in F\cap O,\\
(Pb)_q&\geq0,\\
b&\geq0,\\
\mathbf1^Tb&=1.
\end{aligned}
...
The first group preserves the complete Rays upper endpoint; the second forces one new index. Every strictly positive right-hand-side
extension feasible for the same $F$ remains feasible after normalization, while boundary points with some $b_i=0$ become available.
Thus this feasibility domain contains the old positive-right-hand-side monotone extension for the fixed root $I$ and endpoint $F$.

The simplex solver has no proof authority. G2 rounds a proposed point at four integer scales, forms

$$
z=B^{-1}b
$$

from the already computed exact inverse directions, recomputes $Az$ exactly, and accepts the proposal only if every old endpoint
index remains nonnegative and at least one omitted index enters. The accepted vector's actual lower endpoint is
$L(z)=\operatorname{supp}(z)$; G2 does not require it to remain equal to the previous lower endpoint. Since $L(z)\subseteq I$ and
$F\subsetneq U(z)$, the new interval always preserves the anchored region $[I,F]$. It may exchange the exceptional coverage below
$I$, which is an intentional upward-first policy.

If an exactly reconstructed $z$ has no positive coordinate, then $y=-z\geq0$ and

$$
y^TBy=z^Tb\leq0.
$$

A negative value is an exact non-copositivity witness. A zero value is an exact failure of strict copositivity; combined mode records
that fact and continues the CP proof. No floating result can trigger either conclusion.

After an accepted extension, G2 recomputes $F=U(z)$ and repeats. Each success strictly enlarges $F$, so there can be at most
$n-|F|$ successful repetitions. A repetition with no exact improvement tests every omitted index and stops. There is no internal
wall-clock budget, making both the work and the timings deterministic for a fixed input and build. The scaled halfspace table and
each simplex tableau are limited to 262,144 entries. Excessive size, numerical infeasibility, or failed exact reconstruction simply
retains the exact Rays certificate. G2 remains a feasibility heuristic rather than a proof that the final endpoint is maximal.
...
Improved NBC-G2 retains SAT-B3's singular case. Suppose
...
3. Ask the low frontier for one open support. Copy and exactly factor its principal matrix. Install an upward closure immediately
   ...
   interval. If its upper endpoint is not full, run the bounded closed-cone extension and retain the final exactly verified interval.
...
All proof-producing matrix entries, factorization state, right-hand-side directions, breakpoint comparisons, reconstructed
closed-cone coefficients, products, inertia signs, kernel vectors, and witnesses use arbitrary-precision integers. Binary64 is used
only to nominate high supports for exact positive-semidefiniteness verification and to propose a closed-cone point. A floating
rejection is not stored and cannot affect the low proof or final classification.
...
high rejections, closed-cone improvements, pair-upward, support-upward, Dickinson, and downward certificates. When diagnostics capture is enabled, the model
...
Improved NBC-G2 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
...
The closed-cone stage can add overhead at every bounded nonsingular low support while finding no exactly reconstructible boundary
improvement. It tests every currently omitted target, so a large support with weak extensions can require many floating feasibility
solves. Boundary faces can also be numerically delicate, so the proposal may miss a mathematically feasible extension; that miss
affects only speed and coverage, never correctness.
