...
#ifdef COPOSIT_SAT_HALFSPACE_RAYS_WIDE_DICKINSON_TESTING
    ...
    bool check(const matrix_integer& matrix)
    {
        ...
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            while (supports_->take_first(indices_)) {
                ...
                COPOSIT_SAT_HALFSPACE_RAYS_WIDE_DIAGNOSTICS("process", subset_dimension);
...
private:
...
#endif
            COPOSIT_SAT_HALFSPACE_RAYS_WIDE_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    bool add_certificate()
    {
        ...
        const bool accepted = uses_full_certificate_interval(product_.size(), indices_.size(), free_indices);
        if (accepted) {
        ...
        if (diagnostics_.active()) {
            ...
            if (accepted) diagnostics_.accepted_certificate(free_indices, upper_size);
        }
        return true;
    }
