...
TEST(SatC2Test, RunsTheInitialCurvatureWalksBeforeOrdinaryTraversal)
{
    sat_c2_diagnostics::clear();
    ...
    EXPECT_EQ(model::sat_c2_pair_upward_count_for_testing(), 0U);
    EXPECT_NE(std::find(sat_c2_diagnostics::events.begin(), sat_c2_diagnostics::events.end(),
                       sat_c2_diagnostics::event{"walk_downward", 3}),
              sat_c2_diagnostics::events.end());
}

TEST(SatC2Test, NeverWalksThroughAGloballyCoveredSuccessor)
{
    const matrix_integer matrix = symmetric_matrix(3, {1, -2, -1, 1, 0, 1});
    EXPECT_EQ(model::sat_c2_floating_successor_for_testing(matrix, {0}, {{0, 1}}),
              (std::vector<size_t>{0, 2}));
}

TEST(SatC2Test, UsesOneFixedAlternatingWalkSchedule)
{
    EXPECT_EQ(model::sat_c2_walk_schedule_for_testing(10), (std::array<size_t, 5>{9, 1, 0, 0, 1}));
}

TEST(SatC2Test, FloatingFilterOnlyProposesExactPositiveSemidefinitenessChecks)
{
    EXPECT_TRUE(model::sat_c2_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {2, 0, 0, 3, 0, 4}), {0, 1, 2}));
    EXPECT_TRUE(model::sat_c2_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {5, -1, 2, 5, 2, 2}), {0, 1, 2}));
    EXPECT_FALSE(model::sat_c2_floating_psd_candidate_for_testing(
        symmetric_matrix(3, {1, 0, 0, -1, 0, 1}), {0, 1, 2}));
    EXPECT_FALSE(model::sat_c2_floating_psd_candidate_for_testing(
        symmetric_matrix(2, {0, 1, 0}), {0, 1}));
}

TEST(SatC2Test, FloatingFilterUsesTheSelectedSubmatrixScale)
{
    const matrix_integer matrix = symmetric_matrix(3, {1'000'000'000'000'000'000L, 0, 0, 1, 0, 1});
    EXPECT_TRUE(model::sat_c2_floating_psd_candidate_for_testing(matrix, {1, 2}));
}

TEST(SatC2Test, BuffersClosuresUntilTheBoundedWalkEnds)
{
    const matrix_integer matrix = symmetric_matrix(3, {1, 2, 0, 1, 0, 1});
    const auto [visited, useful, seed_available] = model::sat_c2_curvature_walk_for_testing(matrix, {0, 1});
...
TEST(SatC2Test, FloatingRejectionsRemainVisibleToTheExactLowFrontier)
{
    const std::vector<std::vector<size_t>> rejected{{0, 1}};
    EXPECT_EQ(model::sat_c2_high_filter_uncovered_count(3, 2, rejected, true), 2U);
    EXPECT_EQ(model::sat_c2_high_filter_uncovered_count(3, 2, rejected, false), 3U);
}

TEST(SatC2Test, BuildsAnExactDickinsonIntervalOnTheLowPath)
{
    ...
    EXPECT_TRUE(model::sat_c2_certificate_for_testing(identity, {0}, lower, upper));
...
TEST(SatC2Test, RetainsTheExactHalfspaceRayOptimization)
{
    ...
    for (uint64_t mask = 1; mask < 16 && !improved; ++mask) {
        ...
        (void)model::sat_c2_check_support_for_testing(matrix, indices);
        improved = model::sat_c2_optimized_certificate_count_for_testing() > 0;
    }
    EXPECT_TRUE(improved);
}

TEST(SatC2Test, SchurResidualMatchesTheOriginalReducedHessian)
{
    ...
    for (uint64_t optional_mask = 0; optional_mask < 4; ++optional_mask) {
        ...
        EXPECT_EQ(model::sat_c2_schur_curvature_for_testing(matrix, lower, upper, query),
                  model::sat_c2_reduced_hessian_is_positive_definite_for_testing(matrix, query));
    }
    ...
    for (uint64_t optional_mask = 0; optional_mask < 4; ++optional_mask) {
        ...
        EXPECT_EQ(model::sat_c2_schur_curvature_for_testing(identity, larger_lower, larger_upper, query),
                  model::sat_c2_reduced_hessian_is_positive_definite_for_testing(identity, query));
    }
}

TEST(SatC2Test, SchurResidualMatchesNontrivialExactCrossBlocks)
{
...
TEST(SatC2Test, FindsAHigherOrderCurvatureCoreInsideAnInterval)
{
    const matrix_integer matrix = symmetric_matrix(3, {1, -2, 0, 1, 0, 0});
    const auto cores = model::sat_c2_curvature_boundary_for_testing(matrix, {2}, {0, 1, 2});

    EXPECT_EQ(cores, (std::vector<std::vector<size_t>>{{0, 1, 2}}));
    EXPECT_EQ(model::sat_c2_local_curvature_search_count_for_testing(), 1U);
    EXPECT_EQ(model::sat_c2_local_curvature_core_count_for_testing(), 1U);
    EXPECT_GT(model::sat_c2_local_curvature_exact_query_count_for_testing(), 0U);
}

TEST(SatC2Test, KeepsOneSmallCurvatureCoreFromEachDickinsonInterval)
{
    matrix_integer matrix(6, 6);
    for (size_t row = 0; row < 6; ++row) {
        for (size_t column = row; column < 6; ++column) {
            const slong value = row == column ? static_cast<slong>(row != 0) : static_cast<slong>(row == 0 ? 0 : 2);
            matrix(row, column) = integer(value);
            matrix(column, row) = matrix(row, column);
        }
    }

    const auto cores = model::sat_c2_curvature_boundary_for_testing(matrix, {0}, {0, 1, 2, 3, 4, 5});

    ASSERT_EQ(cores.size(), 1U);
    EXPECT_EQ(cores.front().size(), 3U);
    EXPECT_TRUE(std::binary_search(cores.front().begin(), cores.front().end(), 0U));
    EXPECT_EQ(model::sat_c2_local_curvature_core_count_for_testing(), 1U);
    EXPECT_LE(model::sat_c2_local_curvature_exact_query_count_for_testing(), 5U);
}

TEST(SatC2Test, ExactGoodUpperEndpointSkipsTheCurvatureChain)
{
    ...
    EXPECT_EQ(model::sat_c2_local_curvature_search_count_for_testing(), 0U);
    EXPECT_EQ(model::sat_c2_local_curvature_exact_query_count_for_testing(), 1U);
}

TEST(SatC2Test, StoresDickinsonIntervalsInTheSharedSatState)
{
    const std::vector<std::pair<uint64_t, uint64_t>> intervals{{0b001, 0b011}, {0b100, 0b110}};
    EXPECT_EQ(model::sat_c2_uncovered_count(3, 1, intervals), 1U);
    EXPECT_EQ(model::sat_c2_uncovered_count(3, 2, intervals), 1U);
}

TEST(SatC2Test, EncodesUpwardDownwardAndExactSupportBlocks)
{
    const std::vector<std::vector<size_t>> none;
    const std::vector<std::vector<size_t>> support_01{{0, 1}};

    EXPECT_EQ(model::sat_c2_uncovered_count(3, 1, support_01, none, none), 3U);
    EXPECT_EQ(model::sat_c2_uncovered_count(3, 2, support_01, none, none), 2U);
    EXPECT_EQ(model::sat_c2_uncovered_count(3, 3, support_01, none, none), 0U);

    EXPECT_EQ(model::sat_c2_uncovered_count(3, 1, none, support_01, none), 1U);
    EXPECT_EQ(model::sat_c2_uncovered_count(3, 2, none, support_01, none), 2U);
    EXPECT_EQ(model::sat_c2_uncovered_count(3, 3, none, support_01, none), 1U);

    EXPECT_EQ(model::sat_c2_uncovered_count(3, 1, none, none, support_01), 3U);
    EXPECT_EQ(model::sat_c2_uncovered_count(3, 2, none, none, support_01), 2U);
    EXPECT_EQ(model::sat_c2_uncovered_count(3, 3, none, none, support_01), 1U);
}

TEST(SatC2Test, PreservesExactCombinedClassifications)
{
...
TEST(SatC2Test, ExhaustivelyClassifiesSmallTwoByTwoIntegerMatrices)
{
...
TEST(SatC2Test, MatchesTheIndependentExactThreeByThreeCriterion)
{
...
TEST(SatC2Test, AppliesPairCurvaturePrepass)
{
    ...
    EXPECT_EQ(model::sat_c2_pair_upward_count_for_testing(), 1U);
}
