...
#ifdef COPOSIT_IMPROVED_NBC_X2_TESTING
...
private:
  ...
  void record_interval_certificate(const support &lower, const support &upper) {
    ...
    event << "model=improved_nbc_x2 n=" << product_.size()
  ...
  bool process_low_support(const matrix_integer &matrix, size_t &low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X2_DIAGNOSTICS("stage_low", low);
  ...
  bool process_high_support(const matrix_integer &matrix, size_t low,
    ...
    while (low <= high) {
      ...
      COPOSIT_IMPROVED_NBC_X2_DIAGNOSTICS("stage_high", high);
  ...
  bool process_selected_support(const matrix_integer &matrix,
    ...
    COPOSIT_IMPROVED_NBC_X2_DIAGNOSTICS(
  ...
  void install_pair_curvature_exclusions(const matrix_integer &matrix) {
    ...
    for (size_t first = 0; first < matrix.rows(); ++first) {
      ...
      for (size_t second = first + 1; second < matrix.rows(); ++second) {
        ...
        COPOSIT_IMPROVED_NBC_X2_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_IMPROVED_NBC_X2_TESTING
  ...
  bool process_subset(const matrix_integer &matrix,
    ...
    if (!from_low_frontier) {
      ...
      if (!floating_filter_.looks_positive_semidefinite(indices_)) {
        ...
        COPOSIT_IMPROVED_NBC_X2_DIAGNOSTICS("high_float_reject",
                                            indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X2_TESTING
...
#endif
      COPOSIT_IMPROVED_NBC_X2_DIAGNOSTICS("combined_ray", ray + 1);
  ...
  void shrink_lower_preserving_upper(const matrix_integer &matrix) {
    ...
    do {
      ...
      for (size_t local = 0; local < solution_.rows(); ++local) {
        ...
        if (coefficient.is_zero() || (coefficient.sign() > 0 && positive_entries == 1))
          continue;

        bool preserves_upper = true;
        for (size_t row = 0; row < product_.size(); ++row) {
          candidate_product_[row] = product_[row];
          candidate_product_[row].submul(matrix(row, indices_[local]), coefficient);
  ...
  bool add_certificate() {
    ...
    support_context_.release(std::move(lower));
    support_context_.release(std::move(upper));
    COPOSIT_IMPROVED_NBC_X2_DIAGNOSTICS("dickinson", indices_.size());
    return true;
  }
  ...
  void add_upward_closure() {
    ...
    COPOSIT_IMPROVED_NBC_X2_DIAGNOSTICS("support_upward", indices_.size());
  }
  ...
  void add_downward_closure(std::string_view kind) {
    ...
    record_closure_certificate(kind, "downward", certificate_frontier_,
                               indices_);
    COPOSIT_IMPROVED_NBC_X2_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_IMPROVED_NBC_X2_TESTING
    ++downward_count_;
#endif
  }
...
#ifdef COPOSIT_IMPROVED_NBC_X2_TESTING
bool improved_nbc_x2_prefers_negative_singular_orientation_for_testing(
...
size_t improved_nbc_x2_combined_ray_sweep_count_for_testing() noexcept {
  return last_combined_ray_sweep_count;
}

size_t improved_nbc_x2_combined_ray_improvement_count_for_testing() noexcept {
  return last_combined_ray_improvement_count;
}

size_t improved_nbc_x2_pair_curvature_exclusion_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t
improved_nbc_x2_support_curvature_exclusion_count_for_testing() noexcept {
...
bool improved_nbc_x2_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
size_t improved_nbc_x2_shortlist_limit_for_testing(size_t matrix_dimension,
                                                   size_t support_dimension) {
  return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool improved_nbc_x2_prefers_ray_candidate_for_testing(
...
size_t improved_nbc_x2_fixed_support_upper_size_for_testing() noexcept {
...
size_t improved_nbc_x2_pair_exclusion_uncovered_count(size_t dimension,
                                                      size_t cardinality,
                                                      size_t first,
                                                      size_t second) {
  support_context context(dimension);
...
size_t improved_nbc_x2_interval_clause_size(size_t dimension,
                                            uint64_t lower_mask,
                                            uint64_t upper_mask) {
  size_t lower_size = 0;
  size_t upper_size = 0;
