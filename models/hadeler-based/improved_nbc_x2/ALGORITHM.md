...
Classification: coposit-created exact CP/SCP experiment. Improved NBC-X2 alternates between low and high cardinalities. The low frontier uses
...
optimized certificate is then enlarged once more by greedily deleting lower-endpoint coordinates without shrinking its exact upper
endpoint. The high frontier is an opportunistic floating-point positive-semidefiniteness scan; only candidates for downward pruning
...
Improved NBC-X2 uses that observation as a search certificate.
...
- If a low-frontier face is strictly convex, the retained factorization is reused to build and optimize a Dickinson interval. This
  replaces the former exact-support block. Afterward, X2 tries to set used vector coordinates to zero one at a time. A deletion is
  retained only when exact recomputation keeps every old Dickinson upper index and at least one positive vector coordinate remains.
  Newly nonnegative products enlarge the upper endpoint for free.
...
The identifier is `improved_nbc_x2`.

- **Improved NBC** names the separately owned resumable derivative of NBC MiniSat All.
- **X2** names the experiment that shrinks the lower endpoint after optimizing the upper endpoint.

The model is an independent copy of [`improved_nbc_b7`](../improved_nbc_b7/ALGORITHM.md). It preserves Improved NBC-B7's two-frontier traversal, floating
high-frontier screen, exact nonsingular and singular downward tests, low-frontier Halfspace-Rays machinery, Boolean clauses, and
interval compaction. Its only change is an exact postprocessing step on nonsingular Halfspace-Rays certificates: it greedily removes
used coordinates without shrinking the optimized upper endpoint. `improved_nbc_b7` remains unchanged for direct comparison.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Improved NBC-X2 solves
...
When low-frontier curvature does not already remove the upward closure, Improved NBC-X2 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Improved NBC-X2 performs the inherited exact
...
cardinality: X2 deliberately avoids an LP, MILP, or subset search. Every attempted and accepted calculation uses exact integers.
...
Improved NBC-X2 retains SAT-B3's singular case. Suppose
...
3. Ask the low frontier for one open support. Copy and exactly factor its principal matrix. Install an upward closure immediately
   ...
   a nonsingular Dickinson certificate, greedily delete lower-endpoint coordinates while exact recomputation keeps every old upper
   index.
...
Improved NBC-X2 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
