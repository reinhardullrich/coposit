...
public:
    ...
    bool check(const matrix_integer& matrix)
    {
        const bool result = supports_.generate([&](const support& current, size_t cardinality) {
            ...
            COPOSIT_SAT_C3_DIAGNOSTICS("process", cardinality);
...
private:
    bool process_subset(const matrix_integer& matrix)
    {
        ...
        const bool curvature_certificate = singular
            ? !singular_reduced_hessian_is_positive_definite()
            : !nonsingular_reduced_hessian_is_positive_definite();
        if (curvature_certificate) add_curvature_certificate(matrix.rows());
        add_ceiling_certificate(matrix, curvature_certificate);
        return true;
    }
    ...
    void add_curvature_certificate(size_t matrix_dimension)
    {
        ...
        COPOSIT_SAT_C3_DIAGNOSTICS("curvature-certificate", indices_.size());
    }

    void add_ceiling_certificate(const matrix_integer& matrix, bool curvature_certificate)
    {
        ...
        for (size_t row = 0; row < matrix.rows(); ++row) {
            ...
            if (product_.sign() < 0) {
                COPOSIT_SAT_C3_DIAGNOSTICS("discard-certificate", indices_.size());
                return;
            }
        }
        ...
        if (curvature_certificate && lower_size == indices_.size()) {
            COPOSIT_SAT_C3_DIAGNOSTICS("duplicate-ceiling-certificate", lower_size);
            return;
        }

        supports_.add_forbidden(std::move(lower));
        diagnostics_.certificate(matrix.rows() - lower_size);
        COPOSIT_SAT_C3_DIAGNOSTICS("ceiling-certificate", lower_size);
    }
