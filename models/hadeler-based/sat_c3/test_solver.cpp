...
std::vector<uint64_t> sat_c3_generated_masks(size_t dimension, uint64_t forbidden_trigger);
}
...
TEST(SatC3Test, PreservesStrictAndCopositiveDecisions)
{
...
TEST(SatC3Test, ClassifiesBothPredicatesInOneTraversal)
{
...
TEST(SatC3Test, MatchesTheIndependentExactThreeByThreeCriterion)
{
...
TEST(SatC3Test, KeepsTriggerButSkipsEveryLargerSuperset)
{
    const std::vector<uint64_t> generated = model::sat_c3_generated_masks(4, 0b0101);
...
TEST(SatC3Test, ProcessesEachSupportOnceWithoutExactSupportClauses)
{
    std::vector<uint64_t> generated = model::sat_c3_generated_masks(4, 0);
    EXPECT_EQ(generated.size(), 15U);
    std::sort(generated.begin(), generated.end());
    EXPECT_EQ(std::adjacent_find(generated.begin(), generated.end()), generated.end());
}

TEST(SatC3Test, StoresOnlyDickinsonCertificatesThatReachTheCeiling)
{
    sat_c3_diagnostics::clear();
    EXPECT_TRUE(model::solve(symmetric_matrix(2, {2, -1, 2})));

    EXPECT_EQ(std::count(sat_c3_diagnostics::events.begin(), sat_c3_diagnostics::events.end(),
                         sat_c3_diagnostics::event{"discard-certificate", 1}),
              2);
    EXPECT_EQ(std::count(sat_c3_diagnostics::events.begin(), sat_c3_diagnostics::events.end(),
                         sat_c3_diagnostics::event{"ceiling-certificate", 2}),
              1);
}

TEST(SatC3Test, ChecksDickinsonAfterFindingAnExactCurvatureCertificate)
{
    sat_c3_diagnostics::clear();
    EXPECT_TRUE(model::solve(symmetric_matrix(3, {4, 5, -1, 4, -1, 4})));

    const auto curvature = std::find(sat_c3_diagnostics::events.begin(), sat_c3_diagnostics::events.end(),
                                     sat_c3_diagnostics::event{"curvature-certificate", 2});
    ASSERT_NE(curvature, sat_c3_diagnostics::events.end());
    ASSERT_NE(std::next(curvature), sat_c3_diagnostics::events.end());
    EXPECT_EQ(*std::next(curvature), (sat_c3_diagnostics::event{"discard-certificate", 2}));
}

TEST(SatC3Test, PublishesDiagnosticsAndCeilingCertificateDiagnostics)
{
...
TEST(SatC3Test, RejectsANonCopositiveMatrix)
{
    EXPECT_FALSE(model::solve(symmetric_matrix(2, {1, -2, 2}), model::copositivity_mode::copositive));
}

TEST(SatC3Test, KeepsPackedSupportsBeyondOneWord)
{
