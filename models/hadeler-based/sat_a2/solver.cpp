...
#include "tiny_relaxed_maximum_halfspaces.hpp"
...
size_t last_lp_relaxation_count = 0;
size_t last_lp_candidate_improvement_count = 0;
size_t last_lp_followup_count = 0;
size_t last_lp_followup_improvement_count = 0;
size_t last_lp_estimated_upper_bound = 0;
size_t last_lp_incumbent_upper_size = 0;
size_t last_lp_final_upper_size = 0;
#endif
...
public:
    ...
    bool check(const matrix_integer& matrix)
    {
        ...
        lp_relaxation_count_ = 0;
        lp_candidate_improvement_count_ = 0;
        lp_followup_count_ = 0;
        lp_followup_improvement_count_ = 0;
        lp_estimated_upper_bound_ = 0;
        lp_incumbent_upper_size_ = 0;
        lp_final_upper_size_ = 0;
#endif
        ...
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            while (supports_->take_first(indices_)) {
                ...
                COPOSIT_SAT_A2_DIAGNOSTICS("process", subset_dimension);
...
private:
    ...
    bool process_nonsingular_subset(const matrix_integer& matrix)
    {
        ...
        if (dimension > 1 && current_score_.width + 1 < matrix.rows()) {
            ...
            if (current_score_.upper_size < matrix.rows() && !try_lp_relaxation()) return false;
        }
    ...
    bool try_lp_relaxation()
    {
        ...
        const std::vector<bool> no_forced_rows;
        detail::tiny_relaxed_maximum_halfspaces relaxation(lp_scaled_rows_, positive_floor, no_forced_rows, deadline);
        const detail::relaxed_maximum_halfspaces_result root = relaxation.solve();
#ifdef COPOSIT_SAT_A2_TESTING
        ++lp_relaxation_count_;
        lp_incumbent_upper_size_ = current_score_.upper_size;
#endif
        if (!root.optimal || !std::isfinite(root.upper_bound) || root.upper_bound < 0.0 ||
            root.row_values.size() != outside_dimension)
            return true;

        const double bounded_upper = std::min(root.upper_bound, static_cast<double>(outside_dimension));
        const size_t estimated_outside_upper = static_cast<size_t>(std::floor(bounded_upper + 1e-6));
        const size_t estimated_total_upper = support_dimension + estimated_outside_upper;
#ifdef COPOSIT_SAT_A2_TESTING
        lp_estimated_upper_bound_ = std::max(lp_estimated_upper_bound_, estimated_total_upper);
#endif
        const exact_lp_candidate_result root_candidate = consider_lp_point(root.point);
        if (root_candidate == exact_lp_candidate_result::negative_witness) return false;
#ifdef COPOSIT_SAT_A2_TESTING
        if (root_candidate == exact_lp_candidate_result::improved) ++lp_candidate_improvement_count_;
        lp_final_upper_size_ = current_score_.upper_size;
#endif

        // ponytail: one monotone LP-guided extension only; add conflict extraction or branching only if this measured probe pays.
        if (estimated_total_upper <= current_score_.upper_size + 1) return true;
        size_t target = outside_dimension;
        double target_value = -1.0;
        for (size_t candidate = 0; candidate < outside_dimension; ++candidate) {
            if (product_[lp_outside_rows_[candidate]].sign() >= 0) continue;
            if (root.row_values[candidate] > target_value) {
                target = candidate;
                target_value = root.row_values[candidate];
            }
        }
        if (target == outside_dimension) return true;

        lp_forced_rows_.assign(outside_dimension, false);
        for (size_t row = 0; row < outside_dimension; ++row)
            lp_forced_rows_[row] = product_[lp_outside_rows_[row]].sign() >= 0;
        lp_forced_rows_[target] = true;
        detail::tiny_relaxed_maximum_halfspaces followup(lp_scaled_rows_, positive_floor, lp_forced_rows_, deadline);
        const detail::relaxed_maximum_halfspaces_result extension = followup.solve();
#ifdef COPOSIT_SAT_A2_TESTING
        ++lp_followup_count_;
#endif
        if (!extension.optimal) return true;
        const exact_lp_candidate_result extension_candidate = consider_lp_point(extension.point, &lp_forced_rows_);
        if (extension_candidate == exact_lp_candidate_result::negative_witness) return false;
#ifdef COPOSIT_SAT_A2_TESTING
        if (extension_candidate == exact_lp_candidate_result::improved) ++lp_followup_improvement_count_;
        lp_final_upper_size_ = current_score_.upper_size;
#endif
        return true;
    }

    exact_lp_candidate_result consider_lp_point(const std::vector<double>& point,
                                                 const std::vector<bool>* required_outside_rows = nullptr)
    {
        ...
        for (const uint64_t scale :
            ...
            for (size_t column = 0; column < point.size(); ++column) {
                const double value = point[column];
                if (!(value > 0.0) || !std::isfinite(value) ||
                    value > static_cast<double>(std::numeric_limits<long long>::max()) / static_cast<double>(scale)) {
                    positive = false;
                    break;
                }
            ...
            if (required_outside_rows != nullptr) {
                assert(required_outside_rows->size() == lp_outside_rows_.size());
                bool preserves_required_rows = true;
                for (size_t row = 0; row < required_outside_rows->size(); ++row) {
                    if ((*required_outside_rows)[row] && lp_product_[lp_outside_rows_[row]].sign() < 0) {
...
#endif
            COPOSIT_SAT_A2_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    void publish_test_counters() const noexcept
    {
        ...
        last_lp_relaxation_count = lp_relaxation_count_;
        last_lp_candidate_improvement_count = lp_candidate_improvement_count_;
        last_lp_followup_count = lp_followup_count_;
        last_lp_followup_improvement_count = lp_followup_improvement_count_;
        last_lp_estimated_upper_bound = lp_estimated_upper_bound_;
        last_lp_incumbent_upper_size = lp_incumbent_upper_size_;
        last_lp_final_upper_size = lp_final_upper_size_;
    }
#endif
    ...
    std::vector<bool> lp_forced_rows_;
    ...
    size_t lp_relaxation_count_ = 0;
    size_t lp_candidate_improvement_count_ = 0;
    size_t lp_followup_count_ = 0;
    size_t lp_followup_improvement_count_ = 0;
    size_t lp_estimated_upper_bound_ = 0;
    size_t lp_incumbent_upper_size_ = 0;
    size_t lp_final_upper_size_ = 0;
...
size_t sat_a2_lp_relaxation_count_for_testing() noexcept { return last_lp_relaxation_count; }
size_t sat_a2_lp_candidate_improvement_count_for_testing() noexcept { return last_lp_candidate_improvement_count; }
size_t sat_a2_lp_followup_count_for_testing() noexcept { return last_lp_followup_count; }
size_t sat_a2_lp_followup_improvement_count_for_testing() noexcept { return last_lp_followup_improvement_count; }
size_t sat_a2_lp_estimated_upper_bound_for_testing() noexcept { return last_lp_estimated_upper_bound; }
size_t sat_a2_lp_incumbent_upper_size_for_testing() noexcept { return last_lp_incumbent_upper_size; }
size_t sat_a2_lp_final_upper_size_for_testing() noexcept { return last_lp_final_upper_size; }
