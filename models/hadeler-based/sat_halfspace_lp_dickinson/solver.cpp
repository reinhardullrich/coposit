...
public:
    ...
    tiny_simplex(size_t rows, size_t columns)
        : rows_(rows)
        , columns_(columns)
        , tableau_(tableau_size(rows, columns), 0.0)
        , basic_(rows)
        , nonbasic_(columns + 1)
    {
        if (rows_ == 0 || columns_ == 0) throw std::invalid_argument("simplex tableau must be nonempty");
    ...
    status solve(std::vector<double>& solution)
    {
        ...
        if (at(pivot_row, columns_ + 1) < -epsilon) {
            ...
            if (!run_simplex(true) || at(rows_ + 1, columns_ + 1) < -epsilon) return status::infeasible;
            if (std::abs(at(rows_ + 1, columns_ + 1)) > epsilon) return status::infeasible;
            for (size_t row = 0; row < rows_; ++row) {
        ...
        if (!run_simplex(false)) return status::unbounded;
...
private:
    ...
    static size_t tableau_size(size_t rows, size_t columns)
    {
        const size_t maximum_index = static_cast<size_t>(std::numeric_limits<int>::max());
        if (columns > maximum_index || rows > maximum_index - columns)
            throw std::overflow_error("simplex variable count exceeds its integer index range");
        if (columns > std::numeric_limits<size_t>::max() - 2 || rows > std::numeric_limits<size_t>::max() - 2 ||
    ...
    bool run_simplex(bool phase_one)
    {
        ...
        while (true) {
            ...
            for (size_t row = 0; row < rows_; ++row) {
                ...
                const double current_ratio = at(pivot_row, columns_ + 1) / at(pivot_row, pivot_column);
                if (ratio < current_ratio - epsilon ||
                    (std::abs(ratio - current_ratio) <= epsilon && basic_[row] < basic_[pivot_row]))
                    pivot_row = row;
            }
            if (pivot_row == rows_) return false;
            if (pivots_ != 0 && (pivots_ & 255U) == 0) timeout_checkpoint();
...
enum class full_ceiling_lp_result { not_found, certificate, negative_witness };
...
size_t last_full_ceiling_lp_hit_count = 0;
#endif
...
private:
    ...
    bool process_nonsingular_subset(const matrix_integer& matrix)
    {
        ...
        if (dimension > 1 && current_score_.width + 1 < matrix.rows()) {
            ...
            for (size_t direction = 0; direction < dimension; ++direction)
                calculate_nonsingular_product(matrix, directions_, direction, direction_denominator, direction_products_, direction);

            if (current_score_.upper_size < matrix.rows()) {
                const full_ceiling_lp_result lp_result = try_full_ceiling_lp();
                if (lp_result == full_ceiling_lp_result::negative_witness) return false;
                if (lp_result == full_ceiling_lp_result::certificate) return add_certificate();
            }
    ...
    full_ceiling_lp_result try_full_ceiling_lp()
    {
        timeout_checkpoint();
        const size_t support_dimension = indices_.size();
        lp_outside_rows_.clear();
        lp_row_exponents_.clear();
        lp_outside_rows_.reserve(direction_products_.rows() - support_dimension);
        lp_row_exponents_.reserve(direction_products_.rows() - support_dimension);
        ...
        for (size_t row = 0; row < direction_products_.rows(); ++row) {
            ...
            lp_row_exponents_.push_back(maximum_exponent);
        }

        tiny_simplex lp(lp_outside_rows_.size() + support_dimension + 2, support_dimension + 1);
        lp.set_objective(support_dimension, 1.0);
        for (size_t column = 0; column < support_dimension; ++column) {
            for (size_t outside = 0; outside < lp_outside_rows_.size(); ++outside) {
                const auto entry = direction_products_(lp_outside_rows_[outside], column);
                if (entry.is_zero()) continue;
                slong exponent = 0;
                const double mantissa = entry.to_dbl_2exp(exponent);
                const slong difference = exponent - lp_row_exponents_[outside];
                const double scaled = difference < -1074 ? 0.0 : std::scalbn(mantissa, static_cast<int>(difference));
                if (scaled != 0.0) lp.set_coefficient(outside, column, -scaled);
            }
            lp.set_coefficient(lp_outside_rows_.size() + column, column, -1.0);
            lp.set_coefficient(lp_outside_rows_.size() + column, support_dimension, 1.0);
            lp.set_coefficient(lp_outside_rows_.size() + support_dimension, column, 1.0);
            lp.set_coefficient(lp_outside_rows_.size() + support_dimension + 1, column, -1.0);
        }
        lp.set_rhs(lp_outside_rows_.size() + support_dimension, 1.0);
        lp.set_rhs(lp_outside_rows_.size() + support_dimension + 1, -1.0);

        if (lp.solve(lp_solution_double_) != tiny_simplex::status::optimal || !(lp_solution_double_[support_dimension] > 1e-10))
            return full_ceiling_lp_result::not_found;

        lp_coefficients_.resize(support_dimension);
        lp_solution_exact_.resize(support_dimension, 1);
        lp_product_exact_.resize(direction_products_.rows());
        for (const uint64_t scale : {uint64_t{1000000}, uint64_t{1000000000}, uint64_t{1000000000000}, uint64_t{1000000000000000}}) {
            bool positive = true;
            for (size_t column = 0; column < support_dimension; ++column) {
                const double value = lp_solution_double_[column];
            ...
            bool exact = true;
            for (size_t row = 0; row < direction_products_.rows(); ++row) {
                lp_product_exact_[row].set_zero();
                for (size_t column = 0; column < support_dimension; ++column)
                    lp_product_exact_[row].addmul(direction_products_(row, column), lp_coefficients_[column]);
                exact &= lp_product_exact_[row].sign() >= 0;
            }
            if (!exact) continue;

            for (size_t row = 0; row < support_dimension; ++row) {
                lp_solution_exact_(row, 0).set_zero();
                for (size_t column = 0; column < support_dimension; ++column)
                    lp_solution_exact_(row, 0).addmul(directions_(row, column), lp_coefficients_[column]);
            }
            solution_ = lp_solution_exact_;
            product_ = lp_product_exact_;
            current_score_ = score(solution_, 0, product_);
            assert(current_score_.upper_size == product_.size());
#ifdef COPOSIT_SAT_HALFSPACE_RAYS_DICKINSON_TESTING
            ++full_ceiling_lp_hit_count_;
#endif
            if (all_nonpositive(solution_, 0)) return full_ceiling_lp_result::negative_witness;
            return full_ceiling_lp_result::certificate;
        }
        return full_ceiling_lp_result::not_found;
    }
    ...
    void publish_test_counters() const noexcept
    {
        ...
        last_full_ceiling_lp_hit_count = full_ceiling_lp_hit_count_;
    }
#endif
    ...
    matrix_integer lp_solution_exact_;
    std::vector<integer> product_;
    std::vector<integer> lp_coefficients_;
    std::vector<integer> lp_product_exact_;
    std::vector<double> lp_solution_double_;
    std::vector<size_t> lp_outside_rows_;
    std::vector<slong> lp_row_exponents_;
    ...
    size_t full_ceiling_lp_hit_count_ = 0;
...
size_t sat_halfspace_lp_full_ceiling_hit_count_for_testing() noexcept
{
    return last_full_ceiling_lp_hit_count;
}
