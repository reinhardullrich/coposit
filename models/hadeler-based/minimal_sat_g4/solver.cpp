...
private:
  ...
  void record_interval_certificate(const support &lower, const support &upper) {
    ...
    event << "model=minimal_sat_g4 n=" << product_.size()
  ...
  bool process_low_support(const matrix_integer &matrix, size_t &low,
    ...
    while (low <= high) {
      ...
      COPOSIT_MINIMAL_SAT_G4_DIAGNOSTICS("stage_low", low);
  ...
  bool process_high_support(const matrix_integer &matrix, size_t low,
    ...
    while (low <= high) {
      ...
      COPOSIT_MINIMAL_SAT_G4_DIAGNOSTICS("stage_high", high);
  ...
  bool process_selected_support(const matrix_integer &matrix,
    ...
    COPOSIT_MINIMAL_SAT_G4_DIAGNOSTICS(
  ...
  void install_pair_curvature_exclusions(const matrix_integer &matrix) {
    ...
    for (size_t first = 0; first < matrix.rows(); ++first) {
      ...
      for (size_t second = first + 1; second < matrix.rows(); ++second) {
        ...
        COPOSIT_MINIMAL_SAT_G4_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_MINIMAL_SAT_G4_TESTING
  ...
  bool process_subset(const matrix_integer &matrix,
    ...
    if (!from_low_frontier) {
      ...
      if (!floating_filter_.looks_positive_semidefinite(indices_)) {
        ...
        COPOSIT_MINIMAL_SAT_G4_DIAGNOSTICS("high_float_reject",
                                            indices_.size());
#ifdef COPOSIT_MINIMAL_SAT_G4_TESTING
...
#endif
      COPOSIT_MINIMAL_SAT_G4_DIAGNOSTICS("combined_ray", ray + 1);
  ...
  bool add_certificate() {
    ...
    support_context_.release(std::move(lower));
    support_context_.release(std::move(upper));
    COPOSIT_MINIMAL_SAT_G4_DIAGNOSTICS("dickinson", indices_.size());
    return true;
  }
  ...
  void add_upward_closure() {
    ...
    COPOSIT_MINIMAL_SAT_G4_DIAGNOSTICS("support_upward", indices_.size());
  }
  ...
  void add_downward_closure(std::string_view kind) {
    ...
    record_closure_certificate(kind, "downward", certificate_frontier_,
                               indices_);
    COPOSIT_MINIMAL_SAT_G4_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_MINIMAL_SAT_G4_TESTING
    ++downward_count_;
#endif
  }
  ...
  std::optional<minimal_sat_supports> supports_;
#ifdef COPOSIT_MINIMAL_SAT_G4_TESTING
...
#ifdef COPOSIT_MINIMAL_SAT_G4_TESTING
bool minimal_sat_g4_prefers_negative_singular_orientation_for_testing(
...
size_t minimal_sat_g4_combined_ray_sweep_count_for_testing() noexcept {
  return last_combined_ray_sweep_count;
}

size_t minimal_sat_g4_combined_ray_improvement_count_for_testing() noexcept {
  return last_combined_ray_improvement_count;
}

size_t minimal_sat_g4_pair_curvature_exclusion_count_for_testing() noexcept {
  return last_pair_curvature_exclusion_count;
}

size_t
minimal_sat_g4_support_curvature_exclusion_count_for_testing() noexcept {
...
bool minimal_sat_g4_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer &matrix, const std::vector<size_t> &indices) {
...
size_t minimal_sat_g4_shortlist_limit_for_testing(size_t matrix_dimension,
                                                   size_t support_dimension) {
  return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool minimal_sat_g4_prefers_ray_candidate_for_testing(
...
size_t minimal_sat_g4_fixed_support_upper_size_for_testing() noexcept {
  return last_fixed_support_upper_size;
}

size_t count_uncovered_supports(minimal_sat_supports &supports,
                                size_t cardinality) {
...
size_t minimal_sat_g4_uncovered_count(
  ...
  minimal_sat_supports diagram(context);
