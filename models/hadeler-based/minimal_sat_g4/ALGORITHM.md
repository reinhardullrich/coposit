...
Classification: coposit-created exact CP/SCP experiment. Minimal SAT-G4 alternates between low and high cardinalities. The low frontier uses
...
Minimal SAT-G4 uses that observation as a search certificate.
...
Each forbidden interval is translated into one Boolean clause and added to a deliberately minimal DPLL solver. The solver repeatedly
applies unit propagation, then branches chronologically when no literal is forced. It has no clause learning, watched literals,
restarts, or branching heuristic. Two independent cardinality streams start at $1$ and $n$, while sharing the same clauses.
...
No cardinality is materialized as a list. Each frontier stores a stack of disjoint unexplored prefixes. A SAT call combines one
prefix with native exact-cardinality propagation and returns one open support. Splitting the remaining prefix cube around that model
prevents repetitions without adding blocking clauses.
...
The identifier is `minimal_sat_g4`.

- **Minimal SAT** names the intentionally small non-learning DPLL engine.
- **G4** marks the fourth experiment in the support-generator G line.
...
Its only intended change is replacing Improved NBC and its interval compaction with `minimal_sat` and a model-local support adapter. The
...
search as a complete exact CP/SCP classifier are coposit experiments rather than algorithms from those papers. The SAT core is
project-local code in `cpp/minimal_sat.cpp`; the support adapter belongs only to this experiment.
...
factorization confirms it. On the low frontier it proceeds to the Dickinson construction instead. In the second case, Minimal SAT-G4 solves
...
When low-frontier curvature does not already remove the upward closure, Minimal SAT-G4 reuses the retained exact factorization. For a
...
For a nonsingular $B$, the same factorization also solves all coordinate right-hand sides. Minimal SAT-G4 performs the inherited exact
...
The implementation retains $L(u)$ and $U(u)$ for coverage queries and adds exactly this clause to Minimal SAT.
...
Minimal SAT-G4 retains SAT-B3's singular case. Suppose
...
## Minimal SAT Traversal

The SAT variables are the support-membership bits $s_1,\ldots,s_n$. Clauses are stored as vectors of signed integer literals. Unit
propagation scans the clauses until no further variable is forced. The cardinality constraint is propagated directly: after $k$
variables are true, every unassigned variable is false; if every unassigned variable is needed to reach $k$, all are true.

Every query also fixes one unexplored prefix as assumptions. After a model is returned, disjoint sibling prefixes partition the
remainder of that prefix cube. New certificate clauses affect both streams on their next call. The solver deliberately keeps no
learned clauses or other state derived from an earlier query.

## Complete Decision Flow

1. Create the two Minimal SAT streams and install every failed pair-curvature certificate.
...
Minimal SAT-G4 still omits Dickinson intervals on the high frontier. It is difficult when many large principal matrices fail the floating
...
accumulated. Minimal SAT linearly rescans all clauses during propagation and learns nothing from conflicts. Many weak or redundant
certificates can therefore make both repeated SAT calls and the retained prefix stacks expensive. This deliberately small engine is
the intended difference from Improved NBC-B7 and the main object of the comparison.
