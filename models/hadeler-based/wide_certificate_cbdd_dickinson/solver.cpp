...
#ifdef COPOSIT_CBDD_DICKINSON_TESTING
    ...
    bool check(const matrix_integer& matrix)
    {
        for (size_t subset_dimension = 1; subset_dimension <= matrix.rows(); ++subset_dimension) {
            ...
            while (supports_.take_first(indices_)) {
                ...
                COPOSIT_CBDD_DIAGNOSTICS("process", subset_dimension);
...
private:
    bool process_subset(const matrix_integer& matrix)
    {
        ...
        if (diagnostics_.active()) diagnostics_.decision_diagram_certificate(free_indices);
        return true;
    }
    ...
    size_t add_certificate(const matrix_integer& matrix)
    {
        ...
        if (uses_full_certificate_interval(matrix.rows(), indices_.size(), free_indices)) {
            ...
            COPOSIT_CBDD_DIAGNOSTICS("wide-certificate", free_indices);
        } else {
            support processed = support_context_.make();
            for (const size_t index : indices_) support_context_.set(processed, index);
            supports_.add_interval(processed, processed, indices_.size());
            support_context_.release(std::move(processed));
            COPOSIT_CBDD_DIAGNOSTICS("narrow-certificate", free_indices);
        }
