...
class floating_positive_definite_filter {
public:
    explicit floating_positive_definite_filter(size_t dimension)
    ...
    bool looks_positive_definite(const std::vector<size_t>& indices)
    {
...
private:
    ...
    bool process_low_support(const matrix_integer& matrix, size_t& low, size_t high)
    {
        while (low <= high) {
            ...
            COPOSIT_SAT_B2_DIAGNOSTICS("stage_low", low);
    ...
    bool process_high_support(const matrix_integer& matrix, size_t low, size_t& high)
    {
        while (low <= high) {
            ...
            COPOSIT_SAT_B2_DIAGNOSTICS("stage_high", high);
    ...
    bool process_selected_support(const matrix_integer& matrix, pruning_direction direction, bool from_low_frontier)
    {
        ...
        COPOSIT_SAT_B2_DIAGNOSTICS(from_low_frontier ? "process_low" : "process_high", indices_.size());
    ...
    void install_pair_curvature_exclusions(const matrix_integer& matrix)
    {
        ...
        for (size_t first = 0; first < matrix.rows(); ++first) {
            ...
            for (size_t second = first + 1; second < matrix.rows(); ++second) {
                ...
                COPOSIT_SAT_B2_DIAGNOSTICS("pair_upward", 2);
#ifdef COPOSIT_SAT_B2_TESTING
        ...
        const matrix_integer& matrix, pruning_direction direction = pruning_direction::upward, bool from_low_frontier = true)
    {
        if (!from_low_frontier) {
            ...
            if (!floating_filter_.looks_positive_definite(indices_)) {
                supports_->reject_from_high_frontier(indices_);
                COPOSIT_SAT_B2_DIAGNOSTICS("high_float_reject", indices_.size());
#ifdef COPOSIT_SAT_B2_TESTING
...
#endif
            COPOSIT_SAT_B2_DIAGNOSTICS("combined_ray", ray + 1);
    ...
    bool add_certificate()
    {
        ...
        COPOSIT_SAT_B2_DIAGNOSTICS("dickinson", indices_.size());
        support_context_.release(std::move(lower));
        support_context_.release(std::move(upper));
        return true;
    }
    ...
    void add_upward_closure()
    {
        ...
        COPOSIT_SAT_B2_DIAGNOSTICS("support_upward", indices_.size());
    }
    ...
    void add_downward_closure()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_SAT_B2_DIAGNOSTICS("downward", indices_.size());
#ifdef COPOSIT_SAT_B2_TESTING
        ++downward_count_;
#endif
    }
    ...
    void add_exact_support()
    {
        ...
        diagnostics_.certificate();
        COPOSIT_SAT_B2_DIAGNOSTICS("exact", indices_.size());
#ifdef COPOSIT_SAT_B2_TESTING
        ++exact_count_;
#endif
    }
    ...
    floating_positive_definite_filter floating_filter_;
...
#ifdef COPOSIT_SAT_B2_TESTING
bool sat_b2_prefers_negative_singular_orientation_for_testing(size_t positive_products,
...
size_t sat_b2_combined_ray_sweep_count_for_testing() noexcept
{
    return last_combined_ray_sweep_count;
}

size_t sat_b2_combined_ray_improvement_count_for_testing() noexcept
{
    return last_combined_ray_improvement_count;
}

size_t sat_b2_pair_curvature_exclusion_count_for_testing() noexcept
{
    return last_pair_curvature_exclusion_count;
}

size_t sat_b2_support_curvature_exclusion_count_for_testing() noexcept
{
...
{
    floating_positive_definite_filter filter(matrix.rows());
    filter.prepare(matrix);
    return filter.looks_positive_definite(indices);
}

bool sat_b2_reduced_hessian_is_positive_definite_for_testing(
    const matrix_integer& matrix, const std::vector<size_t>& indices)
{
...
size_t sat_b2_shortlist_limit_for_testing(size_t matrix_dimension, size_t support_dimension)
{
    return ray_shortlist_limit(matrix_dimension, support_dimension);
}

bool sat_b2_prefers_ray_candidate_for_testing(size_t candidate_upper, size_t candidate_width, size_t candidate_gains,
...
size_t sat_b2_fixed_support_upper_size_for_testing() noexcept
{
...
size_t sat_b2_pair_exclusion_uncovered_count(
...
size_t sat_b2_interval_clause_size(size_t dimension, uint64_t lower_mask, uint64_t upper_mask)
{
    support_context support_context_(dimension);
    interval_sat diagram(support_context_);
    support lower = support_context_.make();
