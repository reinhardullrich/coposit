...
-- sqlite3 testdata/copos_testdata.sqlite3 < testdata/archive/refresh_truth_and_fastest_after_xxx_2026_08_18.sql
...
CREATE TEMP TABLE complete_preprocessing_evidence AS
SELECT r.matrix_id,
       min(r.is_copositive) AS is_copositive,
...
CREATE TEMP TABLE preprocessing_guard (
    evidence_count INTEGER NOT NULL CHECK(evidence_count = 2729),
    conflict_count INTEGER NOT NULL CHECK(conflict_count = 0),
    truth_mismatch_count INTEGER NOT NULL CHECK(truth_mismatch_count = 0),
    missing_flag_count INTEGER NOT NULL CHECK(missing_flag_count = 0),
    flagged_without_truth_count INTEGER NOT NULL CHECK(flagged_without_truth_count = 0),
    existing_flag_count INTEGER NOT NULL CHECK(existing_flag_count = 2765)
);
...
SELECT count(*),
       ...
       (SELECT count(*) FROM matrices
        WHERE preprocessing_solved
          AND (is_copositive IS NULL OR is_strictly_copositive IS NULL)),
       (SELECT count(*) FROM matrices WHERE preprocessing_solved)
FROM complete_preprocessing_evidence AS e
JOIN matrices AS m USING(matrix_id);
...
CREATE TEMP TABLE final_guard (
    truth_remaining_count INTEGER NOT NULL CHECK(truth_remaining_count = 103),
    preprocessing_solved_count INTEGER NOT NULL CHECK(preprocessing_solved_count = 2765),
    fastest_evidence_count INTEGER NOT NULL CHECK(fastest_evidence_count = 3351),
...
SELECT count(*) FILTER (WHERE m.is_copositive IS NULL OR m.is_strictly_copositive IS NULL),
       count(*) FILTER (WHERE m.preprocessing_solved),
