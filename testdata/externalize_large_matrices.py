...
def main() -> None:
    ...
    try:
        ...
        references = connection.execute(
            "SELECT dimension, matrix FROM matrices WHERE matrix LIKE 'file:%' ORDER BY matrix_id"
        ).fetchall()
        for dimension, reference in references:
            formats[_optimize_matrix_market(database.parent / reference.removeprefix("file:"), dimension)] += 1
        print(
