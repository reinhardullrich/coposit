...
void print_usage(const char* program)
{
    std::cout << "Usage: " << program << " --model MODEL [--mode strict|non-strict|both] [OPTIONS] [MATRIX|FILE|-]\n"
                 ...
                 "  --timeout SECONDS\n"
                 "The fixed preprocessing pipeline is on by default. Combined-capable models default to mode both; other models require --mode.\n"
                 "Models:\n";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        return launch(companion_path(argv[0], model), "coposit --model " + model, argc, argv, timeout);
