...
Status: current snapshot on 2026-08-07.
...
schema documented in `../PROJECT.md`; the original database remains compressed under `reference/fracessa/testdata/`.
...
1. FracESSA needs **strict** copositivity of an exact symmetric matrix:
   ...
   Ordinary copositivity uses `>= 0` instead. The current production checker decides strict copositivity only.
...
2. The complete exact stability path now uses FLINT arbitrary-precision integers. Rational denominators are cleared before the final
   checker, and positive scaling does not change either ordinary or strict copositivity.
...
### 2.1 Strict and ordinary copositivity
...
The distinction is essential. A zero direction rejects strict copositivity but not ordinary copositivity. Several hard examples are
...
Since `d > 0`, `A` and `dA` have the same ordinary and strict copositivity classifications. The core algorithms can therefore use
...
The paper treats ordinary copositivity. The experiment changed terminal inequalities for strict copositivity, retained the slicing
...
SNC is useful as an independent exact implementation and as a possible ordinary-copositivity starting point. It was not selected
for production.
...
- 55 non-copositive;
- 1,031 legacy rows known only to be not strictly copositive, with ordinary copositivity still null.

`is_copositive = NULL` is meaningful. It means ordinary copositivity was not established, not that the matrix is non-copositive.
...
- SHA-256 identity;
- strict and nullable ordinary classification;
...
- `ok`, `timeout`, or `error`;
- nullable strict and ordinary outputs;
...
| `aidocs/handoffs/COPOSITIVITY_CHECKER_EXTRACTION_HANDOFF.md` | This extraction record and file map. | Copy so the new repository retains the decisions and evidence summarized here. |
...
7. **Keep strict and ordinary outputs separate.** The present algorithms usually return only strict copositivity. Do not fill the
   ordinary result field by guessing from a strict failure.
...
clear denominators at the boundary. Ordinary copositivity, witnesses, progress reporting, and algorithm selection should be added
...
4. Can ordinary and strict copositivity share most of the engine while preserving exact boundary behavior?
...
7. Which imported legacy strict-false matrices are ordinary copositive and which are non-copositive? The database intentionally
   leaves 1,031 ordinary classifications unknown.
