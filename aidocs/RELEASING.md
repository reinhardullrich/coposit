...
- the public launcher, its incumbent companion, and a known CP/SCP boundary classification on every CLI platform;
...
Unlike FracESSA's single-file CLI, coposit deliberately keeps its launcher separate from the one-model companion that it supervises.
Each platform asset is therefore one archive containing both executables, `LICENSE`, and `THIRD_PARTY_NOTICES.md`:
...
After extraction, keep `coposit` and `coposit-improved_nbc_x6` in the same directory and invoke only `coposit`. Python wheel
filenames retain their standardized platform tags.
